...
************** END DECLARATION ****************************************
      ...
      fnam = controldir//'calib/PHOS/'//paramscen(:lenparamscen)
     .          //'/phos_names.dat'
       ...
       if (.not.comment(line).and.line(:6).eq.'TARGET') then
         ...
         do while (line(:10).ne.'END TARGET')
          ...
          do while (comment(line).or.line(:10).ne.'END LIMITS')
            ...
            call shift(line)
            if (.not.comment(line)) then
              if (tempar.eq.'KIMP  ') then
                read(line,*,err=992,end=1001) (limitKIMP(i),i=1,4)
              else if (tempar.eq.'KMP   ') then
                read(line,*,err=992,end=1001) (limitKMP(i),i=1,4)
              else if (tempar.eq.'XFIX  ') then
                read(line,*,err=992,end=1001) (limitXFIX(i),i=1,4)
              else if (tempar.eq.'K1    ') then
                read(line,*,err=992,end=1001) (limitK1(i),i=1,4)
              else
                go to 993
              end if
            end if
...
************************************************************************
     ...
     I                  nplts,tarscen,clu,lsegs,nlsegs,ntargets,tarnam,
     O                  targets)
...
994   report(1) = 'land use '//Tseg(:3)// 'is not valid'
      report(2) = fnam
      report(3) = dline
      go to 999
...
996   report(1) = 'Could not find output file for landuse'//luname(nl)
      report(2) = fnam
      report(3) = ' '
      go to 999
