...
C BHATT        read(dfile,'(a110)',err=992,end=111)dline
        read(dfile,'(a113)',err=992,end=111)dline
        call d2x(dline,last)
...
111   close(dfile)
      ...
      do ns = 1, nlsegs
        if (simEOF(ns) .lt. 0.0001) then
          ...
          facKSER(ns) = max(min(facKSER(ns),4),0.05)
          if (facKSER(ns) .gt. 1) facKSER(ns)=1+1.0*(facKSER(ns)-1)/3
C          if (facKSER(ns) .lt. 1) facKSER(ns)=1-0.5*(1-facKSER(ns))/0.95
          if (facKSER(ns) .lt. 1) facKSER(ns)=1-0.2*(1-facKSER(ns))/0.95
        end if
      end do
      print*,'ITER tar sim: ',targets(1),simEOF(1),facKSER(1)
