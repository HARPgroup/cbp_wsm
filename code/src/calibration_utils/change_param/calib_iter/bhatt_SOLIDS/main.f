...
**************GET THE COLUMN NUMBER OF PARAMETER KEIM
     ...
     O                vcKEIM,err)
      ...
      print*,'varline= ',varline
      print*,'table= ',table
      print*,'variable= ',variable
      print*,'VAR COLUMN KEIM = ',vcKEIM
     ...
     O                vcACCSDP,err)
      ...
      print*,'table= ',table
      print*,'variable= ',variable
      vcACCSDP = vcACCSDP + vcKEIM - 1 !?? IT IS A TEMPORARY FIX
      print*,'VAR COLUMN ACCSDP = ',vcACCSDP
      if (err.ne.0) go to 992
***************** LOOP OVER ALL LAND SEGS AND MAKE PARM CHANGES
      do ns = 1,nlsegs  

        done(ns) = .false.
        if (abs(facKEIM(ns)-1.0).lt.0.0025) then
          done(ns) = .true.
          cycle
        end if
...
c        if(KEIM.gt.10) then
         ACCSDP = (1.5*targets(ns)*2000.)/365./1000 ! tons/year to lb/day
         ACCSDP = min(10,ACCSDP) ! using MAX ACCSDP = 10
c        else
c         ACCSDP = 0.1
c        end if
...
*************** put variables in files in memory
        ...
        call putvar(parline(iline),vcACCSDP,ACCSDP)
