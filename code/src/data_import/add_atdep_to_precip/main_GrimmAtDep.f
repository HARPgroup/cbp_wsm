...
C      read(*,'(a)') seg
      read*,seg,dfolder,sdate(1),edate(1),
     .      WriteDaily,WriteMonthly,WriteAnnual
      print*,seg,' ',sdate(1),' ',edate(1)
      call lencl(dfolder,lendfolder)
      print*,dfolder
      print*,dfolder(:lendfolder)
...
******* initialize monthly + annual precipitation
      ...
      do ny = sdate(1),edate(1)
        write(cyear,'(I4)') ny 
        fnam = tree//'input/unformatted/atdep/'//dfolder(:lendfolder)//
...
c      dsn = 2003
c      if (debug) print*,'writing... ',dsn
c      call putdailydsn(wdmfil,sdate,edate,dsn,ndays,dlydryno3)
c      dsn = 2004
c      if (debug) print*,'writing... ',dsn
c      call putdailydsn(wdmfil,sdate,edate,dsn,ndays,dlydrynh4)
      dsn = 2005
      if (debug) print*,'writing... ',dsn
