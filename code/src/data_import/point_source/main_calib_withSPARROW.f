...
** TODO: Use variables (e.g. lrornx, olrornx) to write factors that
** reflect Stream To River transfer of point source loads
...
c        if ( .not.(iTfac.eq.Tfac .and. iTdisPoint.eq.TdisPoint) ) then
c           iTfac      = Tfac
c           iTdisPoint = TdisPoint
c           call getSPARROWfac(Tfac,TdisPoint,lseg//rseg,'PSFAC',
c     .                     SPARROW_WWTPfnam,facQ,facN,facP,facS)
c        end if
...
************* check for TN and TP errors
     ...
     .      abs((Tnh3+Tno3+Torn-Ndum)/Ndum).gt.0.05) then
            ...
            print*,Tfac,TdisPoint,Tfips,ny,nm,nd
        end if
...
************ Write out list of LRsegs to update GEO file
     ...
     .           '/'//wwtp(:lenwwtp)//'_lrsegs.csv'
      ...
      do nlrseg = 1,nlrsegs
        ...
        write(dfile,*) wwtp(:lenwwtp),',',lrseg(nlrseg)(7:last),',',
     .                 lrseg(nlrseg)(:6)
      end do
...
************** add this line to the 57k bay model variables, WWTP+IND
        flow57(n57cell,ny,nm) = flow57(n57cell,ny,nm)+facQ*Tflow*mg2acft   ! add WWTP,IND for the same cell
        heat57(n57cell,ny,nm) = heat57(n57cell,ny,nm)
     .                        + facQ*Tflow*mg2acft*tempC(nm)*acftC2heat
        doxx57(n57cell,ny,nm) = doxx57(n57cell,ny,nm) + Tdox
        nh3x57(n57cell,ny,nm) = nh3x57(n57cell,ny,nm) + facN*Tnh3
...
************ Write out list of LRsegs to update GEO file 
     ...
     .           '/'//indus(:lenindus)//'_lrsegs.csv'
      ...
      do nlrseg = 1,nlrsegs
        call trims (lrseg(nlrseg),last)
        write(dfile,*) indus(:lenindus),',',lrseg(nlrseg)(7:last),',',
     .                 lrseg(nlrseg)(:6)
      end do
...
************* get SPARROW stream to river factors
c        if ( .not.(ilseg.eq.lseg .and. irseg.eq.rseg) ) then
c           ilseg = lseg
c           irseg = rseg
c        if ( .not.(iTfac.eq.Tfac .and. iTdisPoint.eq.TdisPoint) ) then
c           iTfac      = Tfac
c           iTdisPoint = TdisPoint
c           call getSPARROWfac(Tfac,TdisPoint,lseg//rseg,'CSOX',
c           call getSPARROWfac(Tfac,TdisPoint,lseg//rseg,'PSFAC',
c     .                     SPARROW_WWTPfnam,facQ,facN,facP,facS)
c        end if
         comidcb = 199999
         facQ = SPARROW_CSOfactors(comidcb,1)
         facN = SPARROW_CSOfactors(comidcb,2)
         facP = SPARROW_CSOfactors(comidcb,3)
         facS = SPARROW_CSOfactors(comidcb,4)
...
************** add this line to lrseg variables
     ...
     .                         + facQ*Tflow*mg2acft*tempC(nm)*acftC2heat
        ...
        CSOnh3x(nCSOlrseg,jday) = CSOnh3x(nCSOlrseg,jday) + facN*Tnh3
        CSOno3x(nCSOlrseg,jday) = CSOno3x(nCSOlrseg,jday) + facN*Tno3
        CSOpo4x(nCSOlrseg,jday) = CSOpo4x(nCSOlrseg,jday) + facP*Tpo4
     ...
     .                          + (facN*Torn-F_BOD*bod2lorn) * rorn2rorc
        CSObodx(nCSOlrseg,jday) = CSObodx(nCSOlrseg,jday) + F_BOD
        CSOtssx(nCSOlrseg,jday) = CSOtssx(nCSOlrseg,jday)
C     .                            + max(0.0,Ttss-Tbod*bod2tss)
     .                         + facS*Ttss

        oCSOflow(nCSOlrseg,jday) = oCSOflow(nCSOlrseg,jday)
     .                            + Tflow*mg2acft
        oCSOheat(nCSOlrseg,jday) = oCSOheat(nCSOlrseg,jday)
     .                            + Tflow*mg2acft*tempC(nm)*acftC2heat
        oCSOdoxx(nCSOlrseg,jday) = oCSOdoxx(nCSOlrseg,jday) + Tdox
        oCSOnh3x(nCSOlrseg,jday) = oCSOnh3x(nCSOlrseg,jday) + Tnh3
        oCSOno3x(nCSOlrseg,jday) = oCSOno3x(nCSOlrseg,jday) + Tno3
        oCSOpo4x(nCSOlrseg,jday) = oCSOpo4x(nCSOlrseg,jday) + Tpo4

         ! for organics, calculate the labile portion and compare it
         !  to the total organic, if labile is greater than total,
         !  reset BOD so that labile is equal to total for the most limiting nutrient
        F_BOD = min(Torn/bod2lorn,Torp/bod2lorp,Tbod)
        oCSOornx(nCSOlrseg,jday) = oCSOornx(nCSOlrseg,jday)
     .                            + Torn-F_BOD*bod2lorn
        oCSOorpx(nCSOlrseg,jday) = oCSOorpx(nCSOlrseg,jday)
     .                            + Torp-F_BOD*bod2lorp
        oCSOorcx(nCSOlrseg,jday) = oCSOorcx(nCSOlrseg,jday)
     .                            + (Torn-F_BOD*bod2lorn) * rorn2rorc
        oCSObodx(nCSOlrseg,jday) = oCSObodx(nCSOlrseg,jday) + F_BOD
        oCSOtssx(nCSOlrseg,jday) = oCSOtssx(nCSOlrseg,jday)
C     .                            + max(0.0,Ttss-Tbod*bod2tss)
     .                         + Ttss
************** add this line to cell variables
     ...
     .                         + facQ*Tflow*mg2acft*tempC(nm)*acftC2heat
        ...
        CSOcellnh3x(nCSOcell,jday) = CSOcellnh3x(nCSOcell,jday) 
     .                               + facN*Tnh3
        CSOcellno3x(nCSOcell,jday) = CSOcellno3x(nCSOcell,jday) 
     .                               + facN*Tno3
        CSOcellpo4x(nCSOcell,jday) = CSOcellpo4x(nCSOcell,jday) 
     .                               + facP*Tpo4
     ...
     .                          + (facN*Torn-F_BOD*bod2lorn) * rorn2rorc
        CSOcellbodx(nCSOcell,jday) = CSOcellbodx(nCSOcell,jday) + F_BOD
...
************ Write out list of LRsegs to update GEO file
     ...
     .           '/'//cso(:lencso)//'_lrsegs.csv'
      ...
      do nCSOlrseg = 1,nCSOlrsegs
        ...
        write(dfile,*) cso(:lencso),',',CSOlrseg(nCSOlrseg)(7:last),',',
...
************* put in the wdm
      ...
      do nCSOlrseg = 1,nCSOlrsegs
         ...
         do ny = y1,y2
          ...
          write(dfile,1234),'cso,',CSOlrseg(nCSOlrseg)(7:last),',',
     .     CSOlrseg(nCSOlrseg)(:6),',',ny,annflow,annheat,
     .     anntssx,annnh3x,annno3x,annorgn,annpo4x,
     .     annorpx,annbodx,anndoxx,annorcx
...
999   call stopreport(report)
      ...
      subroutine getSPARROWfac(facility,dispoint,lrseg,srcx,sparrowfnam,
     .            facQ,facN,facP,facS)
      ...
      integer      tsno, tcomid, toutfall, tlocation
      character*10 tid
      character*13 tpsfacility
      character*19 tlrseg
      real         tfloSTR,ttnxSTR,ttpxSTR,tsedSTR
...
c      print*,sparrowfnam
c      print*,facility,' ',dispoint
      open(dfile+1,file=sparrowfnam,status='old',iostat=err)
      if (err.ne.0) go to 991

      read(dfile+1,'(a200)')longline  ! ditch header row
c      print*,'header-> ',longline

      do 
        read(dfile+1,'(a200)',end=223,err=996) longline
        call d2x(longline,last)
c        print*,longline
c        if (longline(:3).eq.'end') go to 223
        !SNo,COMID,Outfall,ID,Location,PS_FACILIT,P6LndRvrSe,floSTR,tnxSTR,tpxSTR,sedSTR
        !1,2598619,1,PA0063321,001,PA0063321001,H42115SU2_0670_0810,1.0000,0.4073,0.0323,1.0000
        read(longline,*,end=992,err=992)
c     .      tsno,tcomid,toutfall,
     .      tid,tlocation,tpsfacility,tlrseg,
     .      tfloSTR,ttnxSTR,ttpxSTR,tsedSTR
        if (srcx .eq. 'LRSEG') then
           if (tlrseg.eq.lrseg) then
              facQ = tfloSTR
              facN = ttnxSTR
              facP = ttpxSTR
              facS = tsedSTR
           ...
           if (facility.eq.tid .and. dispoint.eq.tlocation) then
              facQ = tfloSTR
              facN = ttnxSTR
              facP = ttpxSTR
              facS = tsedSTR
...
223   close(dfile+1)
      print*,srcx,' ',facility,' ',dispoint,' ',lrseg,' -> ',
     .        facQ,facN,facP,facS
