      ...
      character*20  tsensitivity(nquals)
      character*2   tsmethod(nquals)

      real          out_calib_quals(nquals) !for each quals
      real          out_calib_qualg(nquals) !for each qual groups

      real          out_calib_states
      real          out_calib_state(nstates)
...
112         close (dfile)
     ...
     .        //geoscen(:lengeoscen)//'/perlnd'
      ...
      fnam = 'inputs/PQUAL_vs_Input.csv'
      open(dfile,file=fnam,status='old',iostat=err)
      if (err.ne.0) go to 991
     ...
     .          apptypeslong(i),(inp_scen_app(i,j),j=1,nappspecies)
      ...
      ! *
         ...
         !{
            ...
            call lencl(apptypeslong(i),tInt)
            fnam = 'inputs/'//luname(l)//'_'//apptypeslong(i)(:tInt)
     .                 //'_sensitivity.csv'
...
c               print*,(tsensitivity(j),j=1,nquals)
               ...
               !{
                  do j = 1,nquals
                  !{
                     read(tsensitivity(j)(:1),*,end=996,err=996),
     .                       sgroup( tquals(j) )
                     read(tsensitivity(j)(3:),*,end=996,err=996),
     .                       sensitivity( tquals(j) )
                     print*,(sgroup(k),k=1,nquals)
                     print*,(sensitivity(k),k=1,nquals)
c                     sensitivity( tquals(j) ) = tsensitivity(j)
...
118         close(dfile)
            ...
            do j = 1,nquals
               do k = 1,nstates
                  dsn = OutWDMs(j,k)
                  n = sgroup(j)
                  print*,'sgroup:',j,k,sgroup(j)
                  if ( dsn.eq.999 ) cycle
                  call gethourdsn(wdmfil,sdate,edate,dsn,nvals,hvals)
                  do m = 1,nvals
                  !{
                     out_calib_quals(j) = out_calib_quals(j) + hvals(m)
                     out_calib_qualg(n) = out_calib_qualg(n) + hvals(m)
                  !}
                  end do
               end do
            end do
            print*,'out_calib_quals',(out_calib_quals(j),j=1,nquals)
            print*,'out_calib_qualg',(out_calib_qualg(j),j=1,nquals)
            ...
            !{
               ...
               !{
                  ...
                  do m = 1,nvals
                  !{
                     out_calib_state(k) = out_calib_state(k) + hvals(m)
                     out_calib_states   = out_calib_states   + hvals(m)
                  !}
                  end do
               !}
               end do
               ...
               !{
                  !> calculate delta_out
                  print*,'delta_out(nqual=',j,',nstates) = ',
     .                    (delta_out(j,m),m=1,nstates)
                  print*,apptypeslong(i),
     .          ((inp_scen_app(i,m)-inp_calib_app(i,m)),m=1,nappspecies)
                  do m = 1,nappspecies
                  !{
                     if (smethod(j).eq.'D1') then
                     !{
                        delta_out(j,k) = delta_out(j,k)
     .                     + pquals_vs_inputs(j,k,m)
     .                     * sensitivity(j)
     .                     * ( inp_scen_app(i,m) - inp_calib_app(i,m) )
     .             * ( out_calib_quals(j) / out_calib_qualg(sgroup(j)) )
     .                     * ( out_calib_state(k) / out_calib_states )
                        print*,'#A',j,k,m,
     .                     ( inp_scen_app(i,m) - inp_calib_app(i,m) ),
     .                     sensitivity(j),
     .                     out_calib_state(k),
     .                     out_calib_state(k)/out_calib_states,
     .                     pquals_vs_inputs(j,k,m),delta_out(j,k)
                     !}
                     else
                     !{
                        go to 995
...
C               write(*,*),'qual=',j,' state=',k,' out_calib=',
C     .                out_calib, ' delta_out=',
C     .                delta_out(j,k), ' pcent_delta=',pcent_delta
                write(*,'(6H qual=,I2$)') j
                write(*,'(7H state=,I2$)') k
                write(*,'(11H out_calib=,E12.5$)') out_calib
                write(*,'(11H delta_out=,E12.5$)') delta_out(j,k)
                write(*,'(13H pcent_delta=,F7.4)') pcent_delta
c               566  FORMAT(5Hqual=,I2,6Hstate=,I2,10Hout_calib=,F10.2,10Hdelta_out=,F10.2,11Hpcent_delta,F10.2)
               do m = 1,nvals
               !{
...
996   report(1) = 'Problem parsing sensitivity data:'
      report(2) = tsensitivity(j)//' '//longline
      report(3) = 'Oops! this method not yet impelemented?'
      go to 999
