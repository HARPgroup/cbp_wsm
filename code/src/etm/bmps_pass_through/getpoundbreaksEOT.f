...
*********** END DECLARATIONS *******************************************
      ...
      do nt = 1, maxTimeBreaks               ! initialize BmpAcres
        do nb = 1,maxBMPcon
          do i = 1, maxL2R
            do l = 1,nlu
              PoundsReducedEOT(nt,nb,i,l) = 0.0
            end do
          end do
        end do
      end do
      ...
      do nt = 1,nPB
        ...
        fnam= ScenDatDir//'river/bmps/bmp_loadredux_EOT_'//
     .             PBfile(nt)(:lastPB)//'.csv'
        ...
        do nb1 = 1,nBmpCon
          ...
          found = .false.
          do nb2 = 1,nBmpConEOT
            if (debug) print*,C_BMPconnameEOT(nb2)
            if (.not.scompcase(ctemp(:i),C_BMPconnameEOT(nb2))) cycle
C            Pnb(nb1) = I_BMPconnameEOT(nb2)
             Pnb(nb1) = nb2
            found = .true.
          end do
...
c          if (.not.found) go to 998
          ...
          do nb = 1,nBmpConEOT
            ...
            if (debug) print*,C_BMPconnameEOT(nb),' -> ',
     .             BMPconname(I_BMPconnameEOT(nb))
            PoundsReducedEOT(nt,Pnb(nb),nl2r,ll) = 
     .                  PoundsReducedEOT(nt,Pnb(nb),nl2r,ll) + Tpounds
c            print*,nb,Pnb(nb),Tpounds
          end do
...
C     .                (',',BmpName(nBmp),nBmp=1,nBmpTypes)
        write(99,*) 'time,lseg,nlu',(',',BMPconname(nb),nb=1,nBmpConEOT)
        do nt = 1,nPB
          do i = 1,numsegs
            do l = 1,nlu
              write(99,333) PBfile(nt)(:lastPB),',',l2r(i),',',
     .                      luname(l),
     .          (',',PoundsReducedEOT(nt,nb,i,l),nb=1,nBmpConEOT)
            end do
          end do
        end do
        close(99)
