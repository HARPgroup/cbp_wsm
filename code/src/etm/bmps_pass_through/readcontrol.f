...
************************************************************************
     ...
     O                       EndY,EndM,EndD)
      ...
      integer PBEOTyear(maxTimeBreaks)             ! bmp lb factor year
      integer PBEOTmonth(maxTimeBreaks)            ! bmp lb factor month
      integer PBEOTday(maxTimeBreaks)              ! bmp lb factor day
      ...
      integer nLB,nBB,nPB,nPBEOT,l,n,i
...
CBHATT          else if (line(:8).eq.'BMPACRES') then
          else if (line(:12).eq.'BMP PASSTHRU') then
            findbmp = .true.
            nBB = 0
            read(dfile,'(a100)',err=1001)line
CBHATT            do while (line(:12).ne.'END BMPACRES')
            do while (line(:16).ne.'END BMP PASSTHRU')
              if (.not.comment(line)) then
                nBB = nBB + 1
                templine(nBB) = line
              end if
...
c              print*,'BMPPOUNDS '//line
     ...
     .              PByear(n),PBmonth(n),PBday(n),PBfile(n)
          ...
          else if (line(:12).eq.'EOTBMPPOUNDS') then
c            print*,'findpoundEOT'
            findpoundEOT = .true.
            nPBEOT = 0
            read(dfile,'(a100)',err=1001)line
            do while (line(:16).ne.'END EOTBMPPOUNDS')
              if (.not.comment(line)) then
                nPBEOT = nPBEOT + 1
                templine(nPBEOT) = line
              end if
              read(dfile,'(a100)',err=1001)line
            end do
            do n = 1,nPBEOT
c              print*,templine(n)
              read(templine(n),1234)
     .              PBEOTyear(n),PBEOTmonth(n),PBEOTday(n),PBEOTfile(n)
            end do
...
CBHATT      if (.not.findbmp) call stopit(fnam,'BMPACRES')
      if (.not.findbmp) call stopit(fnam,'BMP PASSTHRU')
      if (.not.findpound) call stopit(fnam,'BMPPOUNDS')
      if (.not.findpoundEOT) call stopit(fnam,'BMPPOUNDSEOT')
     ...
     .                        PByear(n),PBmonth(n),PBday(n))
      ...
      do n = 1, nPBEOT
        PBEOTJday(n) = julian(StartY,StartM,StartD,
     .                        PBEOTyear(n),PBEOTmonth(n),PBEOTday(n))
      end do
      call sortB(PBEOTJday,PBEOTfile,nPBEOT)

      return

1234  format(i4,i3,i3,1x,a50)
