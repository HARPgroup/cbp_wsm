************************************************************************
** The program reads STR monthly loads from a file generated by       **
**    Scenario Builder, and then covert the loads to hourly values    **
**    and put into land WDM                                           **
************************************************************************
      subroutine getstrload(rscen,rseg,lscen,lseg,
     ...
     O                      nvals,hval)
      ...
      integer nstrloads
      parameter (nstrloads=6)                                 ! number of str load
      character*4 loadnm(nstrloads)
...
*************** load related variables
      ...
      real load(maxTimeBreaks,12,nstrloads),F_tload,tmon
      real monload(EarliestYear:LatestYear,12,nstrloads)

*************** WDM related variables
      integer wdmfilstr
      parameter (wdmfilstr=dfile+61)
...
********** open land WDM to get flow info
     ...
     .          '/'//clu//lseg(:lenlseg)//'.wdm'
      call wdbopnlong(wdmfilstr,wdmfnam,1,err)     ! open land read only
      if (err .ne. 0) go to 996

      call gethourdsn(wdmfilstr,sdate,edate,flowdsn,
     O                nvals,flowvals)

      call gethourdsn(wdmfilstr,sdate,edate,WSSDdsn,
     O                nvals,WSSDvals)

      call wdflcl(wdmfilstr,err)
...
******** READ THE CONTROL FILE FOR STR LOAD FILE
...
********read in str loads            
      do n = 1,nLB
        ...
        fnam = ScenDatDir//'river/strloads/strload_'//
     .       LBfile(n)(:last)//'.csv'
...
c                print*,Trseg,Tlseg,Tlu
                ...
                do nl = 1,nstrloads+1
                  if (nl.eq.nstrloads+1) go to 994
                  if (Tload.eq.loadnm(nl)) exit
                end do
                foundload(nl) = .true.
                call shift(dline)
                call fread(dline,F_tload)
                do mon = 1,12
c                  call shift(dline)
c                  call fread(dline,load(n,mon,nl))
                   load(n,mon,nl) = F_tload
                end do
              end if
            end if
          end if
...
********** get hourly loads
      ...
      do i = 1,nvals
        ...
        hSEDM(i) = SEDMload(year,month)*WSSDvals(i)
