...
************** variables to calcuate pound BMPs
     ...
     .     fLORN(ndaymax*24), fLORP(ndaymax*24)
      ...
      real TOT_RORN,TOT_RORP,TOT_LORN,TOT_LORP

      integer iWATR
      real fWATR(ndaymax*24)
      real MAX_WATR

      real fLOAD(ndaymax*24)
      real TOT_LOAD

      real fCONC(ndaymax*24)
      real MAX_CONC
...
************* HANDLING NO3D and TOTN Balance: START ***********
     ...
     .                 fNH3C(nv)+fRORN(nv)+fLORN(nv)
          ...
          print*,rseg(:lenrseg),' old NOX fraction = ',AVG_NO3D/AVG_TOTN
          call getnox(
     ...
     O           NOXfrac)
          ...
          do nv = 1,nvals
            if ( AVG_NO3D .gt. 0 ) then
              hvaltemp(nv,iNO3D) = fNO3D(nv) * NO3Dfac
            else
              hvaltemp(nv,iNO3D) = fTOTN(nv) * NOXfrac 
            end if
            hvaltemp(nv,iNH3D) = fNH3D(nv) * NH3Xfac
            hvaltemp(nv,iNH3A) = fNH3A(nv) * NH3Xfac
            hvaltemp(nv,iNH3I) = fNH3I(nv) * NH3Xfac
            hvaltemp(nv,iNH3C) = fNH3C(nv) * NH3Xfac
            hvaltemp(nv,iRORN) = fRORN(nv) * ORGNfac
            hvaltemp(nv,iLORN) = fLORN(nv) * ORGNfac
          end do
        !}
        end if
...
************* HANDLING OF ORGN and ORGP Delivery: START ********
        MAX_WATR = 0.0
        TOT_RORN = 0.0
        TOT_RORP = 0.0
        TOT_LORN = 0.0
        ...
        iLORP = -9
        do Rvar = 1,nRvar
          if ( Rname(Rvar) .eq. 'WATR' ) iWATR = Rvar
          if ( Rname(Rvar) .eq. 'RORN' ) iRORN = Rvar
          if ( Rname(Rvar) .eq. 'RORP' ) iRORP = Rvar
          if ( Rname(Rvar) .eq. 'LORN' ) iLORN = Rvar
          if ( Rname(Rvar) .eq. 'LORP' ) iLORP = Rvar
        end do
        if ( .not. (iRORN.eq.-9 .or. iRORP.eq.-9 .or. iWATR.eq.-9 .or.
     .              iLORN.eq.-9 .or. iLORP.eq.-9 ) ) then
        !{
          do nv = 1,nvals
            fWATR(nv) = hvaltemp(nv,iWATR)
            MAX_WATR  = max(MAX_WATR,hvaltemp(nv,iWATR))
            TOT_RORN = TOT_RORN + hvaltemp(nv,iRORN)
            TOT_RORP = TOT_RORP + hvaltemp(nv,iRORP)
            TOT_LORN = TOT_LORN + hvaltemp(nv,iLORN)
            TOT_LORP = TOT_LORP + hvaltemp(nv,iLORP)
          end do
          print*,'TOT_RORN = ',TOT_RORN
          print*,'TOT_RORP = ',TOT_RORP
          print*,'TOT_LORN = ',TOT_LORN
          print*,'TOT_LORP = ',TOT_LORP

********** POWER FUNCTION FORMULATION
c          PARA_A  =  1.0        ! ( (a + q/qmax)/(a + 1) ) ^ b
c          PARA_B  =  6.0 ! 10.0

c          MAX_CONC = 0.0
c          do nv = 1,nvals
c            fCONC(nv) = ((PARA_A+fWATR(nv)/MAX_WATR)/(PARA_A+1))**PARA_B
c            MAX_CONC = max(MAX_CONC,fCONC(nv))
c          end do

********** ERODIBILITY FORMULATION
c          PARA_A = 0.20 ! critical flow, 0.98, 0.90, 0.50
c          MAX_CONC = 1.0
c          do nv = 1,nvals
c            if ( fWATR(nv)/MAX_WATR .lt. PARA_A ) then
c              fCONC(nv) = 0.0
c            else
c              fCONC(nv) = ((fWATR(nv)/MAX_WATR)/(PARA_A)) - 1
cc              MAX_CONC = max(MAX_CONC,fCONC(nv))
c            end if
c          end do
...
********** C = a * EXP( b*q/qmax )
          ...
          MAX_CONC = 1.0
          do nv = 1,nvals
            fCONC(nv) = PARA_A * EXP(PARA_B*fWATR(nv)/MAX_WATR)
c            MAX_CONC  = max(MAX_CONC,fCONC(nv))
          end do

          TOT_LOAD = 0.0
          do nv = 1,nvals
            fLOAD(nv) = fWATR(nv) * fCONC(nv)/MAX_CONC
            TOT_LOAD  = TOT_LOAD + fLOAD(nv)
          end do
          do nv = 1,nvals
            hvaltemp(nv,iRORN) = ( TOT_RORN / TOT_LOAD ) * fLOAD(nv)
            hvaltemp(nv,iRORP) = ( TOT_RORP / TOT_LOAD ) * fLOAD(nv)
            hvaltemp(nv,iLORN) = ( TOT_LORN / TOT_LOAD ) * fLOAD(nv)
            hvaltemp(nv,iLORP) = ( TOT_LORP / TOT_LOAD ) * fLOAD(nv)
          end do

        !}
        end if
************* HANDLING OF ORGN and ORGP Delivery: STOP  ********
