...
************** variables to calcuate pound BMPs
     ...
     .        iPO4D, iPO4A, iPO4I, iPO4C, iRORP, iBODA
     ...
     .     fLORN(ndaymax*24), fLORP(ndaymax*24)
      real fTNO3, fTOTN, fDPO4, fTPO4
      real splitNO3, splitDPO4
      real facNO3, facxNO3, facDPO4, facxPO4
...
************* HANDLING SPLITS : START ***********
        ! iNO3D, iNH3D, iNH3A, iNH3I, iNH3C, iRORN, iPO4D, iPO4A, iPO4I, iPO4C, iRORP, iBODA
        ! fNO3D, fNH3D, fNH3A, fNH3I, fNH3C, fRORN, fPO4D, fPO4A, fPO4I, fPO4C, fRORP, fBODA, fLORN, fLORP
        ! fTNO3, fTOTN, fDPO4, fTPO4
        ! splitNO3, splitDPO4
        ! facNO3, facxNO3, facDPO4, facxPO4
        call getSplitNO3(ioscen,lenioscen,rseg,lenrseg,splitNO3)
        call getSplitDPO4(ioscen,lenioscen,rseg,lenrseg,splitDPO4)
        CVP2N   = 7.23
        CVN2BOD = 22.9
        do Rvar = 1,nRvar
        ...
        do nv = 1,nvals
           ...
           fLORN(nv) = hvaltemp(nv,iBODA) / CVN2BOD
           fLORP(nv) = ( hvaltemp(nv,iBODA) / CVN2BOD ) / CVP2N
        end do
        ...
        fTPO4 = 0.0
        do nv = 1,nvals
           fTNO3 = fTNO3 + fNO3D(nv)
           fTOTN = fTOTN + fNO3D(nv) + fNH3D(nv) + fNH3A(nv) + 
     .                     fNH3I(nv) + fNH3C(nv) + fRORN(nv) + fLORN(nv)

           fDPO4 = fDPO4 + fPO4D(nv)
           fTPO4 = fTPO4 + fPO4D(nv) + fPO4A(nv) + fPO4I(nv) + fPO4C(nv)
        end do
        print*,'Before Split'
        print*,'TNO3 TOTN',fTNO3,fTOTN
        print*,'DPO4 TPO4',fDPO4,fTPO4

        facNO3  = splitNO3  / (fTNO3/fTOTN)
        facxNO3 = ( 1 - splitNO3 ) / ( 1 - (fTNO3/fTOTN) )
        facDPO4 = splitDPO4 / (fDPO4/fTPO4)
        facxPO4 = ( 1 - splitDPO4 ) / ( 1 - (fDPO4/fTPO4) )

        do nv = 1,nvals
           fNO3D(nv) = facNO3  * fNO3D(nv)

           fNH3D(nv) = facxNO3 * fNH3D(nv)
           fNH3A(nv) = facxNO3 * fNH3A(nv)
           fNH3I(nv) = facxNO3 * fNH3I(nv)
           fNH3C(nv) = facxNO3 * fNH3C(nv)
           fRORN(nv) = facxNO3 * fRORN(nv)
           fLORN(nv) = facxNO3 * fLORN(nv)

           fPO4D(nv) = facDPO4 * fPO4D(nv)
           fPO4A(nv) = facxPO4 * fPO4A(nv)
           fPO4I(nv) = facxPO4 * fPO4I(nv)
           fPO4C(nv) = facxPO4 * fPO4C(nv)
        ...
        do nv = 1,nvals
           hvaltemp(nv,iNO3D) = fNO3D(nv)
           hvaltemp(nv,iNH3D) = fNH3D(nv)
           hvaltemp(nv,iNH3A) = fNH3A(nv)
           hvaltemp(nv,iNH3I) = fNH3I(nv)
           hvaltemp(nv,iNH3C) = fNH3C(nv)
           hvaltemp(nv,iRORN) = fRORN(nv)
           hvaltemp(nv,iPO4D) = fPO4D(nv)
           hvaltemp(nv,iPO4A) = fPO4A(nv)
           hvaltemp(nv,iPO4I) = fPO4I(nv)
           hvaltemp(nv,iPO4C) = fPO4C(nv)
        ...
        do nv = 1,nvals
           fTNO3 = fTNO3 + fNO3D(nv)
           fTOTN = fTOTN + fNO3D(nv) + fNH3D(nv) + fNH3A(nv) +
     .             fNH3I(nv) + fNH3C(nv) + fRORN(nv) + fBODA(nv)/CVN2BOD

           fDPO4 = fDPO4 + fPO4D(nv)
           fTPO4 = fTPO4 + fPO4D(nv) + fPO4A(nv) + fPO4I(nv) + fPO4C(nv)
        end do
        print*,'After Split'
        print*,'NO23 TOTN',fTNO3,fTOTN
        print*,'DPO4 TPO4',fDPO4,fTPO4
************* HANDLING SPLITS : FINISH ***********
...
999   call stopreport(report)
      ...
      subroutine getSplitNO3(ioscen,lenioscen,rseg,lenrseg,splitNO3)
         implicit none
         include '../../lib/inc/standard.inc'
         include '../../lib/inc/locations.inc'
         real splitNO3
         fnam = catdir//'iovars/'//ioscen(:lenioscen)//
     .         '/variable_l2r_factors/NO23_split.csv'
         open(dfile,file=fnam,status='old',iostat=err)
         if (err.ne.0) go to 990
         read(dfile,'(a100)') line ! header line
         do
            read(dfile,'(a100)',err=991,end=992) line
            call d2x(line,last)
            call findcomma(line,last)
C            print*,'<',line(:last-1),'><',rseg(:lenrseg),'>'
            if (line(:last-1).eq.rseg(:lenrseg)) then
               call shift(line)
               call fread(line,splitNO3)
               !call findcomma(line,last)
               !splitNO3 = line(:last-1)
               print*,'NO23split ',rseg(:lenrseg),splitNO3
               close(dfile)
               return
            end if
         end do
...
990   report(1) = 'Problem: unable to open file'
      report(2) = fnam
      report(3) = 'from subroutine getSplitNO3'
      go to 999
991   report(1) = 'Could not read file: near line: '
      report(2) = fnam
      report(3) = line
      go to 999
992   report(1) = 'Could not find rseg in getSplitNO3'
      report(2) = rseg(:lenrseg)
      write(report(3),*) fnam
      go to 999
999   call stopreport(report)
      end

      subroutine getSplitDPO4(ioscen,lenioscen,rseg,lenrseg,splitDPO4)
         implicit none
         include '../../lib/inc/standard.inc'
         include '../../lib/inc/locations.inc'
         real splitDPO4
         fnam = catdir//'iovars/'//ioscen(:lenioscen)//
     .         '/variable_l2r_factors/PO4D_split.csv'
         open(dfile,file=fnam,status='old',iostat=err)
         if (err.ne.0) go to 990
         read(dfile,'(a100)') line ! header line
         do
...
C            print*,'<',line(:last-1),'><',rseg(:lenrseg),'>'
            if (line(:last-1).eq.rseg(:lenrseg)) then
               call shift(line)
               call fread(line,splitDPO4)
               !call findcomma(line,last)
               !splitDPO4 = line(:last-1)
               print*,'DPO4split ',rseg(:lenrseg),splitDPO4
               close(dfile)
               return
            end if
         end do
...
990   report(1) = 'Problem: unable to open file'
      report(2) = fnam
      report(3) = 'from subroutine getSplitDPO4'
      go to 999
991   report(1) = 'Could not read file: near line: '
      report(2) = fnam
      report(3) = line
      go to 999
992   report(1) = 'Could not find rseg in getSplitDPO4'
      report(2) = rseg(:lenrseg)
      write(report(3),*) fnam
      go to 999
999   call stopreport(report)
