...
c                else if ( luname(l).eq.'dst' ) then
     ...
     O                            nvals,hval)                           
                ...
                print*,Lname(Rvar,l,Lvar)
                if ( Lname(Rvar,l,Lvar) .eq. 'ANO3' ) then
                   if ( L_rSAS .eqv. .true. ) then
                      print*,luname(l),isastrlu(l),
     .       luname(l).eq.'fnp',luname(l).eq.'fsp',isastrlu(l),
     . (.not.(luname(l).eq.'fnp' .or.luname(l).eq.'fsp').or.isastrlu(l))
                      if (.not.(
     .                   luname(l).eq.'fnp' .or. 
     .                   luname(l).eq.'fsp') .or. 
     .                   isastrlu(l) ) then
                            ...
                            !>> Get rSAS Loads
                            call getrsasload(
     I    rscen,lenrscen,rseg,lenrseg,LandScen(l),lenls,l2r(ns),lenlseg,
     I    luname(l),sdate,edate,Lname(Rvar,l,Lvar),
     I    nvals,
     M    hval)
