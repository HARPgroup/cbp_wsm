...
!    M                 NO3,VOL, dddnit) 
!    o               vol, dddnit)
          call denit (tw, no3, volsp, dddnit) 
...
!!! note fctdv2 = 1/zdpth, 
        ...
        ddnit = 0.0
          dvvt  =                 dvvt *(TzDEN**(TW*20.))
          umaxt =                 umaxt*(TzDEN**(TW-20.))  ! red fro segmen
!            goto 221
         if(cno3w .gt. cno3s) then
          dnitv = dvvt * (cno3w-cno3s)/fctdv2  ! diffus rate:water 2 soil
          dnitv = dnitv*bedarea*porop       ! * area * time = distance or mass into
         else            ! kg / time step ->      
          dnitv = 0.
         endif
!         dnitv  = min(wtno3w*0.95, dnitv)
          dnitv  = min(wtno3w*0.95, dnitv*1000.) ! ????? -> g if from kg 
!        dw2s = 0.000000000
          wtno3w = wtno3w - dnitv
          dnitn = umaxt* cno3s / (cno3s + dnt50)  !kg/m2/depth
          dnitn = dnitn * bedarea * porop * zdpth
!         dnitn = min(wtno3s*0.95, dnitn)  
          dnitn = min(wtno3s*0.95, dnitn*1000.) ! ???? ->g if from kg, uhr unit
!        ddnit = 0.000000000
          wtno3s = wtno3s + dnitv - dnitn   ! update g? 
!         cno3s  = wtno3s * 1000. / (bedarea*porop*zdpth) ! update, g/m3 = mg/l
          cno3s  = wtno3s *1.    / vsoilw    ! update, g / m3 = mg/l
          NO3 = wtno3w / (VOLtt     *0.0283168) !not times 43560: g / m3 -> mg/l )
!         NO3 = wtno3w / ((VOL/delt60)*43560.*0.0283168)  ! not times 43560. )
!         ddnit = (ddnit + dnit1)        * 2.2045 !      kg -> pound  
          ddnit = (        dnitv)        * 2.2045 !      kg -> pound  
221      continue 
         goto 222
