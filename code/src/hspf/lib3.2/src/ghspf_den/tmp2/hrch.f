...
C     set flux accumulators to zero
 ...
 20   CONTINUE
C
! pwo VSOILW = ength * bedwidd * zdpth * porop 
! pwo bedarea = ength * bedwidd 
...
!!!!!!!!!!!!!!!!!!!!! pw Rhyd
      ...
      iTBSB= 1
      iNVAL= 7
      CALL RTABLE (iTBNO,iTBSB,iNVAL,UUNITS,
     M             RRVAL)
!       ENGTH = 8000.  ! RRVAL(3) * 1609.
        ENGTH =          RRVAL(3) * 1609.
C  sed..   get general sediment-related parms- table-type sed-genparm
      I2= 29
      I4= 3
      I1 = 1
      CALL RTABLE
...
!    I               (I2,I1,I4,UUNITS,
!    M               RRVAL)
!      DVVHR   =0.000000005   ! RRVAL(1) * 0.000001
!      zdpth   =10.  ! RRVAL(2)
!      css     =0.2  ! RRVAL(3)  !
!      sno3s   = css
       fctdv2  = 1./ zdpth
...
!hyd     process values in table-type HYDR-INIT
      iTBNO= 7
      iTBSB= 1
      iNVAL= 12
      CALL RTABLE (iTBNO,iTBSB,iNVAL,UUNITS,
     M             RRVAL)
      vwater = 40000.  ! RRVAL(1) * 43560.  ! ft3
...
!!!!!!!!!!!!!!!!!!!!! pw end!
       ...
       cssss   =0.2  ! RRVAL(3)  !
       sno3s   = css
          write(*,*) 'hrch, 0','icont,ength,zdpth,vsoilw,cssss,dvvhr'
          write(*,*) 'hrch, 0', icnnt,ength,zdpth,vsoilw,cssss,dvvhr 
C
C     time loop
!      iyytt = 0  ! pw 
!         write(*,*) 'hrch, 1:', iyytt 
...
!hyd     process values in table-type HYDR-INIT
      ...
      CALL RTABLE (iTBNO,iTBSB,iNVAL,UUNITS,
     M             RRVAL)
      vwater = RRVAL(1) * 43560.  ! ft3
!      vwater = VOL * 43560.   ! ft3
      VSOILW = ength * bedwidd * zdpth * porop 

          write(*,*) 'hrch, inside :', icont, vsoilw 
!!!!!!!!!!!!!!!!!!!!! pw end!
