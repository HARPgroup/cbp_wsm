...
C     convert units from 1/hr to 1/ivl
      ...
      UMAXHR = 0.00000002  ! NUPM4(4)*0.000001   
      TzDEN =  1.07   ! NUPM4(5)   
      DNT50 =  0.5  !  NUPM4(6)    
      UMAXT =  UMAXHR* DELT60  
C 
...
C
!   !!!!!!!!!!!!!     adding !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! pwo  cww = RVAL(1)    ! = NO3 ?? in water concentration,  mg/l 
...
C         consider denitrification processes, and compute associated fluxes
            if(NO3 .gt. 20.     ) NO3=20.      
            if(NO3 .lt. 0.00001) NO3=0.00001
          CALL DENIT
     I         (KNO320,TCDEN,TW,DOX,DENOXT,NO3, DDDNIT) 
C     M                 NO3,
C     O                ,DDNIT)   
C
          DENNO3= -DDDNIT  
!         DENNO3= 0.0      ! don't count 
C
        END IF
C
...
C
      SUBROUTINE   DENIT
     I          (KNO320,TCDEN,TW,DOX,DENOXT, NO3, DDNIT) 
C     M                    NO3,
C     O                   ,DDNIT )   
C
C     + + + PURPOSE + + +
...
C
            ...
            if(NO3 .gt. 20.) NO3 = 20.
            if(NO3 .lt. 0.00001) NO3=0.00001
            if(VOL .lt. 0.001) VOL = max(VOL,vwater * 0.001)
            volzx = vol * 1233.   ! m3 
        wtno3s = cno3s * vsoilw  
        cno3w = NO3            
        wtno3w = NO3 * volzx  ! VOL *         * 0.0283168 
        dvvt = dvvhr * DELT60           
        umaxt = umaxhr * DELT60
C       convert units from 1/hr to 1/ivl
        ddnit = 0.0
          dvvt  =  dvvt * TzDEN**(TW*20.)
          umaxt =  umaxt*(TzDEN**(TW-20.))  
         if(cno3w .gt. cno3s) then
          dw2s = dvvt * (cno3w-cno3s)/fctdv2  
          dw2s = dw2s*bedarea*porop      
         else               
          dw2s = 0.
         endif
          dw2s = min(wtno3w*0.9, dw2s)   
          wtno3w = wtno3w - dw2s  
          ddnit = umaxt* cno3s / (cno3s + dnt50)
          ddnit = ddnit * bedarea * porop * zdpth
          ddnit = min(wtno3s*0.9, ddnit)  
          wtno3s = wtno3s + dw2s - ddnit   
          cno3s  = wtno3s * 1000. / vsoilw  
          NO3 = wtno3w / volzx     !  VOL        
          ddnit = ddnit         * 2.2045  
               write(*,*)'denit, 1' 
