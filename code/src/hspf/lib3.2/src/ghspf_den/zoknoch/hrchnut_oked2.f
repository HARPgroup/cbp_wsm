...
C
! pw adding
!      write(* ,*) 'nut,add:','dvvhr,zdpth,cwwi, cssss,cssi, ength,'
!      write( *,*) 'nut,add:', dvvhr,zdpth,cwwi, cssss,cssi, ength
!        pause 
! pw added 
!     write(*,*) 'test 2:', nupm4(4),nupm4(5),nupm4(6) 
...
C     initial conditions - table-type nut-dinit
     ...
     M              RVAL)
       cwwi = RVAL(1)
       cno3w= RVAL(1)     ! pw 
       cssi = 0.5 * cwwi   ! tmp set 0.5, can be 0.2 
       cno3s = cssi
C
...
C     + + + LOCAL VARIABLES + + +
      INTEGER      J,K,FPT,I,N
      REAL         NH3VLT,BENTAM,BENPO4,DECNIT,DECPO4,
     $             DODEMD,TAMNIT,NO2NTC,NO3NIT,NO3DE,VOLSP,TOTN1,
     $             TOTN2,TOTP1,TOTP2,TOTPM1,TOTPM2,TOTPM3,TOTNM1,
     $             TOTNM2,TOTNM3,TWKELV,AVDEPM,DUMXXX,INNO3,INTAM,INPO4
...
C
C     + + + EXTERNALS + + +
      EXTERNAL     DAYVAL,ADVECT,ADVNUT,AMMION,BENTH
...
C       dry deposition
        IF (NUADFG(N) .LE. -1) THEN
          NUADDR(I)= SAREA*PAD(NUAFFP(I)+IVL1)
...
C       wet deposition
        IF (NUADFG(N+1) .LE. -1) THEN
          NUADWT(I)= PREC*SAREA*PAD(NUACFP(I)+IVL1)
...
!!!   write(*,*)'bdenit 0: ,umaxhr,zdpth,Tzden,dlt,volsp,vwater,no3,d50'
!!!   write(*,*)'bdenit0:',umaxhr,zdpth,Tzden,delt60,volsp,vwater,no3,
!!!  .      dnt50
! pw      CALL DENIT
!    I                (KNO320,TCDEN,TW,DOX,DENOXT,
!    M                 NO3,VOL, dddnit) 
!    o               vol, dddnit)
          call denit (tw, no3, volsp, NO3DE)   !  dddnit) 
!!!   write(*,*)'adenit 4: ,umaxhr,zdpth,Tzden,dlt,volsp,vwater,no3,d50'
!!!   write(*,*)'adenit4:',umaxhr,zdpth,Tzden,delt60,volsp,vwater,no3,
!!!  .      dnt50
!    O                 NO3DE)
C
          DENNO3= -NO3DE*VOLSP
...
C
C         In some segments in a large reduction scenario, this condition does
C           persist over time, so the tolerance here was increased two orders
C           of magnitude 2.8 mg is still a very small mass
C          Gary Shenk 4/2007
C
C         Still problems with forest scenario as of 3/2008
C           Changed again to 2.8 g - Gary Shenk
          IF ((ABS(ISNUT)) .GT. 1.0E+2 .OR.
     $        (ABS(RSNUTS)) .GT. 1.0E+2) THEN
...
!     SUBROUTINE   DENIT
!    I                   (KNO320,TCDEN,TW,DOX,DENOXT,
!    M                    NO3,voltt,ddnit)      ! adding volt, bedwidd
       subroutine denit (tw,no3, voltt,denno3) ! ddnit) 
!    0                 voltt, ddnit)
!    O                    DENNO3)
C
C     + + + PURPOSE + + +
...
C     + + + END SPECIFICATIONS + + +
C
!!!   write(*,*)'denit 1:umaxhr,zdpth,Tzden,delt,vwater,no3,d50,vt'
!!!   write(*,*)'denit1:',umaxhr,zdpth,Tzden,delt60,vwater,no3,dnt50
!!!  ,  , voltt
!!!   write(*,*)'denit1: ,dvvhr,ength,bedwidd,bedarea,cno3w,cno3s,vsw,d'
!!!   write(*,*)'denit1:',dvvhr,ength,bedwidd,bedarea,cno3w,cno3s, 
!!!  .     vsoilw ,delt60
!         goto 221
        wtno3s = cno3s * vsoilw  ! mg/l * m3 = g  ! useful if first time
        cno3w = NO3             ! water no3 conc   mg/l
        wtno3w = NO3 * VOLtt         * 0.0283168 !  g/m3 * m3  => g 
!       tNO3i = NO3
!     write(*,*)'dnrt1 ','dnitn,dnitv,umaxhr,dvvhr'
!     write(*,*)'dnrt1 ',dnitn,dnitv,umaxhr,dvvhr
      write(*,*)'dnrt1 ','dnitn,dnitv,umaxhr,dvvhr,denno3'
      write(*,*)'dnrt1 ',dnitn,dnitv,umaxhr,dvvhr,denno3
      write(*,*)'dnrt1 ','no3,cno3w,cno3s,dvvt,umaxt'
      write(*,*)'dnrt1 ',no3,cno3w,cno3s,dvvt,umaxt
      write(*,*)'dnrt1 ','wtno3w,wtno3s,vsoilw,voltt'
      write(*,*)'dnrt1 ',wtno3w,wtno3s,vsoilw,voltt
!up     wtno3s = cno3s * vsoilw  ! mg/l * m3 = g  ! useful if first time
!up     cno3w = NO3             ! water no3 conc   mg/l
!up     wtno3w = NO3 * VOLtt         * 0.0283168 !  g/m3 * m3  => g 
!       wtno3w = NO3 * VOL/delt60/60. *43560.* 0.0283168 !  g/m3 * m3  => g 
!       wtno3w = NO3 * (VOL/delt60)   *43560.* 0.0283168 !  g/m3 * m3  => g 
C                  internal-> ft3 ->  m3  
        dvvt = dvvhr  !* DELT60t            ! mg/m2/hr=ivl
        umaxt = umaxhr !* DELT60t 
        TCDEN = TZden
C       convert units from 1/hr to 1/ivl
!!!!!!!!!! pw added parameter
!!! note fctdv2 = 1/zdpth, 
...
!       ddnit = 0.0
          dvvtx  =                 dvvt *(TzDEN**(TW*20.))
          umaxtx =                 umaxt*(TzDEN**(TW-20.))  ! red fro segmen
!            goto 221
         if(cno3w .gt. cno3s) then
          dnitv = dvvtx * (cno3w-cno3s)/fctdv2  ! diffus rate:water 2 soil
          dnitv = 1000.*dnitv*bedarea*porop       ! * area * time = distance or mass into        ?????? be sure g !!!!! 
          dnitv  = min(wtno3w*0.99,dnitv) ! ????? -> g should be sure 
           chngconc = dnitv / (voltt * 0.0283168)  ! g / m3 , mg/l 
!          NO3 = NO3 - chngconc
         else            ! kg / time step ->      
          dnitv = 0.
          chngconc = 0.
...
!         wtno3w = wtno3w - dnitv
          dnitn = umaxtx* cno3s / (cno3s + dnt50)  !kg/m2/depth
          dnitn = 1000.*dnitn * bedarea * porop * zdpth !!!! should sure G????
          dnitn = min(wtno3s*0.99, dnitn) ! ???? ->g if from kg, uhr unit
          wtno3s = wtno3s + dnitv - dnitn   ! update g? 
!!        cno3s  = wtno3s * 1000. / (bedarea*porop*zdpth) ! update, g/m3 = mg/l
          cno3s  = wtno3s / vsoilw    ! update, g / m3 = mg/l
!         NO3 = wtno3w / (VOLtt     *0.0283168) !not times 43560: g / m3 -> mg/l )
!        if(NO3 .gt. tNO3i) then
!         chngconc = NO3 - tNO3i 
!         ddnit =  chngconc * voltt  !      kg -> pound  
!        else
!         ddnit = 0.
!        endif
        DENNO3= 0.0
        IF (NO3 .GT. 0.001 .and. chngconc.gt.0.) THEN
          DENNO3= chngconc   
          NO3   = NO3 - DENNO3
C
...
C           is not a negative number; set no3 to a value of .001 mg/l
          ...
          END IF
        END IF
      write(*,*)'dnrt2 ','dnitn,dnitv,umaxhr,dvvhr,denno3'
      write(*,*)'dnrt2 ',dnitn,dnitv,umaxhr,dvvhr,denno3
      write(*,*)'dnrt2 ','no3,cno3w,cno3s,dvvt,umaxt'
      write(*,*)'dnrt2 ',no3,cno3w,cno3s,dvvt,umaxt
      write(*,*)'dnrt2 ','wtno3w,wtno3s,vsoilw,voltt'
      write(*,*)'dnrt2 ',wtno3w,wtno3s,vsoilw,voltt
221      continue 
         goto 222
...
C
!     END IF
      write(*,*)'denit 2:umaxhr,zdpth,Tzden,    delt,vwater,no3,d50,vt'
      write(*,*)'denit2:',umaxhr,zdpth,Tzden,    delt60,vwater,no3,dnt50
     ,  , voltt
      write(*,*)'denit2: ,dvvhr,ength,bedwidd,bedarea,cno3w,cno3s,vsw,d'
      write(*,*)'denit2:',dvvhr,ength,bedwidd,bedarea,cno3w,cno3s, 
     .     vsoilw ,delt60
!        pause 
222   continue 
C
...
C
C     + + + LOCAL VARIABLES +
      REAL       NH3KL,NH3KG,DOKL,HCNH3,KRINV,KR,WINDSP,KNVOL
C
C     + + + EXTERNALS + + +
      EXTERNAL   HCINTP
C
...
C
C     + + + OUTPUT FORMATS + + +
 2000 FORMAT (/,1H ,'*** NUTRX ***')
 2010 FORMAT (/,1H ,'  TOTAL AMMONIA(TAM-N)')
 2020 FORMAT (/,1H ,'  STATE VARIABLES',28X,'TAM',7X,'NH4',7X,'NH3')
 2030 FORMAT (' ','    DISSOLVED CONCENTRATION',2X,'(MG/L)',5X,
     $        1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2040 FORMAT (/,1H ,'    CONCENTRATION OF NH4 ASSOCIATED',5X,
     $        'SUSP. SAND  SUSP. SILT  SUSP. CLAY')
 2050 FORMAT (' ','    WITH SEDIMENT',2X,'(MG/KG)',14X,1PE10.3,2X,
     $        1PE10.3,2X,1PE10.3)
 2060 FORMAT (/,1H ,'    DISSOLVED STORAGE',2X,'(',A3,')',12X,1PE10.3,
     $        2X,1PE10.3,2X,1PE10.3)
 2070 FORMAT (/,'     STORAGE ON SUSPENDED',18X,
     $        'ON SAND','     ON SILT     ON CLAY       TOTAL')
 2080 FORMAT (' ',6X,'SEDIMENT',2X,'(',A3,')',19X,3(1PE10.3,2X),1PE10.3)
 2090 FORMAT (/,1H ,'    TOTAL STORAGE',2X,'(',A3,')',16X,1PE10.3)
 2100 FORMAT (/,1H ,'  FLUXES (',A3,')')
 2110 FORMAT (' ','    INFLOW OF DISSOLVED TAM',13X,1PE10.3)
 2120 FORMAT (' ','    INFLOW OF DISSOLVED TAM',3X,'<---ATMOSPHERIC',
     #        ' DEPOSITION--->     OTHER')
 2130 FORMAT (' ',37X,'DRY       WET     TOTAL    INFLOW')
 2140 FORMAT (' ',31X,'NUADDR(2) NUADWT(2)   ATM DEP      ITAM')
 2150 FORMAT (' ',30X,5(1PE10.3))
 2160 FORMAT (/,1H ,'    INFLOW OF NH4 ON SEDIMENT',11X,'   ON SAND',
     $        '     ON SILT     ON CLAY')
 2170 FORMAT (' ',40X,1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2180 FORMAT (/,1H ,'    TOTAL INFLOW OF TAM',17X,1PE10.3)
 2190 FORMAT (/,1H ,'    ADSORPTION/DESORPTION OF NH4',8X,'SUSP. SAND  '
     $        ,'SUSP. SILT  SUSP. CLAY       TOTAL')
 2200 FORMAT (' ',6X,'(POSITIVE INDICATES ADSORPTION)',3X,1PE10.3,
     $        2X,1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2210 FORMAT (/,1H ,'    DEPOSITION/SCOUR OF NH4',13X,'   ON SAND     ',
     $        'ON SILT     ON CLAY       TOTAL')
 2220 FORMAT (' ',6X,'(POSITIVE INDICATES DEPOSITION)',3X,1PE10.3,
     $        2X,1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2230 FORMAT (/,1H ,41X,'  NITRIF-    VOLATIL-     BENTHAL',
     $        '   BOD DECAY  PHYTOPLANK   ZOOPLANK.  BENTH.ALG.')
 2240 FORMAT (' ','    OTHER GAINS/LOSSES',11X,'TOTAL     ICATION',
     $        '     IZATION     RELEASE            ',
     $        '      GROWTH       DEATH      GROWTH')
 2250 FORMAT (' ',6X,'(POS. INDICATES GAIN)',1X,8(1PE10.3,2X))
 2260 FORMAT (/,1H ,'    TOTAL OUTFLOW OF DISSOLVED TAM',6X,1PE10.3)
 2270 FORMAT (/,1H ,'    TOTAL OUTFLOW OF NH4 ON SEDIMENT',4X,
     $        '   ON SAND     ON SILT     ON CLAY       TOTAL')
 2280 FORMAT (' ',40X,1PE10.3,2X,1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2290 FORMAT (/,1H ,'    TOTAL OUTFLOW',23X,1PE10.3)
 2300 FORMAT (/,1H ,'    DISSOLVED TAM OUTFLOW FOR EACH EXIT')
 2310 FORMAT (' ',6X,'EXIT',I3,27X,1PE10.3)
 2320 FORMAT (/,1H ,'    NH4 OUTFLOW ON SEDIMENT FOR EACH EXIT',
     $        '  ON SAND     ON SILT     ON CLAY')
 2330 FORMAT (' ',6X,'EXIT',I3,27X,1PE10.3,2X,1PE10.3,2X,1PE10.3)
 2340 FORMAT ('     ',A3)
 2350 FORMAT (/,1H ,'  NITRITE(NO2-N)')
 2360 FORMAT (/,1H ,'  STATE VARIABLES')
 2370 FORMAT (' ','    CONCENTRATION  (MG/L)',15X,1PE10.3)
 2380 FORMAT (/,1H ,'    STORAGE (',A3,')',11X,1PE10.3)
 2390 FORMAT (' ','    INFLOW',18X,1PE10.3)
 2400 FORMAT (' ','    INFLOW OF NITRATE',9X,'<---ATMOSPHERIC',
     $        ' DEPOSITION--->     OTHER     TOTAL')
 2410 FORMAT (' ',31X,'NUADDR(1) NUADWT(1)   ATM DEP      INO3',
     $        '    INFLOW')
 2420 FORMAT (/,1H ,43X,'NITRIF-')
 2430 FORMAT (' ','    OTHER GAINS/LOSSES',11X,'TOTAL     ICATION')
 2440 FORMAT (/,1H ,'    OUTFLOWS',21X,'TOTAL',6X,
     $        'INDIVIDUAL GATE OUTFLOWS',/,32X,'OUTFLOW',2X,5(I10,2X))
 2450 FORMAT (' ',28X,6(1PE10.3,2X))
 2460 FORMAT (/,1H ,'    OUTFLOW',17X,1PE10.3)
 2470 FORMAT (/,1H ,'  NITRATE(NO3-N)')
 2480 FORMAT (/,1H ,43X,'NITRIF-   DENITRIF-   BOD DECAY',
     $        '  PHYTOPLANK   ZOOPLANK.  BENTH.ALG.')
 2490 FORMAT (' ','    OTHER GAINS/LOSSES',11X,'TOTAL     ICATION',
     $        '     ICATION                  GROWTH',
     $        '       DEATH      GROWTH')
 2500 FORMAT (/,1H ,'  ORTHO-PHOSPHATE(PO4-P)')
 2510 FORMAT (' ','    DISSOLVED CONCENTRATION',2X,'(MG/L)',5X,
     $        1PE10.3)
 2520 FORMAT (/,1H ,'    CONCENTRATION ASSOCIATED',12X,'SUSP. SAND  ',
     $        'SUSP. SILT  SUSP. CLAY')
 2530 FORMAT (/,1H ,'    DISSOLVED STORAGE',2X,'(',A3,')',12X,1PE10.3)
 2540 FORMAT (/,'     STORAGE ON SUSPENDED',18X,
     $        'ON SAND','     ON SILT     ON CLAY       TOTAL')
 2550 FORMAT (' ','    INFLOW OF DISSOLVED PO4',13X,1PE10.3)
 2560 FORMAT (' ','    INFLOW OF DISSOLVED PO4',3X,'<---ATMOSPHERIC',
     #        ' DEPOSITION--->     OTHER')
 2570 FORMAT (' ',31X,'NUADDR(3) NUADWT(3)   ATM DEP      IPO4')
 2580 FORMAT (/,1H ,'    INFLOW OF PO4 ON SEDIMENT',11X,'   ON SAND',
     $        '     ON SILT     ON CLAY')
 2590 FORMAT (/,1H ,'    TOTAL INFLOW',24X,1PE10.3)
 2600 FORMAT (/,1H ,'    ADSORPTION/DESORPTION',15X,'SUSP. SAND  ',
     $        'SUSP. SILT  SUSP. CLAY       TOTAL')
 2610 FORMAT (/,1H ,'    DEPOSITION/SCOUR',20X,'   ON SAND     ',
     $        'ON SILT     ON CLAY       TOTAL')
 2620 FORMAT (/,1H ,43X,'BENTHAL   BOD DECAY  PHYTOPLANK   ZOOPLANK.',
     $        '  BENTH.ALG.')
 2630 FORMAT (' ','    OTHER GAINS/LOSSES',11X,'TOTAL     RELEASE',
     $        '                  GROWTH       DEATH      GROWTH')
 2640 FORMAT (/,1H ,'    TOTAL OUTFLOW OF DISSOLVED PO4',6X,1PE10.3)
 2650 FORMAT (/,1H ,'    TOTAL OUTFLOW OF PO4 ON SEDIMENT',4X,
     $        '   ON SAND     ON SILT     ON CLAY       TOTAL')
 2660 FORMAT (/,1H ,'    DISSOLVED OUTFLOW FOR EACH EXIT')
 2670 FORMAT (/,1H ,'    OUTFLOW ON SEDIMENT FOR EACH EXIT',3X,
     $        '   ON SAND     ON SILT     ON CLAY')
C
C     + + + END SPECIFICATIONS + + +
C
...
C         inflow of nh4 on sediment
          CALL TRNVEC
     I                (I3,NUIF2(1,1,LEV),FACTA,FACTB,
     O                 PISFLX(1,1))
C
...
C         inflow of phosphate on sediment
          CALL TRNVEC
     I                (I3,NUIF2(1,2,LEV),FACTA,FACTB,
     O                 PISFLX(1,2))
C
...
C
        IF (ADNHFG .EQ. 1) THEN
          WRITE (PRINTU,2160)
          WRITE (PRINTU,2170) (PISFLX(J,1), J= 1,3)
        END IF
C
C       calculate and print total inflow of tam
C
        TOTIN= PADTOT(2)+ PIFLX(2)+ PISFLX(1,1)+ PISFLX(2,1)+
     #         PISFLX(3,1)
...
C
        IF (NEXITS .GT. 1) THEN
          WRITE (PRINTU,2300)
          DO 150 J= 1,NEXITS
            WRITE (PRINTU,2310) J,PCFLX9(J,2)
 150      CONTINUE
C
          IF (ADNHFG .EQ. 1) THEN
            WRITE (PRINTU,2320)
            DO 160 J= 1,NEXITS
              WRITE (PRINTU,2330) J,(PCFX10(J,K,1), K=1,3)
 160        CONTINUE
          END IF
        END IF
C
C       material balance
        TAMIN = PADTOT(2)+ PIFLX(2)+ PISFLX(1,1)+ PISFLX(2,1)+
     #          PISFLX(3,1)- PCFLX3(4,1)+ PCFLX5(3)+ PCFLX5(4)+
     #          PCFLX5(6)
...
C
        IF (NEXITS .GT. 1) THEN
          WRITE (PRINTU,2440)  (J,J=1,NEXITS)
          WRITE (PRINTU,2450)  PCFLX1(3), (PCFLX9(J,3),J=1,NEXITS)
        ELSE
...
C
      IF (NEXITS .GT. 1) THEN
        WRITE (PRINTU,2440)  (J,J=1,NEXITS)
        WRITE (PRINTU,2450)  PCFLX1(1), (PCFLX9(J,1),J=1,NEXITS)
      ELSE
...
C
        IF (ADPOFG .EQ. 1) THEN
          WRITE (PRINTU,2580)
          WRITE (PRINTU,2170) (PISFLX(J,2), J= 1,3)
        END IF
C
C       calculate and print total inflow of po4
C
        TOTIN= PADTOT(3)+ PIFLX(4)+ PISFLX(1,2)+ PISFLX(2,2)+
     #         PISFLX(3,2)
...
C
        IF (NEXITS .GT. 1) THEN
          WRITE (PRINTU,2660)
          DO 170 J= 1,NEXITS
            WRITE (PRINTU,2310) J,PCFLX9(J,4)
 170      CONTINUE
C
          IF (ADPOFG .EQ. 1) THEN
            WRITE (PRINTU,2670)
            DO 180 J= 1,NEXITS
              WRITE (PRINTU,2330) J,(PCFX10(J,K,2), K=1,3)
 180        CONTINUE
          END IF
        END IF
C
...
C       calculate quantity of po4 entering rchres
        PO4IN= PADTOT(3)+ PIFLX(4)+ PISFLX(1,2)+ PISFLX(2,2)+ 
     #         PISFLX(3,2)- PCFLX3(4,2)+ PCFLX7(1)+ PCFLX7(2)+
     #         PCFLX7(4)
C
...
C
      IF (NEXITS .GT. 1) THEN
        DO 70 J= 1, 4
          DO 60 I= 1, NEXITS
            IF (NUCF9X(I,J) .GE. 1) THEN
              PAD(NUCF9X(I,J) + IVL1)= NUCF9(I,J,1)
            END IF
 60       CONTINUE
 70     CONTINUE
C
        DO 100 K= 1, 2
          DO 90 J= 1, 3
            DO 80 I= 1, NEXITS
              IF (NUC10X(I,J,K) .GE. 1) THEN
                PAD(NUC10X(I,J,K) + IVL1)= NUCF10(I,J,K,1)
              END IF
 80         CONTINUE
 90       CONTINUE
 100    CONTINUE
