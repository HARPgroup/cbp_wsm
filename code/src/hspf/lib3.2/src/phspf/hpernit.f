...
C       read in soil data - table type soil-data
      END IF   ! pw moving up to here, so that it will always call SOLDAT 
        CALL SOLDAT (UUNITS,
     O               SOILM,SOILD)
c     END IF   ! pw moving up   ! getting soild(4) and pwsoild(4) 
C                                besides SOILD(4) and SOILM(5)????
...
c ! pw adding 
         do jw=1,4
          pwwiltpt(jw)= wiltpt(jw)/soild(jw) ! pwwiltpt from (in) back to in/in 
c           wiltpt(jw) = 0.1 * 0.3333 ! 5   ! hardwireding   (1n/1n) 
          if(pwwiltpt(jw) .gt. 0.05) then 
          fieldcp(jw) = pwwiltpt(jw)**0.6  !0.57   ! pw 8/25/06  (in/in) 
          else
          fieldcp(jw) = pwwiltpt(jw)**0.8   ! pw 8/25/06  (in/in) 
          endif 
c           pwwiltpt(jw) =   wiltpt(jw) * pwsoild(jw) !             as (in) 
          fieldcp(jw) = fieldcp(jw) * pwsoild(jw)   ! pw 8/25/06 , as (in)
c           wiltpt(jw) = pwwiltpt(jw) ! wiltpt(jw) * pwsoild(jw)   as (in) 
...
c                                    
c! pw: the next xx lines are added by pw! ??????? 
...
c          goto 551  ! pw 9/1/06
     ...
     M             WILTPT)
c      write(98,*) wiltpt
c        goto 551  ! no wiltpt for forest.  hardwire here to assign 
C     convert from in/in or in/cm to inches
         do jw=1,4
c        pwwiltpt = pwwiltpt * pwsoild ! wiltpt from in/in as (in) spec soil
           wiltpt(jw) = 0.1 * 0.15  ! 0.2 ! 0.3333 ! 5  ! hardwireding   (1n/1n) 
           if(wiltpt(jw) .le. 0.01) then 
           fieldcp(jw) = wiltpt(jw)**0.9  ! .65 0.57   ! pw 8/25/06  (in/in) 
           elseif(wiltpt(jw) .le. 0.05) then 
           fieldcp(jw) = wiltpt(jw)**0.85  ! .65 0.57   ! pw 8/25/06  (in/in) 
           else
           fieldcp(jw) = wiltpt(jw)**0.6  ! .65 0.57   ! pw 8/25/06  (in/in) 
           endif 
...
c pw for dynamic litter, not using!       include     'pwgligl.inc'
      ...
      common/pwdate/pwYR, pwMON, pwDAY, pwHR
      integer pwYR, pwMON, pwDAY,pwHR
C
C     + + + LOCAL VARIABLES + + +
...
C       perform reactions on nitrogen in the surface layer storage
     ...
     x             , pwsoild(1), fieldcp(1), uunits)    ! pw 
C
...
C       principal storage
     ...
     x             , pwsoild(2), fieldcp(2), uunits)    ! pw 
C
      ELSE
...
C     perform reactions on nitrogen in the lower layer storage
     ...
     x             , pwsoild(3), fieldcp(3), uunits)    ! pw 
C
...
C
     ...
     x             , pwsoild(4), fieldcp(4), uunits)    ! pw 
C
...
C     + + + LOCAL VARIABLES + + +
     ...
     .           , KSUNIKe, KSUAMKe, KSINIKe, KSIAMKe  
...
c          nmxrat = nmxrat*0.7   !  061203 
c         BRXNFG = 1   ! pw 
C
C     + + + END SPECIFICATIONS + + +
c  ! pw adding  
c !         goto 550          
c        fact = 1000./2.2046/102.75   ! pound/in-acre -> g/m3, mg/l
         if(uunits .eq. 1) then
            CFINMA = 2.264E05
         else
            CFINMA = 2.54E05
...
c no! er   pwwiltpt = wiltpt  * cfinma    ! for forest only
         pwmoistd= moistm  /  cfinma   ! correct, pound -> volume (inch)
cifout   pwmoistV=pwmoistd*1000.*1233./12.   ! water: in-arce -> volume L 
cifout   factmg=1000000./2.2046              ! pound to mg
cifout   fldcpV=fldcp*1000.*1233./12.   ! fldcp water: in-arce -> volume L 
cifout   fldcpW=fldcpV*2.2046           ! fldcp water weight: pound
         fldcpW=fldcp*cfinma     ! fldcp water weight: pound or kg  
c    then conc of pd NO3 to kg NO3 , / m3 ---> g/m3, mg/l 
c   !  should be = SMST?? soil moisture in inches (after mstley). 
c no     pwsoilv  = pwsoild * cfinma                     
c !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c         factk = pwsoildd*0.015 !.1,.2! for input M-M max to be pound /acre/in
          factk = pwsoildd*0.015 !.1,.2! for input M-M max to be pound /acre/in
c         factk = 1.0             !!! for input M-M max to be mg/l 
c  ! pw end added
...
c             KSUNIK= (KSUNI*0.1)*THPLN**DIF35   ! 061129 
              KSUNIK= (KSUNI    )*THPLN**DIF35   ! 061214 back 
...
c ! pw 8/9/06 adding, because input max is mass/area/day for per depth soil. 
              KSUNIK = KSUNIK * factk           ! otherwise, input *0.01  
              KSUAMK = KSUAMK * factk           ! otherwise, input *0.01  
c             KSINIK = KSINIK * factk * 100.    ! otherwise, input *0.01  
c             KSIAMK = KSIAMK * factk * 10.    ! otherwise, input *0.01  
              KSINIK = KSINIK * factk  ! 061214 back                      
              KSIAMK = KSIAMK * factk  ! 061214 back                     
c ! pw end added 
555            continue 
            ELSE
C             zero rates
...
C             first order plant uptake
            ...
            IF ( (NUPTFG .EQ. 2) .OR. (NUPTFG .EQ. -2) ) THEN
C             max rates for saturation kinetics
c             KSUNIK= KSUNI * 0.1   ! 061127 
              KSUNIK= KSUNI   ! 061214 back  
              KSUAMK= KSUAM       
              KSINIK= KSINI
              KSIAMK= KSIAM
cccc            goto 556  ! pw 
...
C             no nitrate present
            ...
            ELSE
c ! pw adding !!!!
chh          if(SMST .lt. wiltpt ) then   ! compare depth of water
cbb          if(pwmoistD .lt. wiltpt ) then   ! compare depth of water
chh            immni  = 0.0 
chh          else          ! greater than wilting point 
               if( SMST    .le. fldcp) then ! less than field capacity
c             goto 331 ! if m-m mg/l/day
c !!!    M-M pound/acre/in/day 
c                  NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
cifout     NICONC = factmg*( SMST /(fldcp))*NO3/pwmoistV ! MOISTM*1.0E6 
           NICONC = ( SMST /(fldcp))*NO3/MOISTM*1.0E6   ! cif 
                KSINIKe=KSINIK !*SMST   /(fldcp) !effective mass max immobil
c               KSINIKe=KSINIK  *SMST   /(fldcp) !effective mass max immobil
                immni = KSINIKe *NICONC/(NICONC+CSINI)
               else
cifout     NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
           NICONC =        NO3/MOISTM*1.0E6    ! cif 
                 immni= KSINIK*NICONC / (NICONC + CSINI)   ! pounds 
               endif
              goto 332
331           continue   
c !!!   M-M mg/l/day 
...
cifout     NICONC = factmg*(SMST  /(fldcp))*NO3/pwmoistV  ! /MOISTM*1.0E6 
           NICONC = (SMST  /(fldcp))*NO3/MOISTM*1.0E6   ! cif 
                immni= KSINIK*NICONC / (NICONC + CSINI)/factk  ! mg/l
cifout          IMMNI= IMMNI*pwmoistV / factmg  ! MOISTM/1.0E6 ! to pound
                IMMNI= IMMNI*MOISTM/1.0E6    ! cif 
               else
cifout        NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
              NICONC =        NO3/MOISTM*1.0E6  ! cif 
                 immni= KSINIK*NICONC / (NICONC + CSINI)/factk    ! mg/l
cifout        IMMNI= IMMNI*fldcpV  /factmg ! MOISTM/1.0E6, to pound     
c             IMMNI= IMMNI*fldcpW/1.0E6    ! cif      
              IMMNI= IMMNI*fldcp /1.0E6    ! cif      
               endif
332           continue  
chh          endif
c  ! pw end added   
...
chh          else          ! greater than wilting point 
               if( SMST    .le. fldcp) then ! less than field capacity
c             goto 333 ! if m-m mg/l/day
c !!!    M-M pound/acre/in/day 
c                  AMCONC = factmg*AMSU/pwmoistV ! MOISTM*1.0E6
cifout     AMCONC = factmg*( SMST /(fldcp))*AMSU/pwmoistV ! MOISTM*1.0E6 
           AMCONC = ( SMST /(fldcp))*AMSU/MOISTM*1.0E6   ! cif 
                KSIAMKe=KSIAMK !*SMST   /(fldcp) !effective mass max immobil
c               KSIAMKe=KSIAMK  *SMST   /(fldcp) !effective mass max immobil
                immam = KSIAMKe *AMCONC/(AMCONC+CSIAM)
               else
cifout     AMCONC = factmg*AMSU/pwmoistV ! MOISTM*1.0E6
           AMCONC =        AMSU/MOISTM*1.0E6    ! cif 
                 immam= KSIAMK*AMCONC / (AMCONC + CSIAM)
               endif
              goto 334
333           continue   
c !!!   M-M mg/l/day 
...
cifout     AMCONC = factmg*(SMST  /(fldcp))*AMSU/pwmoistV  ! /MOISTM*1.0E6 
           AMCONC = (SMST  /(fldcp))*AMSU/MOISTM*1.0E6   ! cif 
                immam= KSIAMK*AMCONC / (AMCONC + CSIAM)/factk  ! mg/l
cifout          IMMAM= IMMAM*pwmoistV / factmg  ! MOISTM/1.0E6 ! to pound
                IMMAM= IMMAM*MOISTM/1.0E6 ! cif          
               else
cifout     AMCONC = factmg*AMSU/pwmoistV ! MOISTM*1.0E6
           AMCONC =        AMSU/MOISTM*1.0E6   ! cif 
                 immam= KSIAMK*AMCONC / (AMCONC + CSIAM)/factk    ! mg/l
c                IMMAM= IMMAM*pwmoistV/factmg ! MOISTM/1.0E6, to pound    
cifout           IMMAM= IMMAM*fldcpV  /factmg ! MOISTM/1.0E6, to pound     
                 IMMAM= IMMAM*fldcpW/1.0E6    ! cif      
               endif
334           continue  
chh          endif
c  ! pw end added   
...
c            unptgg = min(nuptg+ndefc, nuptg*nmxrat)
             unptgg = min(nuptg+ndefc, nuptg*nmxrat*0.7)  ! pw 
c           goto 311     ! goto as old code
...
cpw     .         (        ((smst-wiltpt)/(fldcp - wiltpt) )**0.25)
                 utntot = unptgg          * 
     .         (        ((smst-wiltpt)/(fldcp - wiltpt) )**0.25)
c the next is for linear 
!! using next  utntot = (nuptg + ndefc)*(smst-wiltpt)/(fldcp-wiltpt)
c061201 out      utntot = unptgg*(smst-wiltpt)/(fldcp-wiltpt)  !!!!!!
...
c !usingNext  utntot = (nuptg + ndefc) *
c    .     ( - 0.1*(smst-fldcp)*(smst-fldcp) -0.05*(smst-fldcp) +1.)
              utntot =  unptgg         *       
     .     ( - 0.1*(smst-fldcp)*(smst-fldcp) -0.05*(smst-fldcp) +1.) !keep
c    .   (-0.001  *(smst-fldcp)*(smst-fldcp)-0.025*(smst-fldcp) +1.)
               endif
             if(utntot .lt. 0.00000001) utntot = 0. 
c            endif
c ! pw end added   
C
...
c!pw out!!    UTNTOT= NUPTG+ NDEFC
              goto 312
311            continue   ! old cade
C             try to take up optimum target plus seasonal deficit
c ! pw out    UTNTOT= NUPTG+ NDEFC   ! pw taking out
c             utntot= min(nuptg+ndefc,nuptg*nmxrat)       ! pw changing
              utntot = unptgg
312            continue
C
C             make sure maximum rate is not exceeded
             goto 313    ! comment out! ! pw
c             MAXUPT= NUPTG*NMXRAT       
              MAXUPT= NUPTG*NMXRAT * 0.7   ! pw 061203
...
c               goto 441   ! if M-M using mg/l/day 
c       for M-M pound/acre/in/day 
               if(smst .le. fldcp) then ! less than field capacity
c           NICONC =  factmg*             NO3/pwmoistV ! MOISTM*1.0E6 ! tests 
cifout      NICONC = factmg*(  SMST  /(fldcp))*NO3/pwmoistV ! MOISTM*1.0E6 
            NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6  ! cif 
...
cifout      NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
            NICONC =                    NO3/MOISTM*1.0E6  ! cif
                 utni = KSUNIK*NICONC / (NICONC + CSUNI)  ! pound M-M 
               endif 
               goto 442   ! if M-M using pound/acre/in/day
441            continue 
c       for M-M mg/l uptake 
               if(smst .le. fldcp) then ! less than field capacity
cifout      NICONC = factmg*(  SMST  /(fldcp))*NO3/pwmoistV ! MOISTM*1.0E6 
            NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6  ! cif 
                utni = KSUNIK *NICONC/(NICONC+CSUNI)/factk  ! mg /l
cifout          utni = utni * pwmoistV / factmg   ! to pound 
                utni = utni * MOISTM/1.0E6        ! cif
cff           UTNI= UTNI*MOISTM/1.0E6      ! out this getting more uptake 
               else
cifout        NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
              NICONC =                    NO3/MOISTM*1.0E6  ! cif
                 utni = KSUNIK*NICONC/(NICONC + CSUNI)/factk  ! mg/l 
cifout        utni = utni * fldcpV / factmg  ! to pound if M-M is mg/l
              UTNI = UTNI*fldcpW/1.0E6       ! cif 
               endif 
442            continue 
             endif
c  ! pw end added 
...
cifout      AMCONC =factmg*AMSU/pwmoistV ! MOISTM*1.0E6
            AMCONC =       AMSU/MOISTM*1.0E6    ! cif
                ...
                goto 444
443            continue  ! if M-M is mg/l/day 
c       m-m mg/l/day 
               if(smst .le. fldcp) then ! less than field capacity
cifout      AMCONC = factmg*(  SMST  /(fldcp))*AMSU/pwmoistV ! MOISTM*1.0E6 
            AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6   ! cif
                utam = KSUAMK *AMCONC/(AMCONC+CSUAM)/factk  ! mg /l
cifout          utam = utam * pwmoistV / factmg   ! to pound 
                utam = utam * MOISTM/1.0E6        ! cif      
cff           UTAM= UTAM*MOISTM/1.0E6      ! out this getting more uptake 
               else
cifout           AMCONC =factmg*AMSU/pwmoistV ! MOISTM*1.0E6
                 AMCONC =       AMSU/MOISTM*1.0E6    ! cif 
c06/12/13!!      utam = KSUAMK*AMCONC/(AMCONC + CSUNI)/factk  ! mg/l 
                 utam = KSUAMK*AMCONC/(AMCONC + CSUAM)/factk  ! mg/l 06/12/13
cifout        utam = utam * fldcpV / factmg  ! to pound if M-M is mg/l
              utam = utam * fldcpW/1.0E6     ! cif                
               endif 
444           continue 
             endif
c  ! pw end added   
...
C
          END IF
c     write(98,431)
c    .  YR,MON,DAY, pwsoildd,wiltpt,fldcp,utni,utam,SMST ! niconc,amconc,
c    .     SMST,pwsoildd,wiltpt,fldcp,utni,utam,niconc,amconc,
c    .     MOISTM, TMP, pwmoistV, ksunik, NO3, AMSU
...
C
        END IF
c          goto 445   ! no print for Rob  
c     write(98,431)SMST,pwsoildd,wiltpt,fldcp,utni,utam,niconc,amconc
        iF ( (NUPTFG .EQ. 2) .OR. (NUPTFG .EQ. -2) ) THEN ! adding ping
      write(98,431)
     .  pwYR,pwMON,pwDAY,pwHR,utni,utam,pwsoildd,wiltpt,fldcp,utni,utam
c    .  YR,MON,DAY, pwsoildd,wiltpt,fldcp,utni,utam,SMST ! niconc,amconc,
        elseif (NUPTFG .EQ. 1) THEN
      write(98,431)
     . pwYR,pwMON,pwDAY,pwHR,utntot,pwsoildd,wiltpt,fldcp,SMST,
     .   ndefc, nuptg
        endif
445     continue 
      END IF
...
C
      RETURN
c 431    format(4(e11.4,1x),4(e11.4,1x),e8.2,1x,f4.1,4(1x,e8.2))
431    format(i4,1x,i2,1x,i2,1x,i2,1x,8(e11.4,1x))
      END
