...
c           wiltpt(jw) = 0.1 * 0.3333 ! 5   ! hardwireding   (1n/1n) 
          fieldcp(jw) = pwwiltpt(jw)**0.57   ! pw 8/25/06  (in/in) 
...
c        pwwiltpt = pwwiltpt * pwsoild ! wiltpt from in/in as (in) spec soil
           wiltpt(jw) = 0.1 * 0.3333 ! 5   ! hardwireding   (1n/1n) 
...
c !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c         factk = pwsoildd * 0.2  !!! for input M-M max to be pound /acre/in
...
cccccc     NICONC = ( SMST /(fldcp))*NO3/MOISTM*1.0E6   ! cif 
...
c ! pw adding !!!!
             if(smst .lt. wiltpt ) then   ! compare depth of water
                ...
                goto 441   ! if M-M using mg/l/day 
...
C
      write(98,431)
     .     SMST,pwsoildd,wiltpt,fldcp,utni,utam,niconc,amconc,
     .     MOISTM, TMP, pwmoistV, ksunik, NO3, AMSU
...
C
      RETURN
431    format(4(e8.2,1x),4(e11.4,1x),e8.2,1x,f4.1,4(1x,e8.2))
      END
