...
c ! pw adding 
     ...
     M             WILTPT)
        do jw=1,4
           wiltpt(jw) = 0.1 *0.1   ! 5   ! hardwireding   (1n/1n)
...
C       perform reactions on nitrogen in the surface layer storage
     ...
     I              pwWILTPT(1),SORNPM,SKVOL,THVOL,TRFVOL,SKSATN,SCSATN,
...
C       principal storage
     ...
     I              pwWILTPT(2),UORNPM,UKVOL,THVOL,TRFVOL,UKSATN,UCSATN,
...
C         perform reactions on nitrogen in the surface layer storage
     ...
     I                 NUPTFG,FIXNFG,ALPNFG,AMVOFG,SNUPTG,NMXRAT,
     I                 pwWILTPT(1),SORNPM,SKVOL,THVOL,TRFVOL,SKSATN,
...
C         principal storage
     ...
     I                 NUPTFG,FIXNFG,ALPNFG,AMVOFG,UNUPTG,NMXRAT,
     I                 pwWILTPT(2),UORNPM,UKVOL,THVOL,TRFVOL,UKSATN,
...
C     perform reactions on nitrogen in the lower layer storage
     ...
     I            NUPTFG,FIXNFG,ALPNFG,AMVOFG,LNUPTG,NMXRAT,pwWILTPT(3),
...
C
     ...
     I            NUPTFG,FIXNFG,ALPNFG,AMVOFG,ANUPTG,NMXRAT,pwWILTPT(4),
...
c         ! pw addding: wiltpt(in) -> in/in, then fldcp(in/in)
c       fldcp = (WILTPT/pwsoildd)**0.57   ! pw add
c       fldcp = fldcp * pwsoildd   ! in/in -> in for fldcp 
C
C     + + + END SPECIFICATIONS + + +
C
      I4    = 4
...
C             max rates for saturation kinetics
              ...
              KSUNIK= KSUNIK*pwsoildd    !  pw adding 4 
              KSUAMK= KSUAMK*pwsoildd    
              KSINIK= KSINIK*pwsoildd     
              KSIAMK= KSIAMK*pwsoildd    
            ELSE
C             zero rates
              KSUNIK= 0.0
              KSUAMK= 0.0
...
C             no nitrate present
            ...
            ELSE
C             nitrate present
c             UTNI= KSUNIK*NICONC/(CSUNI+ NICONC)
c             UTNI= UTNI*MOISTM/1.0E6
...
C             no solution ammonia present
            ...
            ELSE
C             solution ammonia present
c             UTAM= KSUAMK*AMCONC/(CSUAM+ AMCONC)
c             UTAM= UTAM*MOISTM/1.0E6
            if(smst .lt.   wiltpt ) then   ! compare depth of water
                 utam = 0.0
             else          ! greater than wilting point
               if(smst .le. fldcp) then ! less than field capacity
