...
!      if (nB.lt.2) then
      if (nB.lt.0) then  ! try to go for defaults
...
************************************************************************
     ...
     .                   modules,nmod,species,nspecies)
      ...
      double precision monapp(year1:year2,12,nspecies)
      character*6 apptype
      data apptype /'legume'/
...
!      if (nB.lt.2) then
      if (nB.lt.0) then  ! try for default
        keyword = 'DEFAULT CROPDATA'
        call readcontrol1key(lscen,lenlscen,keyword,
     O                       nB,Jday,Byear,Bmonth,Bday,Lfile)
     ...
     I                year1,year2,apptype,annapp,monapp)
        return
      end if
...
***** WRITE SPECIAL ACTION for varied land cover IN UCI FILE
     ...
     .               real(year-Byear(indx1))
            ...
            monapp(year,month,sp) = monapp(year,month,sp) + fix
            write(line(26:27),'(i2)') month
...
********* loop over species, create temporary monthly variable
     ...
     .                         real(year-Byear(indx1))
                  ...
                  monapp(year,month,sp) = monapp(year,month,sp) + man
...
********* loop over species, create temporary monthly variable
     ...
     .                   real(year-Byear(indx1))
                  ...
                  monapp(year,month,sp) = monapp(year,month,sp) + fer
...
C {
     ...
     .                real(Byear(indx2)-Byear(indx1)) *
     .                   real(year-Byear(indx1))
c                  print*,pctnup(indx1,nt,month),',',
c     .                   pctnup(indx2,nt,month),',',
c     .                   Byear(indx2),',',Byear(indx1),',',year,',',pct
                  monapp(year,month,sp) = monapp(year,month,sp) +nup*pct
...
************************************************************************
** variable biosolids applications                                    **
**  this subroutine writes continual monthly applications             **
**    that are distributed over the month                             **
************************************************************************
      subroutine Baction(lscen,lenlscen,lseg,lenlseg,clu,year1,year2,
     .                   modules,nmod,species,nspecies)
      implicit none
      include 'lug.inc'
      include 'acts.inc'
      ...
      real biosolids(maxTimeBreaks,nspecies,12),bios   ! biosolid
      double precision annapp(year1:year2,nspecies) !annual application
      double precision monapp(year1:year2,12,nspecies)

      character*9 apptype
      data apptype /'biosolids'/

      character*30 Bfile(maxTimeBreaks)        ! biosolids file name
      integer nB, n, nm
      ...
      character*4 uvname(maxspecies)
      data uvname /'BNO3','BNH3','BORN','BPO4','BORP'/
...
***** READ CONTROL FILE AND DATA
     ...
     O                       nB,Jday,Byear,Bmonth,Bday,Bfile)
      ...
      call readbiosolids(lseg,lenlseg,clu,nB,Bfile,biosolids,
     I                species,nspecies)
       
      call rmMissingMonthlySpecies(maxTimeBreaks,lseg,clu,year1,year2,
     I                        nspecies,
     M                        nB,Jday,Byear,Bmonth,Bday,biosolids)
...
***** WRITE SPECIAL ACTION for varied land cover IN UCI FILE
      line = '***Action Lines for biosolids'
      if (writeact) call ryt(line,uci)
      line  = 'IF (prec < 0.1) THEN'
      if (writeact) call ryt(line,uci)
...
********* loop over species, create temporary monthly variable
     ...
     .                      .and.year.le.year2)
              ...
              do month = 1,12
                write(line(26:27),'(i2)') month
                if (biosolids(indx1,sp,month).gt.-1) then
                  bios = biosolids(indx1,sp,month) + 
     .            (biosolids(indx2,sp,month)-biosolids(indx1,sp,month))/
     .                    real(Byear(indx2)-Byear(indx1)) * 
     .                         real(year-Byear(indx1))
                  bios = max(bios,0.0)
                  annapp(year,sp) = annapp(year,sp) + bios
                  monapp(year,month,sp) = monapp(year,month,sp) + bios
                  if (writeact) call ritef10(line(61:70),bios)
                  if (writeact) call ryt(line,uci)
                end if
              end do
              year = year + 1
...
************************************************************************
** variable wep applications                                          **
**  this subroutine writes continual monthly applications             **
**    that are distributed over the month                             **
************************************************************************
      subroutine Waction(lscen,lenlscen,lseg,lenlseg,clu,year1,year2,
     .                   modules,nmod,species,nspecies)
      implicit none
      include 'lug.inc'
      include 'acts.inc'
      ...
      real wep(maxTimeBreaks,nspecies,12),twep   ! wep
      double precision annapp(year1:year2,nspecies) !annual application
      double precision monapp(year1:year2,12,nspecies)
      ...
      character*30 Bfile(maxTimeBreaks)        ! wep file name
      integer nB, n, nm
      ...
      character*4 uvname(maxspecies)
      data uvname /'WNO3','WNH3','WORN','WPO4','WORP'/
...
***** READ CONTROL FILE AND DATA
     ...
     O                       nB,Jday,Byear,Bmonth,Bday,Bfile)
      ...
      call readwep(lseg,lenlseg,clu,nB,Bfile,wep,
     I                species,nspecies)

      call rmMissingMonthlySpecies(maxTimeBreaks,lseg,clu,year1,year2,
     I                        nspecies,
     M                        nB,Jday,Byear,Bmonth,Bday,wep)
...
********* loop over species, create temporary monthly variable
     ...
     .                      .and.year.le.year2)
              ...
              do month = 1,12
                write(line(26:27),'(i2)') month
                if (wep(indx1,sp,month).gt.-1) then
                  twep = wep(indx1,sp,month) +
     .            (wep(indx2,sp,month)-wep(indx1,sp,month))/
     .                    real(Byear(indx2)-Byear(indx1)) *
     .                         real(year-Byear(indx1))
                  twep = max(twep,0.0)
                  annapp(year,sp) = annapp(year,sp) + twep
                  monapp(year,month,sp) = monapp(year,month,sp) + twep
                  if (writeact) call ritef10(line(61:70),twep)
                  if (writeact) call ryt(line,uci)
                end if
              end do
              year = year + 1
