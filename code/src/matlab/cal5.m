...
% Last Modified by GUIDE v2.5 22-Aug-2008 09:35:00
...
handles.default_mindate = '1/1/1985';
handles.default_maxdate = '12/31/2005';
...
try
    ...
    handles.win_path = prefs.win_path; % search path for windowed data.
    ...
    handles.DriveLetter = prefs.DriveLetter;
    handles.BaseDir = prefs.BaseDir;
    handles.ObScen = prefs.ObScen;
    clear prefs;
...
%        [handles.river_sim_path,handles.land_sim_path,handles.obs_path, ...
%         handles.connect_path,handles.batch_path,handles.win_path,err] = get_paths;
...
%    end
    ...
    handles.DriveLetter = 'Q';
    handles.BaseDir = 'p5';
    handles.ObScen = 'calib';

end
set(findobj('Tag','catcode_edit'), 'String', handles.catcode);
set(findobj('Tag','scenario_edit'), 'String', handles.scenario);
set(findobj('Tag','DriveLetter_edit'), 'String', handles.DriveLetter);
set(findobj('Tag','BaseDirectory_edit'), 'String', handles.BaseDir);
set(findobj('Tag','ObservedDataSet_edit'), 'String', handles.ObScen);
...
% Build Paths
comp_type = computer;
if ~strcmpi(comp_type(1:3),'PCW'), % not a PC Windows machine - use MAC path style.
    handles.river_sim_path = ['/bluefish/archive/',handles.BaseDir,'/output/river/daily/']; % search path for river simulated data.
    handles.land_sim_path = ['/bluefish/archive/',handles.BaseDir,'/output/pltgen/land/']; % search path for land simulated data.
    % handles.obs_path = ['/bluefish/archive/p5_observed/']; % search path for observed data.
    % handles.obs_path = ['/bluefish/archive/',folder,'/pp/observed/'] % search path for observed data.
    handles.obs_path = ['/bluefish/archive/',handles.BaseDir,'/input/calib/observed/',handles.ObScen,'/']; % search path for observed data.
    handles.connect_path = ['/bluefish/archive/',handles.BaseDir,'/config/catalog/geo/p52/']; % search path for connect files.
    handles.batch_path = ['/bluefish/archive/',handles.BaseDir,'/config/seglists/']; % search path for ".calib" file for batch processing.
    handles.win_path = ['/bluefish/archive/',handles.BaseDir,'/output/river/window/']; % search path for windowed data file.
else % assume PC Windows machine - use ordinary path style.
    handles.river_sim_path = [handles.DriveLetter,':\',handles.BaseDir,'\output\river\daily\']; % search path for river simulated data.
    handles.land_sim_path = [handles.DriveLetter,':\',handles.BaseDir,'\output\pltgen\land\']; % search path for land simulated data.
    % handles.obs_path = [drive,':\p5_observed\']; % search path for observed data.
    % handles.obs_path = [drive,':\',folder,'\pp\observed\']; % search path for observed data.
    handles.obs_path = [handles.DriveLetter,':\',handles.BaseDir,'\input\calib\observed\',handles.ObScen,'\']; % search path for observed data.
    handles.connect_path = [handles.DriveLetter,':\',handles.BaseDir,'\config\catalog\geo\p52\']; % search path for connect files.
    handles.batch_path = [handles.DriveLetter,':\',handles.BaseDir,'\config\seglists\']; % search path for ".calib" file for batch processing.
    handles.win_path = [handles.DriveLetter,':\',handles.BaseDir,'\output\river\window\']; % search path for windowed data file.
end
...
try
    [handles.river_names(:,1),handles.river_names(:,2)] = textread([handles.connect_path,'cal5_river_names.csv'],'%s %q','delimiter',',');
catch
...
function scenario_edit_CreateFcn(hObject, eventdata, handles) %#ok<DEFNU>
...
function scenario_edit_Callback(hObject, eventdata, handles) %#ok<DEFNU>
% hObject    handle to scenario_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if strcmp(handles.data_type,'SNOW') % land segment.
...
if  size(slashes,2) ~= 2 % doesn't have 2 "/"s.
    ...
    mo = str2double(date_str(1:slashes(1)-1));
    dy = str2double(date_str(slashes(1)+1:slashes(2)-1));
    yr = str2double(date_str(slashes(2)+1:length(date_str)));    
...
%================ DRIVE LETTER SPECIFICATION BOX ===============
% --- Executes during object creation, after setting all properties.
function DriveLetter_edit_Callback(hObject, eventdata, handles)
% hObject    handle to DriveLetter_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of DriveLetter_edit as text
%        str2double(get(hObject,'String')) returns contents of DriveLetter_edit as a double
handles.DriveLetter = get(hObject,'String'); % get and store Drive Letter (letter of Drive in Windows).
guidata(hObject,handles);

% --- Executes during object creation, after setting all properties.
function DriveLetter_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to DriveLetter_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%================ BASE DIRECTORY SPECIFICATION BOX ===============
% --- Executes during object creation, after setting all properties.
function BaseDirectory_edit_Callback(hObject, eventdata, handles)
% hObject    handle to BaseDirectory_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of BaseDirectory_edit as text
%        str2double(get(hObject,'String')) returns contents of BaseDirectory_edit as a double
handles.BaseDir = get(hObject,'String'); % get and store Base Directory (version diretory)
guidata(hObject,handles);

% --- Executes during object creation, after setting all properties.
function BaseDirectory_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to BaseDirectory_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%================ OBSERVED DATA SCENARIO SPECIFICATION BOX ===============
% --- Executes during object creation, after setting all properties.
function ObservedDataSet_edit_Callback(hObject, eventdata, handles)
% hObject    handle to ObservedDataSet_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of ObservedDataSet_edit as text
%        str2double(get(hObject,'String')) returns contents of ObservedDataSet_edit as a double
handles.ObScen = get(hObject,'String'); % get and store observed data scenario (directory name of scenario to process).
guidata(hObject,handles);

% --- Executes during object creation, after setting all properties.
function ObservedDataSet_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to ObservedDataSet_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% the following is probably unnecessary and klugy
handles.scenario              = get(findobj('Tag','scenario_edit'             ), 'String');
handles.DriveLetter           = get(findobj('Tag','DriveLetter_edit'          ), 'String');
handles.BaseDir               = get(findobj('Tag','BaseDirectory_edit'        ), 'String');
handles.ObScen                = get(findobj('Tag','ObservedDataSet_edit'      ), 'String');
handles.data_type_menu_select = get(findobj('Tag','data_type_popupmenu'       ), 'Value' );
handles.mindate               = get(findobj('Tag','mindate_edit'              ), 'String');
handles.maxdate               = get(findobj('Tag','maxdate_edit'              ), 'String');
% handles.load_precip           = get(findobj('Tag','load_precip_checkbox'      ), 'Value' );
% handles.hide_precip           = get(findobj('Tag','timeseries_precip_checkbox'), 'Value' );
% handles.hide_obs_ts           = get(findobj('Tag','timeseries_obs_checkbox'   ), 'Value' );
% handles.logscale_ts           = get(findobj('Tag','log_scale_checkbox'        ), 'Value' );
% handles.log_simobs            = get(findobj('Tag','log_simobs_checkbox'       ), 'Value' );
% handles.log_cum               = get(findobj('Tag','plot_log_data_checkbox'    ), 'Value' );
% handles.hide_obs_cum          = get(findobj('Tag','cumprob_obs_checkbox'      ), 'Value' );
...
for i = 1:n_run,
    ...
    if (str2double(handles.mindate(min_slashes(end)+1:end)) == 0)&&(str2double(handles.maxdate(max_slashes(end)+1:end)) == 0), % seasonal filter specified.
...
%         waitbar(i/n_run,h_wait,[num2str(i),' of ',num2str(n_run),' segments processed...']);
        window_save_PDF_Callback(hObject, eventdata, handles);
    end
end
...
prefs.win_path = handles.win_path; % search path windowed data file.
...
prefs.DriveLetter = handles.DriveLetter;
prefs.BaseDir     = handles.BaseDir    ;
prefs.ObScen      = handles.ObScen     ;
...
% the following section is commmented out.  It is not necessary to check a
% separate file to see if it is a calibration site.  If the data exists,
% then it is a calibration site
% check if specified catcode is calibration site (for FLOW only)...
% if not(handles.batch), % don't give messages when batching.
%     if strcmp(upper(handles.data_type),'FLOW'),
...
comp_type = computer;
if ~strcmp(comp_type(1:3),'PCW'), % not a PC Windows machine - use MAC path style.
...
if exist(simfile,'file') == 2 % sim data file exists.
    have_sim = 1;
...
if exist(obsfile,'file') == 2 % obs data file exists.
    have_obs = 1;
    %??? OLD OBS FILE FORMATS
    try % observed flow files do not have 2 extra text columns.
        temp = csvread(obsfile);
        % all data values are applied to mid-day of given date (noon).
        obs_data = [datenum(temp(:,1),temp(:,2),temp(:,3), ...
                    12*ones(length(temp(:,1)),1),zeros(length(temp(:,1)),1),zeros(length(temp(:,1)),1)), ...
                    temp(:,6)];
    %??? NEW OBS FILE FORMATS
    catch % observed QW files have 2 extra text columns (flag and station name).
		[c1,c2,c3,c4,c5,c6,c7,c8] = textread(obsfile,'%f %f %f %f %f %f %s %s','delimiter',','); %#ok<NASGU>
        obs_data = [datenum(c1,c2,c3,12*ones(length(c1),1),zeros(length(c1),1),zeros(length(c1),1)),c6];
    end
end

if handles.load_precip % load precip data only if user wants it.
    waitbar(0.6,h_wait,'reading precipitation data set...');
    if exist(precfile,'file') == 2 % precip data file exists.
        have_prec = 1;
...
% [serial_time, sim, log_sim, obs, log_obs, prec]
...
if not(handles.have_sim & handles.have_obs)
    ...
    xlabel(['OBSERVED ', get_label(handles)]);
    ylabel(['SIMULATED ',get_label(handles)]);
...
%%% % transfer to temp data set
%%% temp_plot_data = [handles.plot_data(:,3),handles.plot_data(:,5)];
%%% % remove any row with NaN...
%%% temp_plot_data(any(isnan(temp_plot_data)'),:) = [];
%%% if size(temp_plot_data,1) ~= 0,
  % add least-square fit line with stats...
...
% only use paired data for plots and stats.
cdf_data = handles.plot_data(:,[2,4]);
cdf_data(any(isnan(cdf_data)'),:) = []; % remove any row with NaN.
if handles.have_sim
    sim_h = cdfplot(cdf_data(:,1));
    set(sim_h,'Color','r');
end
if handles.have_obs
    obs_h = cdfplot(cdf_data(:,2));
...
% add kstest2 stats...
if handles.have_sim && handles.have_obs,
	[H,P,KSSTAT] = kstest2(cdf_data(:,1),cdf_data(:,2),0.05,0);
    xmin = get(gca,'XLim'); xmin = xmin(1);
    xmax = get(gca,'XLim'); xmax = xmax(2);
    ymin = get(gca,'YLim'); ymin = ymin(1);
    ymax = get(gca,'YLim'); ymax = ymax(2);
	text(xmin+0.1*(xmax-xmin), ymin+0.45*(ymax-ymin),['h = ',num2str(H)],'Color',[1,0,0]);
	text(xmin+0.1*(xmax-xmin), ymin+0.50*(ymax-ymin),['p = ',num2str(P)],'Color',[1,0,0]);
	text(xmin+0.1*(xmax-xmin), ymin+0.55*(ymax-ymin),['k = ',num2str(KSSTAT)],'Color',[1,0,0]);
end
grid off;
...
if exist(obsfile,'file') == 2, % obs data file exists.
    ...
    %??? NEW OBS FILE FORMATS
		...
		[c1,c2,c3,c4,c5,c6,c7,c8] = textread(obsfile,'%f %f %f %f %f %f %s %s','delimiter',','); %#ok<NASGU>
        obs_flow = [datenum(c1,c2,c3,12*ones(length(c1),1),zeros(length(c1),1),zeros(length(c1),1)),c6];
    end
else % abort.
...
plot([xmin,xmax],[b(2)*xmin+b(1),b(2)*xmax+b(1)],'Color',[0 0.5 0]);
% lsline; % this gives same line as above command - use to check above command.
...
plot([xmin,xmax],[b(2)*xmin+b(1),b(2)*xmax+b(1)],'Color',[1 0 0]);
% lsline; % this gives same line as above command - use to check above command.
...
function window_plot_pushbutton_Callback(hObject, eventdata, handles) %#ok<DEFNU>
...
comp_type = computer;
if ~strcmpi(comp_type(1:3),'PCW'), % not a PC Windows machine - use MAC path style.
	winfile  = [handles.win_path,handles.scenario,'/',handles.catcode,'.', handles.data_type];
else % assume PC Windows machine - use ordinary path style.
	winfile  = [handles.win_path,handles.scenario,'\',handles.catcode,'.', handles.data_type];
end
h_wait = waitbar(0.0,'reading window data file...','Name','Loading Data'); % launch waitbar.
if exist(winfile,'file') == 2, % sim data file exists.
    temp = csvread(winfile,1,0); % skip text header line.
    % csv format: [year,month,day,hour,sim_conc,obs_conc,sim_flow,obs_flow,stress] with 1 text header line.
    win_data = [datenum(temp(:,1),temp(:,2),temp(:,3), temp(:,4), ...
                zeros(length(temp(:,1)),1), zeros(length(temp(:,1)),1)), ...
                temp(:,5:end)];
else % abort.
    uiwait(errordlg(['Cannot open file: "',winfile,'". Aborting windowed data plotting procedure.' ],'File Error','modal'));
    close(h_wait);
    return;
end

% trim data to specified time interval...
win_data(win_data(:,1)<datenum(handles.mindate),:) = [];
win_data(win_data(:,1)>datenum(handles.maxdate)+1,:) = []; % +1 required to get "missing day".
% remove any row with NaN...
win_data(any(isnan(win_data)'),:) = [];

% find logged data set...
log_win_data = [win_data(:,1),log10(win_data(:,2:end))]; 
% remove any row with inf...
log_win_data(any(isinf(win_data)'),:) = [];

% determine whether to use raw or logged data...
% if a column is zeros (such as when there are no flow data)
%   quit this funtion
if get(findobj('Tag','log_scale_checkbox'),'Value'), % use log.
    log_data = 1;
    if size(log_win_data,1)==0,
        close(h_wait);
        close(h_fig);
        return;
    end    
...
%========== PAGE 1: CONCENTRATION DATA =============
%  (1) sim/obs conc.
%  (2) C-Q
%  (3) conc. CDF
%  (4) conc. histogram
waitbar(0.75,h_wait,'generating plots...'); % update waitbar.
title_catcode = handles.catcode;
title_catcode(findstr(title_catcode,'_'))='-';
h_fig = figure('Name','Windowed Data-Set Plots (CONCENTRATIONS)'); % create separate figure window.

%------- (I.1) 1:1 concentration scatter plot --------
% [serial_date, sim_conc, obs_conc, sim_flow, obs_flow, stress]
if log_data,
    data = log_win_data;
    label = ['LOG10 ',get_label(handles)];
else
    data = win_data;
    label = get_label(handles);
end
subplot(2,2,1); hold on;
xymin = min([data(:,2);data(:,3)]);
xymax = max([data(:,2);data(:,3)]);
if xymin == xymax,
    xymin = xymin - 1;
    xymax = xymax + 1;
end
plot(data(:,3),data(:,2),'ko','MarkerSize',4); % sim vs. obs dots.
% add least-square fit line with stats...
[b, bint, r, rint, stats] = regress(data(:,2),[ones(size(data(:,3),1),1),data(:,3)]);
plot([xymin,xymax],[b(2)*xymin+b(1),b(2)*xymax+b(1)],'r');
% lsline; % this gives same line as above command - use to check above command.
text(xymin+0.1*(xymax-xymin), xymin+0.90*(xymax-xymin),[  'm = ',num2str(b(2),2)],    'Color',[1,0,0]) % add m stats text
text(xymin+0.1*(xymax-xymin), xymin+0.85*(xymax-xymin),[ 'b  = ',num2str(b(1),2)],    'Color',[1,0,0]) % add b stats text
text(xymin+0.1*(xymax-xymin), xymin+0.80*(xymax-xymin),['r^2 = ',num2str(stats(1),2)],'Color',[1,0,0]) % add r2 stats text
% add ideal y = x line...
plot([xymin xymax], [xymin xymax], 'Color',[0 0.5 0], 'LineStyle',':'); % add ideal y=x line
xlabel(['OBSERVED ', label]);
ylabel(['SIMULATED ',label]);
title(['\bf',upper(title_catcode),': SIM VS. OBS CONCENTRATION']);
axis([xymin, xymax, xymin, xymax]);
set(gca,'XTick',get(gca,'YTick'));
set(gca,'XTickLabel',get(gca,'YTickLabel'));
axis equal;
...
if log_data,
    data = log_win_data;
%    data(data(:,2)<=log10(10),:) = []; % eliminate min threshold values.
%    data(data(:,3)<=log10(10),:) = []; % eliminate min threshold values.
    label = ['LOG ',get_label(handles)];
else
    data = win_data;
%    data(data(:,2)<=10,:) = []; % eliminate min threshold values.
%    data(data(:,3)<=10,:) = []; % eliminate min threshold values.
    label = get_label(handles);
end

subplot(2,2,2); hold on;
xmin = min([data(:,4);data(:,5)]); % flow data.
xmax = max([data(:,4);data(:,5)]); % flow data.
if xmin == xmax,
    xmin = xmin - 1;
    xmax = xmax + 1;
end
ymin = min([data(:,2);data(:,3)]); % conc data.
ymax = max([data(:,2);data(:,3)]); % conc data.
if ymin == ymax,
    ymin = ymin - 1;
    ymax = ymax + 1;
end
obs_h = plot(data(:,5),data(:,3),'o','MarkerSize',4,'Color',[0 0.5 0]); % obs conc vs. obs flow - green dots.
sim_h = plot(data(:,4),data(:,2),'o','MarkerSize',4,'Color',[1 0.0 0]); % sim conc vs. obs flow - red dots.
legend('Simulated','Observed');
% add least-square fit lines with stats...
text(xmin+0.1*(xmax-xmin), ymin+0.90*(ymax-ymin),  'm = ','Color',[0 0 0]) % add obs m stats text.
text(xmin+0.1*(xmax-xmin), ymin+0.85*(ymax-ymin), 'b  = ','Color',[0 0 0]) % add obs b stats text.
text(xmin+0.1*(xmax-xmin), ymin+0.80*(ymax-ymin),'r^2 = ','Color',[0 0 0]) % add obs r2 stats text.
[b, bint, r, rint, stats] = regress(data(:,3), [ones(size(data(:,5),1),1), data(:,5)]);
plot([xmin,xmax],[b(2)*xmin+b(1),b(2)*xmax+b(1)],'Color',[0 0.5 0]);
% lsline; this gives same line as above command - use to check above command.
text(xmin+0.3*(xmax-xmin), ymin+0.90*(ymax-ymin),[num2str(b(2),2),','],    'Color',[0 0.5 0]) % add obs m stats text
text(xmin+0.3*(xmax-xmin), ymin+0.85*(ymax-ymin),[num2str(b(1),2),','],    'Color',[0 0.5 0]) % add obs b stats text
text(xmin+0.3*(xmax-xmin), ymin+0.80*(ymax-ymin),[num2str(stats(1),2),','],'Color',[0 0.5 0]) % add obs r2 stats text
[b, bint, r, rint, stats] = regress(data(:,2), [ones(size(data(:,4),1),1), data(:,4)]);
plot([xmin,xmax],[b(2)*xmin+b(1),b(2)*xmax+b(1)],'Color',[1 0 0]);
% lsline; % this gives same line as above command - use to check above command.
text(xmin+0.5*(xmax-xmin), ymin+0.90*(ymax-ymin),num2str(b(2),2),    'Color',[1 0 0]) % add sim m stats text.
text(xmin+0.5*(xmax-xmin), ymin+0.85*(ymax-ymin),num2str(b(1),2),    'Color',[1 0 0]) % add sim b stats text.
text(xmin+0.5*(xmax-xmin), ymin+0.80*(ymax-ymin),num2str(stats(1),2),'Color',[1 0 0]) % add sim r2 stats text.
if log_data,
    xlabel('LOG10 TOTAL DISHARGE, IN CFS');
else
    xlabel('TOTAL DISHARGE, IN CFS');
end
ylabel(label);
title(['\bf',upper(title_catcode),': C-Q SCATTER PLOTS']);
axis([xmin,xmax,ymin,ymax]);
box on;
...
sim_h = cdfplot(data(:,2)); set(sim_h,'Color','r');
obs_h = cdfplot(data(:,3)); set(obs_h,'Color',[0 0.5 0]);
title(['\bf',upper(title_catcode),': CONCENTRATION CDF']);
xlabel(label);
ylabel('CUMULATIVE DISTRIBUTION');
box on;
legend('Simulated','Observed',4);
% add kstest2 stats...
[H,P,KSSTAT] = kstest2(data(:,2),data(:,3),0.05,0);
xmin = get(gca,'XLim'); xmin = xmin(1);
xmax = get(gca,'XLim'); xmax = xmax(2);
ymin = get(gca,'YLim'); ymin = ymin(1);
ymax = get(gca,'YLim'); ymax = ymax(2);
text(xmin+0.1*(xmax-xmin), ymin+0.45*(ymax-ymin),['h = ',num2str(H,2)],     'Color',[0,0,0]);
text(xmin+0.1*(xmax-xmin), ymin+0.50*(ymax-ymin),['p = ',num2str(P,2)],     'Color',[0,0,0]);
text(xmin+0.1*(xmax-xmin), ymin+0.55*(ymax-ymin),['k = ',num2str(KSSTAT,2)],'Color',[0,0,0]);
grid off;
...
subplot(2,2,4);
hist(data(:,2)); hold on; % sim.
hist(data(:,3)); % obs.
h = get(gca,'Children');
set(h(1),'FaceColor','none',   'EdgeColor','r');
set(h(2),'FaceColor',[0 0.5 0],'EdgeColor',[0 0.5 0]);
legend('Simulated','Observed');
xlabel(label);
ylabel('COUNT');
title(['\bf',upper(title_catcode),': CONCENTRATION HISTOGRAMS']);
box on;

% ----- log message text ---
if log_data, % show log meesage text.
    h_warn = uicontrol('Style', 'text', ...
		'String', 'ALL DATA LOG10 TRANSFORMED', ...
        'Units', 'normalized', ...
		'Position', [0.88 0.95 0.12 0.05], ...
...
%  (3) load CDF
%  (4) load histogram
h_fig = figure('Name','Windowed Data-Set Plots (LOADS)'); % create separate figure window.

% [serial_date, sim_conc, obs_conc, sim_flow, obs_flow, stress]
% adjust label units...
label = get_label(handles);
index = strfind(upper(label),'MG/L');
if ~isempty(index), % replace with "TON/DAY" (replaces 1st occurence only).
    label(index(1):index(1)+3) = [];
    label = [label(1:index(1)-1),'TON/DAY',label(index(1):end)];
else % append "TON/DAY".
    label = [label,' IN TON/DAY'];
end
if log_data,
    label = ['LOG10 ',label];
end
% calculate loads and replace conc with them...
orig_data = win_data; % keep original for "stress" plot below.
conv = 2.69686023414422; % cfs*mg/L => ton/day.
win_data(:,2) = conv*win_data(:,2).*win_data(:,4); % sim: replace conc. with converted load.
win_data(:,3) = conv*win_data(:,3).*win_data(:,5); % obs: replace conc. with converted load.
% [serial_date, sim_load, obs_load, sim_flow, obs_flow, stress]

% find logged data set...
log_win_data = [win_data(:,1),log10(win_data(:,[2,3])),win_data(:,4:end)]; % just log loads.
% remove any row with inf...
log_win_data(any(isinf(log_win_data)'),:) = [];

%------- (II.1) 1:1 load scatter plot --------
% [serial_date, sim_load, obs_load, sim_flow, obs_flow, stress]
if log_data,
    data = log_win_data;
else
...
xlabel(['OBSERVED ', label]);
ylabel(['SIMULATED ',label]);
title(['\bf',upper(title_catcode),': SIM VS. OBS LOADS']);
axis([xymin, xymax, xymin, xymax]);
set(gca,'XTick',get(gca,'YTick'));
set(gca,'XTickLabel',get(gca,'YTickLabel'));
% axis equal;
...
% [serial_date, sim_conc, obs_conc, sim_flow, obs_flow, stress]
subplot(2,2,2); hold on;
% no log transforms for this.
stress = orig_data(:,6);
error = (orig_data(:,2) - orig_data(:,3)); % error = (sim_conc - obs_conc).
xmin = min(stress);
xmax = max(stress);
if xmin == xmax,
    xmin = xmin - 1;
...
ymin = min(error);
ymax = max(error);
if abs(ymin) < abs(ymax), % make y-axis symetrical about 0.
    ymin = -abs(ymax);
    ymax =  abs(ymax);
...
plot(stress,error,'ro','MarkerSize',4); hold on; % error dots.
plot([xmin,xmax],[0 0], 'Color',[0 0.5 0], 'LineStyle',':'); % add y=0 line.
xlabel('SHEAR STRESS');
ylabel('CONCENTRATION ERROR');
title(['\bf',upper(title_catcode),': ERROR VS. SHEAR STRESS']);
axis([xmin,xmax,ymin,ymax]);
box on;
...
sim_h = cdfplot(data(:,2)); set(sim_h,'Color','r');
obs_h = cdfplot(data(:,3)); set(obs_h,'Color',[0 0.5 0]);
title(['\bf',upper(title_catcode),': LOAD CDF']);
xlabel(label);
ylabel('CUMULATIVE DISTRIBUTION');
box on;
legend('Simulated','Observed',4);
...
xlabel(label);
ylabel('COUNT');
title(['\bf',upper(title_catcode),': LOAD HISTOGRAMS']);
box on;

% ----- log message text ---
if log_data, % show log meesage text.
    h_warn = uicontrol('Style', 'text', ...
		'String', 'LOAD DATA LOG10 TRANSFORMED', ...
        'Units', 'normalized', ...
		'Position', [0.88 0.95 0.12 0.05], ...
		'ForegroundColor', [1 0 0], ...
		'BackgroundColor', [1 1 1], ...
...
%------------------------------------------------------------------------------------------------------
%=============== WINDOWED DATA PLOTS with save to PDF ==================
% --- Executes on button press in window_plot_pushbutton.
function window_save_PDF_Callback(hObject, eventdata, handles)
% hObject    handle to window_plot_pushbutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
date(findstr(date,':')) = [];
date(15:16) = [];
file_name = upper([handles.catcode,'_',handles.data_type,'_window_conc_',date,'.ps']);

% save image...
set(h_fig,'PaperOrientation','landscape','InvertHardcopy','off','PaperPositionMode','manual',...
        'PaperUnits','normalized','PaperPosition',[0,0,1,1]);

print(h_fig,'-dpsc','-r0',file_name);
...
% can not work without data
if size(data(:,2),1) == 0 || size(data(:,3),1) == 0,
   close(h_fig);
   return;
end
...
% add least-square fit line with stats if no error...
[b, bint, r, rint, stats] = regress(data(:,2),[ones(size(data(:,3),1),1),data(:,3)]);
plot([xymin,xymax],[b(2)*xymin+b(1),b(2)*xymax+b(1)],'r');
% lsline; % this gives same line as above command - use to check above command.
text(xymin+0.1*(xymax-xymin), xymin+0.90*(xymax-xymin),[  'm = ',num2str(b(2),2)],    'Color',[1,0,0]) % add m stats text
...
date(findstr(date,':')) = [];
date(15:16) = [];
file_name = upper([handles.catcode,'_',handles.data_type,'_window_load_',date,'.ps'])
...
file_name = upper([handles.catcode,'_',handles.data_type,'_',date,'.ps']);
...
print('-dpsc','-r0',file_name);
if exist(handles.image_dir,'dir') ~= 7, % image directory doesn't exist - create.
...
% This function executes if there is no ".prefs" file, prompting the user to speciy the file search paths.
function [river_sim_path,land_sim_path,obs_path,connect_path,batch_path,win_path,err] = get_paths
river_sim_path = [];
land_sim_path = [];
obs_path = [];
connect_path = [];
...
comp_type = computer;
if ~strcmpi(comp_type(1:3),'PCW'), % not a PC Windows machine - use MAC path style. Don't need drive letter.
    prompt = {'Enter your workspace folder name:','Enter the data scenario:'};
    dlg_title = 'Set File Search Paths';
    num_lines= 1;
    answers  = inputdlg(prompt,dlg_title,num_lines);
    answers{3} = answers{2};
    answers{2} = answers{1};
else % assume PC Windows machine - use ordinary path style. Need drive letter.
    prompt = {'Enter letter of mapped CBP network drive:','Enter your workspace folder name:','Enter the data scenario:'};
    dlg_title = 'Set File Search Paths';
    num_lines= 1;
    answers  = inputdlg(prompt,dlg_title,num_lines);
end
if isempty(answers), % cancel was pressed.
    err = 2;
    return;
end
drive = answers{1};
folder = answers{2};
data_scen = answers{3};
if (isempty(drive) || isempty(folder) || isempty(data_scen)), % at least 1 null entery.
    err = 1;
    return;
else % something was entered for all.
    err = 0;
end
...
if ~isempty(findstr(folder,' ')), % folder has space - need single quotes.
    folder = ['''',folder,''''];
end
if ~isempty(findstr(data_scen,'\')) % data_scen has slash - remove.
    data_scen(findstr(data_scen,'\')) = [];
end
if ~isempty(findstr(data_scen,'/')) % data_scen has slash - remove.
    data_scen(findstr(data_scen,'/')) = [];
end
if ~isempty(findstr(data_scen,' ')), % data_scen has space - need single quotes.
    data_scen = ['''',data_scen,''''];
end

% build search paths...
if ~strcmpi(comp_type(1:3),'PCW'), % not a PC Windows machine - use MAC path style.
    river_sim_path = ['/bluefish/archive/',folder,'/output/river/daily/']; % search path for river simulated data.
    land_sim_path = ['/bluefish/archive/',folder,'/output/pltgen/land/']; % search path for land simulated data.
    % obs_path = ['/bluefish/archive/p5_observed/']; % search path for observed data.
    % obs_path = ['/bluefish/archive/',folder,'/pp/observed/'] % search path for observed data.
    obs_path = ['/bluefish/archive/',folder,'/input/calib/observed/',handles.ObScen,'/']; % search path for observed data.
    connect_path = ['/bluefish/archive/',folder,'/config/catalog/geo/p52/']; % search path for connect files.
    batch_path = ['/bluefish/archive/',folder,'/config/seglists/']; % search path for ".calib" file for batch processing.
    win_path = ['/bluefish/archive/',folder,'/output/river/window/']; % search path for windowed data file.
else % assume PC Windows machine - use ordinary path style.
    ...
    % obs_path = [drive,':\',folder,'\pp\observed\']; % search path for observed data.
    obs_path = [drive,':\',folder,'\input\calib\observed\',handles.ObScen,'\']; % search path for observed data.
    connect_path = [drive,':\',folder,'\config\catalog\geo\p52\']; % search path for connect files.
    batch_path = [drive,':\',folder,'\config\seglists\']; % search path for ".calib" file for batch processing.
    win_path = [drive,':\',folder,'\output\river\window\']; % search path for windowed data file.
end
...
% obs_path = '..\..\..\..\p5_observed\'; % search path for observed data.
