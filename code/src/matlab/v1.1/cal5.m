...
handles.sim_path = '..\..\..\output\river\daily\'; % search path for simulated data.
handles.obs_path = '..\..\data\observed\'; % search path for observed data.
handles.connect_path = '..\..\catalog\geo\'; % search path for connect files.
...
% set filter flag if seasonal filter is specified...
...
have_obs  = 0;
have_prec = 0;
if handles.data_type == 'SNOW'
    simfile  = ['..\..\..\output\pltgen\land\',handles.scenario,'\grs_',handles.catcode,'.',handles.data_type]; % use grass (grs) land use as representative snow depth for entire segment.
else
    simfile  = [handles.sim_path,handles.scenario,'\',handles.catcode,'.', handles.data_type];
end
...
%?****************
% clc;
% if handles.have_sim,  disp('have sim'), else, disp('* no sim'), end;
% if handles.have_obs,  disp('have obs'), else, disp('* no obs'), end;
% if handles.have_prec, disp('have prc'), else, disp('* no prc'), end;
%*****************
