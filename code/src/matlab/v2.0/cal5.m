...
% Last Modified by GUIDE v2.5 20-Apr-2005 10:06:20
...
handles.default_maxdate = '12/31/1999';
...
comp_type = computer;
if comp_type(1:3) ~= 'PCW', % not a PC Windows machine - use MAC path style.
...
if exist(obsfile) == 2 % obs data file exists.
    ...
    %??? NEW OBS FILE FORMATS
		...
		[c1,c2,c3,c4,c5,c6,c7,c8] = textread(obsfile,'%f %f %f %f %f %f %s %s','delimiter',',');
...
if exist(winfile) == 2, % sim data file exists.
...
file_name = upper([handles.catcode,'_',date,'.ps']);
...
comp_type = computer;
if comp_type(1:3) ~= 'PCW', % not a PC Windows machine - use MAC path style. Don't need drive letter.
...
folder = answers{2};
data_scen = answers{3};
if (length(drive)==0 | length(folder)==0 | length(data_scen)==0), % at least 1 null entery.
    err = 1;
    return;
