...
********* get and process input
      read(*,*,err=995,end=996) rscen,DFscen,rseg,
     ...
     .                          year1,year2
      ...
      print*,' with delivery factors from ',Dfscen(:lenDFscen)

********* create directories
      fnam= outdir//'del/lseg/monthly/'//rscen(:lenrscen)
     .         //'_DF_'//DFscen(:lenDFscen)
      command = 'mkdir -p '//fnam
      call system(command)
...
*************** all book keeping done
      ...
      if (monthly) then
        do ns = 1,numsegs
          ...
          call getmondfs(rseg,lenrseg,DFscen,lenDFscen,lseg,
     ...
     I                 sdate,edate,segDFmon,basinDFmon,resDFmon)
      ...
      if (annual) then
        do ns = 1,numsegs
          lseg = l2r(ns)
          call lencl(lseg,lenlseg)
          call getanndfs(rseg,lenrseg,DFscen,lenDFscen,lseg,
     I                   ndel,delname,sdate,edate,
     ...
     I                 sdate,edate,segDFann,basinDFann,resDFann)
      ...
      if (aveann) then
        do ns = 1,numsegs
          lseg = l2r(ns)
          call lencl(lseg,lenlseg)
          call getavedfs(rseg,lenrseg,DFscen,lenDFscen,lseg,
