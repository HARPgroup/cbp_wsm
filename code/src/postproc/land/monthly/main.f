      ...
      integer     I_year1,I_year2,I_YR,I_MM
      ...
      integer     ival
      real        F_AVG_Load,F_MON_LOAD(1984:2025,12,10)
...
c      fnam = catdir//'iovars/'//'p600'//'/land_loads'
      ...
      ofnam=outdir//'land/monthly/'//lscen(:lenlscen)//'/'//C_lu//'_'//
     .       lseg(:lenlseg)//'.out'
      print*,ofnam
...
c            print*,'-->',t_C_VAR,' x',t_F_FAC
         ...
         F_MON_Load(I_YR,I_MM,I_NCOL) = F_AVG_Load
...
111   do I_YR = sdate(1),edate(1)
       do I_MM = 1,12
         write(dfile+1,'(A,A,A,A,I4,A,I2$)'),lseg(:lenlseg),',',C_lu,
     .    ',',I_YR,',',I_MM
         do I_COL = 1,I_NCOL
            write(dfile+1,'(A,E10.5$)'),',',F_MON_Load(I_YR,I_MM,I_COL)
         end do
         write(dfile+1,*),''
       end do
      end do
