...
************ END DECLARATIONS ******************************************
      ...
      read(*,*,err=997,end=998) rscen,DFscen,basin,year1,year2,
     .                          iEOF,iEOS,iDEL
     ...
     .       rscen(:lenrscen),' river basin ',basin(:lenbasin)
      print*,'using delivery factors from ',DFscen(:lenDFscen)

*********** open all data files to feed scenario builder postprocessor
      fnam = sumoutdir//'aveann/'//CombScen(:lenCombScen)//
...
***************** DEL ***********************
          if (doDEL) then
            ...
            do nl = 1,nlu
              ...
              call readEOaveann(
     I                          CombScen,lsegs(nlseg),rsegs(nrseg),
