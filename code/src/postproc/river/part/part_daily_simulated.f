...
C      write(*,*) 'Enter a 13-digit reach identifier: '
      read*, rscen, catcode
      print*,'separating base and quick flow for ',catcode,' ',rscen
      do i = 1,25
        if (rscen(i:i).ne.' ') last = i
      end do
      fnam = outdir//'river/daily/'//rscen(:last)
     .       //'/'//catcode//'.flow'
     ...
     O                          geoscen)
      ...
      watershedfile = catdir//'geo/'//geoscen(:lengeoscen)//
     .              '/watershed_area.csv'
c
c     if fnam exists,
...
c     simulated input flow file is 'catcode.flow'
      ...
      catcode = ofile(last-17:last-5)
      fname1 = ofile(:last-4)//'bflw'
      fname2 = ofile(:last-4)//'qflw'
c
