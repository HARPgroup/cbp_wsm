c     ==================================================================
      SUBROUTINE PART(sim,ndaymax,ndays,watershedfile,year1,nm,nd,
     .                catcode,
     O                bsim,qsim,err)
...
c
*********** added variables for phase5 software
      integer ndaymax, ndays  ! dimension and meaningful size of variables
      character*100 watershedfile ! name of watershed area file
      integer year1,nm,nd         ! starting date of observations
      real sim(ndaymax), bsim(ndaymax), qsim(ndaymax) ! flow, base, and quick 
      integer err  ! return error code

      integer daycount  ! index


      INTEGER MODAYS(120,12)
      INTEGER IYR1D(44000),IMO1D(44000),IDA1D(44000)
      INTEGER ITBASE,TBASE1,IYEAR,IMONTH,IDAY,IHR,IMIN,IYRKEEP
c
...
c
      ...
      CHARACTER*100 ofile,fname1,fname2,fnam
      CHARACTER*90 LINE
...
c
      ...
      IFRSTYR=0
c
      IYRKEEP = year1   ! set initial date (first 'read' statement)
      IMONTH = nm
      IDAY = nd

      call yesterday(IYRKEEP,imonth,iday)
      daycount = 0

   35 continue
*********** READ (9,'(A90)',END = 37) LINE
***********      READ (LINE,*) IYEAR,IMONTH,IDAY,IHR,IMIN,QRD
c
      daycount = daycount + 1
      if (daycount.gt.ndays) go to 37
      call tomorrow(IYRKEEP,imonth,iday)
      QRD = sim(daycount)

      IF(IFRSTYR.EQ.0) IFRSTYR=IYRKEEP
      IYEAR = IYRKEEP-IBEFORE
c      DO 36 J=1,8
...
c      WRITE (*,*) 'READING FILE NAMED watershed_area.csv'
  ...
  120 READ (11,'(A90)',END = 121,err=122) LINE
      do i=1,90
	if (LINE(i:i).eq.CHAR(13)) LINE(i:i)=' '
      end do
      if (LINE(:3).eq.'END'.or.LINE(:3).eq.'end') GO TO 123
c
      READ (LINE,*,err=122,end=121) ccode,DA
      IF(ccode.NE.catcode) GO TO 120
c      WRITE (*,*) 'FILE NAME:', catcode
c      WRITE (*,*) 'DRAINAGE AREA:', DA
CGSHENK      CLOSE (11,STATUS='KEEP')
      CLOSE (11)
      go to 124
121   err = 998
      return  
...
c     $          DATE '
  ...
  835       CONTINUE
           ...
           DO 840 I=ISTRT, ISTRT+NUMDAYS-1
              JULDATE= I-ISTRT+1
              daycount = daycount + 1
C                 WRITE (21,15) IYR1D(I)+IBEFORE,IMO1D(I),
C     $			IDA1D(I),B1D(I,1)
                  bsim(daycount) = B1D(I,1)
C                 WRITE (22,15) IYR1D(I)+IBEFORE,IMO1D(I),
C     $			IDA1D(I),Q1D(I)-B1D(I,1)
                  qsim(daycount) = Q1D(I) - B1D(I,1)
  840       CONTINUE
c           WRITE (*,*) ' '
