...
C      include '../lib/inc/land_use.inc'
      ...
      integer year1,year2,year,month  !GY
...
********** write annual file
c      fnam = outdir//'input/'//lscen(:lenlscen)//'/annual_'//apptype//
c     .       '_'//lseg//'_'//clu//'.csv'

c      open(11,file=fnam,status='unknown',iostat=err)
c      if (err.ne.0) go to 991

c      write(11,*,err=951) 'year,',species(1),
c     .                    (',',species(sp),sp=2,nspecies)
c      do year = year1,year2
c        write(11,1233,err=951) year,(annapp(year,sp),sp=1,nspecies)
c      end do

c      close(11)

********** write average annual file
c      fnam = outdir//'input/'//lscen(:lenlscen)//'/'//apptype//
c     .       '_'//lseg//'_'//clu//'_'//cy1//'_'//cy2//'.csv'

c      open(11,file=fnam,status='unknown',iostat=err)
c      if (err.ne.0) go to 991

c      write(11,*,err=951) species(1),(',',species(sp),sp=2,nspecies)
c      write(11,1234,err=951) (totapp(sp),sp=1,nspecies)
