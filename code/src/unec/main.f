      ...
      double precision   D_tSFLO
      double precision   D_SURFfactor,D_BASEfactor
      double precision   D_Dfactor,D_Sfactor,D_Ifactor,D_Afactor
...
c      integer            lentarget
      ...
      double precision   D_tAnnApp
      double precision   D_AnnApp_Max, D_AnnApp_Min, D_AnnApp_Amp
...
c      logical L_S2MIN_ZERO            ! ** wheather to set -ve s2 UNEC to 0 or positive
c      parameter ( L_S2MIN_ZERO = .false. ) ! default = false, set to positive, not 0
...
c     .            I_YearC1,I_YearC2,
     ...
     I                             lscen,lenlscen,
     O                             I_Year1,I_null,I_null,
     O                             I_Year2,I_null,I_null)

      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC PARAM',10,
     .                             C25_UNEC_PARAM)
      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC PARAM',10,
     .                             C25_SENSITIVITY)
c      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC TARGET',11,
c     .                             C25_TARGET)
      call readcontrol_load(lscen,lenlscen,
     .                      'UNEC TARGET',11,
     .                      'END UNEC TARGET',15,
     O                      I_YearC1,I_YearC2,C25_TARGET)
...
! BHATT ??
      ...
      if ( C_fracSDIA .eq. 'N' ) then
         print*,'C_fracSDIA = ',C_fracSDIA
         data D_fracS /7*1.00D0/
         data D_fracD /7*1.00D0/
         data D_fracI /7*1.00D0/
         data D_fracA /7*1.00D0/
      end if
...
c^      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC PARAM',10,
c^     .                             C25_UNEC_PARAM)
c^      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC PARAM',10,
c^     .                             C25_SENSITIVITY)
c^      call readcontrol_Lunec_Table(lscen,lenlscen,'UNEC TARGET',11,
...
c^      call lencl(C25_UNEC_PARAM,lC25_UNEC_PARAM)
c^      call lencl(C25_SENSITIVITY,lC25_SENSITIVITY)
c^      call lencl(C25_TARGET,lC25_TARGET)
...
c      data D_MonApp_Pve /I_MAXMONTHS*0.0D0/
c      data D_MonApp_Nve /I_MAXMONTHS*0.0D0/
c      data D_MonApp     /I_MAXMONTHS*0.0D0/
c      data D_MonApp_Pve_Tot /0.0/
c      data D_MonApp_Nve_Tot /0.0/
c      data D_MonApp_Tot     /0.0/
...
c                  D_tAGWO = D_tAGWO + F_hAGWO(j)
         ...
         print*,'SURO (in/ac/yr) ', D_tSURO
         print*,'SEDM (tn/ac/yr) ', D_tSEDM
         print*,'IFWO (in/ac/yr) ', D_tIFWO
         print*,'AGWO (in/ac/yr) ', D_tAGWO

         D_tSFLO = D_tSURO + D_tIFWO
...
c            end do
            ...
            !>> SPLIT SURFACE TARGET INTO S,D,I
            D_TargetA = D_bTarget(l,k)

            if ( quals_to_states(k,2) .eq. 0 ) D_TargetD = 0
            if ( quals_to_states(k,2) .ne. 0 ) D_TargetD = 
     .         D_sTarget(l,k) * ( 1 /
     .               ( 1+(D_tSFLO*0.057)/(D_tSEDM*0.168) ) )
            D_TargetS = ( D_sTarget(l,k) - D_TargetD ) * D_tSURO/D_tSFLO
            D_TargetI = ( D_sTarget(l,k) - D_TargetD ) * D_tIFWO/D_tSFLO
            ...
            D_TargetIc = 0.0
            D_TargetAc = 0.0
            if (D_tSURO.gt.0) D_TargetSc = D_TargetS / D_tSURO
            if (D_tSEDM.gt.0) D_TargetDc = D_TargetD / D_tSEDM
            if (D_tIFWO.gt.0) D_TargetIc = D_TargetI / D_tIFWO
            if (D_tAGWO.gt.0) D_TargetAc = D_TargetA / D_tAGWO

            if ( C_fracSDIA .eq. 'D' ) then
              print*,'C_fracSDIA = ',C_fracSDIA
              if (D_TargetT.gt.0) D_fracS(k) = D_TargetS / D_TargetT
              if (D_TargetT.gt.0) D_fracD(k) = D_TargetD / D_TargetT
              if (D_TargetT.gt.0) D_fracI(k) = D_TargetI / D_TargetT
              if (D_TargetT.gt.0) D_fracA(k) = D_TargetA / D_TargetT
            end if

            print*,luname(l),' ',quals(k),D_sTarget(l,k),D_bTarget(l,k)
            print*,luname(l),' ',quals(k),' TRGT ',
     .         D_TargetS,D_TargetD,D_TargetI,D_TargetA
            print*,luname(l),' ',quals(k),' FLOW ',
     .         D_tSURO,D_tSEDM,D_tIFWO,D_tAGWO
            print*,luname(l),' ',quals(k),' CONC ',
     .         D_TargetSc,D_TargetDc,D_TargetIc,D_TargetAc
...
c     .                          * D_APP_WEIGHTS(l,i,k)
     ...
     .                       D_MonApp_Tot = D_MonApp_Tot + D_tMonApp
            ...
            D_AnnApp_Max = -999.0D0
            D_AnnApp_Min = +999.0D0

            if ( L_APP_BALANCE_UPTK .eqv. .false. ) then
              D_APP_BALANCE_UPTKf = 1.0D0
            else
              if ( (0.99D0 * D_MonApp_Pve_Tot + D_MonApp_Nve_Tot) 
     .          .lt. 0.0 ) then
                D_APP_BALANCE_UPTKf = -0.99D0 * 
     .                                D_MonApp_Pve_Tot/D_MonApp_Nve_Tot
              else
                D_APP_BALANCE_UPTKf = 1.0D0
              end if
            end if

            D_APP_BALANCE_TRGTf = 1.0D0
            D_APP_BALANCE_TRGTb = 0.0D0
            if ( L_APP_BALANCE_TRGT .eqv. .true. ) then
              if ( C_APP_BALANCE_TRGT .eq. 'b' ) then
               D_APP_BALANCE_TRGTb = D_TargetT - 
     .          ( (0.99D0 * D_MonApp_Pve_Tot +
     .           D_APP_BALANCE_UPTKf * D_MonApp_Nve_Tot)/
     .           (I_YearC2 - I_YearC1 + 1) )
               D_APP_BALANCE_TRGTb = D_APP_BALANCE_TRGTb / 12
              else
               D_APP_BALANCE_TRGTf = D_TargetT / 
     .          ( (0.99D0 * D_MonApp_Pve_Tot + 
     .           D_APP_BALANCE_UPTKf * D_MonApp_Nve_Tot)/
     .           (I_YearC2 - I_YearC1 + 1) )
              end if
              print*,D_TargetT,0.99D0 * D_MonApp_Pve_Tot,
     .         D_APP_BALANCE_UPTKf * D_MonApp_Nve_Tot,
     .         D_APP_BALANCE_TRGTf,D_APP_BALANCE_TRGTb
            end if

            D_MonApp_Max = -999.0D0
            D_MonApp_Min = +999.0D0
            D_MonApp_Amp = -999.0D0
            do yy = I_Year1,I_Year2
            !{
              D_tAnnApp = 0.0D0
c              D_MonApp_Max = -999.0D0
c              D_MonApp_Min = +999.0D0
              do mm = 1,12
              !{
                D_tMonApp = 
     .           D_APP_BALANCE_TRGTb + D_APP_BALANCE_TRGTf *
     .           (D_MonApp_Pve((yy-I_Year1)*12+mm) + 
     .           D_MonApp_Nve((yy-I_Year1)*12+mm) * D_APP_BALANCE_UPTKf)
                D_tAnnApp = D_tAnnApp + D_tMonApp
                D_MonApp((yy-I_Year1)*12+mm) = D_tMonApp
                if (yy.ge.I_YearC1 .and. yy.le.I_YearC2) then
...
c              if ( D_MonApp_Amp.gt.(D_MonApp_Max-D_MonApp_Min) ) 
c     .          D_MonApp_Amp = D_MonApp_Max-D_MonApp_Min
              if (yy.ge.I_YearC1 .and. yy.le.I_YearC2) then
                if (D_tAnnApp.gt.D_AnnApp_Max) D_AnnApp_Max=D_tAnnApp
                if (D_tAnnApp.lt.D_AnnApp_Min) D_AnnApp_Min=D_tAnnApp
              end if
            !}
            end do
            D_AnnApp_Amp = D_AnnApp_Max - D_AnnApp_Min
            print*,quals(k),' ANN MAX MIN ',D_AnnApp_Max,D_AnnApp_Min,
     .        D_AnnApp_Amp
            D_MonApp_Amp = D_MonApp_Max-D_MonApp_Min
            print*,quals(k),' MON MAX MIN ',D_MonApp_Max,D_MonApp_Min,
     .        D_MonApp_Amp

            if ( I_DEBUG0 .eq. 1 )
     .       print*,"Total Monthly Application/Input ",quals(k)
...
c               if (D_MonApp(i).gt.D_MonApp_Max) D_MonApp_Max=D_MonApp(i)
c               if (D_MonApp(i).lt.D_MonApp_Min) D_MonApp_Min=D_MonApp(i)
               if ( I_DEBUG0 .eq. 1 )
...
C            return
            ...
            do i = 1,I_SPIN
            !{
c               if ( I_DEBUG0 .eq. 1) print*,'SPIN ',i
...
c                  if ( a2Factors(j).lt.0 .and. D_MonApp(i).lt.0 ) cycle
     ...
     .       write(*,'(A)') 'Break Through Curve of Analyte '//quals(k)
            ...
            ! *** Adjust S2 Amplitude to Input x Sensitivity
            s2FacAnnMin = +999.0D0
            d2FacAnnMin = +999.0D0
            i2FacAnnMin = +999.0D0
            a2FacAnnMin = +999.0D0
            ...
            do yy = I_YearC1,I_YearC2
             ...
             a2FactorsAnn = 0.0D0
             do mm = 1,12
              s2FactorsAnn=s2FactorsAnn+s2Factors((yy-I_Year1)*12+mm)/12
              d2FactorsAnn=d2FactorsAnn+d2Factors((yy-I_Year1)*12+mm)/12
              i2FactorsAnn=i2FactorsAnn+i2Factors((yy-I_Year1)*12+mm)/12
              a2FactorsAnn=a2FactorsAnn+a2Factors((yy-I_Year1)*12+mm)/12
             end do
             if(s2FactorsAnn.gt.s2FacAnnMax) s2FacAnnMax=s2FactorsAnn
             if(d2FactorsAnn.gt.d2FacAnnMax) d2FacAnnMax=d2FactorsAnn
             if(i2FactorsAnn.gt.i2FacAnnMax) i2FacAnnMax=i2FactorsAnn
             if(a2FactorsAnn.gt.a2FacAnnMax) a2FacAnnMax=a2FactorsAnn

             if(s2FactorsAnn.lt.s2FacAnnMin) s2FacAnnMin=s2FactorsAnn
             if(d2FactorsAnn.lt.d2FacAnnMin) d2FacAnnMin=d2FactorsAnn
             if(i2FactorsAnn.lt.i2FacAnnMin) i2FacAnnMin=i2FactorsAnn
             if(a2FactorsAnn.lt.a2FacAnnMin) a2FacAnnMin=a2FactorsAnn
            end do

            s2FacAnnAmp = s2FacAnnMax - s2FacAnnMin
            d2FacAnnAmp = d2FacAnnMax - d2FacAnnMin
            i2FacAnnAmp = i2FacAnnMax - i2FacAnnMin
            a2FacAnnAmp = a2FacAnnMax - a2FacAnnMin
            ...
            i2FacMonMax = -999.0D0
            a2FacMonMax = -999.0D0 
            do i = I_MonthsC1,I_MonthsC2
              if(s2Factors(i).gt.s2FacMonMax) s2FacMonMax=s2Factors(i)
              if(d2Factors(i).gt.d2FacMonMax) d2FacMonMax=d2Factors(i)
              if(i2Factors(i).gt.i2FacMonMax) i2FacMonMax=i2Factors(i)
              if(a2Factors(i).gt.a2FacMonMax) a2FacMonMax=a2Factors(i)

              if(s2Factors(i).lt.s2FacMonMin) s2FacMonMin=s2Factors(i)
              if(d2Factors(i).lt.d2FacMonMin) d2FacMonMin=d2Factors(i)
              if(i2Factors(i).lt.i2FacMonMin) i2FacMonMin=i2Factors(i)
              if(a2Factors(i).lt.a2FacMonMin) a2FacMonMin=a2Factors(i)
            end do

            s2FacMonAmp = s2FacMonMax - s2FacMonMin
            d2FacMonAmp = d2FacMonMax - d2FacMonMin
            i2FacMonAmp = i2FacMonMax - i2FacMonMin
            a2FacMonAmp = a2FacMonMax - a2FacMonMin

            print*,quals(k),' ANN MAX MIN S ',s2FacAnnMax,s2FacAnnMin
     .                     ,s2FacAnnAmp
            print*,quals(k),' ANN MAX MIN D ',d2FacAnnMax,d2FacAnnMin
     .                     ,d2FacAnnAmp
            print*,quals(k),' ANN MAX MIN I ',i2FacAnnMax,i2FacAnnMin
     .                     ,i2FacAnnAmp
            print*,quals(k),' ANN MAX MIN A ',a2FacAnnMax,a2FacAnnMin
     .                     ,a2FacAnnAmp
            print*,' '
            print*,quals(k),' MON MAX MIN S ',s2FacMonMax,s2FacMonMin
     .                     ,s2FacMonAmp
            print*,quals(k),' MON MAX MIN D ',d2FacMonMax,d2FacMonMin
     .                     ,d2FacMonAmp
            print*,quals(k),' MON MAX MIN I ',i2FacMonMax,i2FacMonMin
     .                     ,i2FacMonAmp
            print*,quals(k),' MON MAX MIN A ',a2FacMonMax,a2FacMonMin
     .                     ,a2FacMonAmp
            print*,' '
            print*,quals(k),' AINP MAX MIN ',D_AnnApp_Max, D_AnnApp_Min,
     .           D_AnnApp_Amp
            print*,quals(k),' MINP MAX MIN ',D_MonApp_Max, D_MonApp_Min,
     .           D_MonApp_Amp
            print*,D_MonApp_Amp/D_AnnApp_Amp
            print*,s2FacMonAmp/s2FacAnnAmp,d2FacMonAmp/d2FacAnnAmp
     .             ,i2FacMonAmp/i2FacAnnAmp,a2FacMonAmp/a2FacAnnAmp
            print*,quals(k),' ANN ',
     .            (D_AnnApp_Amp/s2FacAnnAmp),
     .            (D_AnnApp_Amp/d2FacAnnAmp),
     .            (D_AnnApp_Amp/i2FacAnnAmp),
     .            (D_AnnApp_Amp/a2FacAnnAmp)
            print*,quals(k),' MON ',
     .            (D_MonApp_Amp/s2FacMonAmp),
     .            (D_MonApp_Amp/d2FacMonAmp),
     .            (D_MonApp_Amp/i2FacMonAmp),
     .            (D_MonApp_Amp/a2FacMonAmp)

            print*,quals(k),' MAX MIN SLPIT ',
     .           (D_TargetS/(D_TargetT))*
     .           (D_AnnApp_Amp),
     .           (D_TargetD/(D_TargetT))*
     .           (D_AnnApp_Amp),
     ...
     .         / D_tAGWO
            ...
            if ( C_SCALE_S2 .eq. 'A' ) then
            !{
             print*,'C_SCALE_S2 = ',C_SCALE_S2
             do i = 1,I_nMonths
             !{
              if(s2FacAnnAmp .ne. 0 )
     .         s2Factors(i) = s2Factors(i)*
     .         (D_TargetS/(D_TargetT))*
     .         ((D_AnnApp_Amp)/(s2FacAnnAmp))
     .         / D_tSURO
              if(d2FacAnnAmp .ne. 0 )
     .         d2Factors(i) = d2Factors(i)*
     .         (D_TargetD/(D_TargetT))*
     .         ((D_AnnApp_Amp)/(d2FacAnnAmp))
     .         / D_tSEDM
              if(i2FacAnnAmp .ne. 0 )
     .         i2Factors(i) = i2Factors(i)*
     .         (D_TargetI/(D_TargetT))*
     .         ((D_AnnApp_Amp)/(i2FacAnnAmp))
     .         / D_tIFWO
              if(a2FacAnnAmp .ne. 0 )
     .         a2Factors(i) = a2Factors(i)*
     .         (D_TargetA/(D_TargetT))*
     .         ((D_AnnApp_Amp)/(a2FacAnnAmp))
     .         / D_tAGWO
             !}
            ...
            !}
            else if ( C_SCALE_S2 .eq. 'M' ) then
            !{
             print*,'C_SCALE_S2 = ',C_SCALE_S2
             do i = 1,I_nMonths
             !{
c              print*,'before',s2Factors(i)
              if(s2FacMonAmp.ne.0.and.D_TargetT.ne.0.and.D_tSURO.ne.0)
     .         s2Factors(i) = s2Factors(i)*
     .         (D_TargetS/(D_TargetT))*
     .         ((D_MonApp_Amp)/(s2FacMonAmp))
     .         / D_tSURO
c              print*,'after',s2Factors(i),D_TargetS,D_TargetT,
c     .         D_MonApp_Amp,s2FacMonAmp,D_tSURO
              if(d2FacMonAmp.ne.0.and.D_TargetT.ne.0.and.D_tSEDM.ne.0)
     .         d2Factors(i) = d2Factors(i)*
     .         (D_TargetD/(D_TargetT))*
     .         ((D_MonApp_Amp)/(d2FacMonAmp))
     .         / D_tSEDM
              if(i2FacMonAmp.ne.0.and.D_TargetT.ne.0.and.D_tIFWO.ne.0)
     .         i2Factors(i) = i2Factors(i)*
     .         (D_TargetI/(D_TargetT))*
     .         ((D_MonApp_Amp)/(i2FacMonAmp))
     .         / D_tIFWO
              if(a2FacMonAmp.ne.0.and.D_TargetT.ne.0.and.D_tAGWO.ne.0)
     .         a2Factors(i) = a2Factors(i)*
     .         (D_TargetA/(D_TargetT))*
     .         ((D_MonApp_Amp)/(a2FacMonAmp))
     .         / D_tAGWO
            ...
            ! *** Adjust S2 to Average Concentration
            s2FactorsAvg = 0.0D0
            d2FactorsAvg = 0.0D0
            i2FactorsAvg = 0.0D0
            a2FactorsAvg = 0.0D0
            do i = 1,I_nMonths
               s2FactorsAvg = s2FactorsAvg + s2Factors(i)/I_nMonths
               d2FactorsAvg = d2FactorsAvg + d2Factors(i)/I_nMonths
               i2FactorsAvg = i2FactorsAvg + i2Factors(i)/I_nMonths
               a2FactorsAvg = a2FactorsAvg + a2Factors(i)/I_nMonths
            end do
            do i = 1,I_nMonths
c               if ( s2FactorsAvg .gt. 0)
c     .          s2Factors(i) = D_TargetSc + (s2Factors(i)-s2FactorsAvg)
c               if ( d2FactorsAvg .gt. 0)
c     .          d2Factors(i) = D_TargetDc + (d2Factors(i)-d2FactorsAvg)
c               if ( i2FactorsAvg .gt. 0)
c     .          i2Factors(i) = D_TargetIc + (i2Factors(i)-i2FactorsAvg)
c               if ( a2FactorsAvg .gt. 0)
c     .          a2Factors(i) = D_TargetAc + (a2Factors(i)-a2FactorsAvg)
               s2Factors(i) = D_TargetSc + (s2Factors(i)-s2FactorsAvg)
               d2Factors(i) = D_TargetDc + (d2Factors(i)-d2FactorsAvg)
               i2Factors(i) = D_TargetIc + (i2Factors(i)-i2FactorsAvg)
               a2Factors(i) = D_TargetAc + (a2Factors(i)-a2FactorsAvg)
            end do

            ! *** If L_S2MIN_ZERO is false then find the minimum s2factor
            ! *** And then shift the curve to set positive dynamic equilibrium
            if ( L_S2MIN_ZERO .eqv. .false. ) then
               s2FactorMin = 0.0
               d2FactorMin = 0.0
               i2FactorMin = 0.0
               a2FactorMin = 0.0
...
c            D_bTarget(l,2) = 24.2846
            ...
            print*,'\nSTARGET ',lseg(:lenlseg),' ',luname(l),' ',
     .                   quals(k),D_sTarget(l,k),
     .             '\nBTARGET ',lseg(:lenlseg),' ',luname(l),' ',
     .                   quals(k),D_bTarget(l,k)

            !>> Compute Scalaing Factor TARGET / YIELD
            if ( (D_tSCON + D_tDCON + D_tICON).eq.0 ) then
               D_SURFfactor = 0
            else
               D_SURFfactor = D_sTarget(l,k) * (I_YearC2-I_YearC1+1)
     .                  / (D_tSCON + D_tDCON + D_tICON)
            end if
            if (D_tACON .eq. 0) then
               D_BASEfactor = 0.0D0
            else
               D_BASEfactor = D_bTarget(l,k) * (I_YearC2-I_YearC1+1)
     .                  / (D_tACON)
            end if

            if ( I_DEBUG0 .eq. 1 )
     .       write(*,'(/A F12.5)') 'Factor S = ',D_SURFfactor
            if ( I_DEBUG0 .eq. 1 )
     .       write(*,'(A F12.5)' ) 'Factor B = ',D_BASEfactor

            if ( D_tSCON .eq. 0 ) D_Sfactor = 0
            if ( D_tSCON .ne. 0 )
     .         D_Sfactor = D_TargetS * (I_YearC2-I_YearC1+1) / D_tSCON

            if ( D_tDCON .eq. 0 ) D_Dfactor = 0
            if ( D_tDCON .ne. 0 ) 
     .         D_Dfactor = D_TargetD * (I_YearC2-I_YearC1+1) / D_tDCON

            if ( D_tICON .eq. 0 ) D_Ifactor = 0
            if ( D_tICON .ne. 0 )
     .         D_Ifactor = D_TargetI * (I_YearC2-I_YearC1+1) / D_tICON

            if ( D_tACON .eq. 0 ) D_Afactor = 0
            if ( D_tACON .ne. 0 )
     .         D_Afactor = D_TargetA * (I_YearC2-I_YearC1+1) / D_tACON

            if ( I_DEBUG0 .eq. 1 )
     .       write(*,'(/A A A A A A F12.5 F12.5 F12.5 F12.5)') 
     .        'Factors SDIA ',lseg(:lenlseg),' ',luname(l),' ',quals(k),
     .        D_Sfactor,D_Dfactor,D_Ifactor,D_Afactor
            ...
            do yy = I_Year1,I_Year2
               do mm = 1,12
                  do i=1,ndaysinmonth(yy,mm)*24
c                     D_hSCON(j) = D_hSCON(j) * D_SURFfactor
c                     D_hDCON(j) = D_hDCON(j) * D_SURFfactor
c                     D_hICON(j) = D_hICON(j) * D_SURFfactor
c                     D_hACON(j) = D_hACON(j) * D_BASEfactor
                     D_hSCON(j) = D_hSCON(j) * D_Sfactor
                     D_hDCON(j) = D_hDCON(j) * D_Dfactor
                     D_hICON(j) = D_hICON(j) * D_Ifactor
                     D_hACON(j) = D_hACON(j) * D_Afactor
                     if( yy.ge.I_YearC1.and.yy.le.I_YearC2 ) then
