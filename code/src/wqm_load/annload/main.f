...
************************************************************************
     ...
     .             'SEDM'/
      ...
      character*50 LBfile(maxTimeBreaks)
...
*************** WDM related variables
      ...
      real sedmvals(ndaymax*24)
      double precision annflow(EarliestYear:LatestYear)
      double precision annsedm(EarliestYear:LatestYear)
...
********** create WDM variable from loads
     ...
     .    l2r(ns)(:lenlseg)
        do year = year1, year2
          ...
          SLONload(year) = ORGN(year) * fLORN
          SRONload(year) = ORGN(year) * (1-fLORN)
          if(debug0)print*,'... ... ... SLON ',year,SLONload(year)
          SNH3load(year) = NH3N(year)
          DPO4load(year) = PO4P(year) * fPIPX
          SPO4load(year) = PO4P(year) * (1-fPIPX)
          SLOPload(year) = ORGP(year) * fLORP
          SROPload(year) = ORGP(year) * (1-fLORP)
          SEDMload(year) = SEDM(year)
          if(debug0)print*,'... ... ... SEDM ',year,SEDMload(year)
        end do
...
********** get annual flow
        ...
        do i = 1,nvals
          ...
          annsedm(oldyear) = annsedm(oldyear) + sedmvals(i)
          if (hour.eq.24) then
            hour = 0
          ...
          if (year.ne.oldyear .and.oldyear.le.year2) then
            ...
            SRONcon(oldyear)= SRONload(oldyear)/annflow(oldyear)
            SNH3con(oldyear)= SNH3load(oldyear)/annflow(oldyear)
c            DPO4con(oldyear)= DPO4load(oldyear)/annsedm(oldyear)
            DPO4con(oldyear)= DPO4load(oldyear)/annflow(oldyear)
            SPO4con(oldyear)= SPO4load(oldyear)/annflow(oldyear)
            SLOPcon(oldyear)= SLOPload(oldyear)/annflow(oldyear)
            SROPcon(oldyear)= SROPload(oldyear)/annflow(oldyear)
...
********** get hourly loads
        ...
        do i = 1,nvals
          ...
          hSRON(i) = SRONcon(oldyear)*flowvals(i)
          hSNH3(i) = SNH3con(oldyear)*flowvals(i)
          hDPO4(i) = DPO4con(oldyear)*flowvals(i)
          hSPO4(i) = SPO4con(oldyear)*flowvals(i)
          hSLOP(i) = SLOPcon(oldyear)*flowvals(i)
          hSROP(i) = SROPcon(oldyear)*flowvals(i)
