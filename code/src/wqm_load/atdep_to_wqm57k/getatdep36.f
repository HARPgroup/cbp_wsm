...
**   of 0.75 over the water until we get the 12km grid, which can     **
**   resolve the bay and also give deposition to different land uses  **
**                                                                    **
...
************ make dry variables into monthly fractions
      totaldryno3 = 0.0
      totaldrynh3 = 0.0
      do nm = 1,12
        totaldryno3 = totaldryno3 + mondryno3(nm)
        totaldrynh3 = totaldrynh3 + mondrynh3(nm)
      end do
      do nm = 1,12
        mondryno3(nm) = mondryno3(nm) / totaldryno3
        mondrynh3(nm) = mondrynh3(nm) / totaldrynh3
      end do
...
************* find total wet and dry deposition
      ...
      totaldryno3 = totalwetno3 * 0.75
      totaldrynh3 = totalwetnh3 * 0.75
...
********** find slope in change in lbs wet per year
      ...
      do year = 1,nyears
        y(year) = annwetno3(year1+year-1)/annprecip(year1+year-1)
      end do
...
********* convert slope to fraction change per year 
      ...
      avey = avey / real(nyears)
      no3slope = no3slope / avey

********* convert slope to lb change in dry per year
      no3slope = no3slope * totaldryno3/real(nyears)

********* repeat for ammonia
      avey = 0.0
      do year = 1,nyears
        y(year) = annwetnh3(year1+year-1)/annprecip(year1+year-1)
        avey = avey + y(year)
      end do
      avey = avey / real(nyears)
      nh3slope = slope(years,y,nyears,vsize,err)
      nh3slope = nh3slope / avey
      nh3slope = nh3slope * totaldrynh3/real(nyears)

*********** calculate intercepts
********** dryno3 = int + slope * year
********* sum both sides for time domain and solve for int
******** int = ( totalDry - slope*sum(years) ) / nyears
      sumyears = 0.0
      do year = year1,year2
        sumyears = sumyears + real(year)
      end do

      no3intercept = (totaldryno3 - no3slope*sumyears)/real(nyears)

      nh3intercept = (totaldrynh3 - nh3slope*sumyears)/real(nyears)

      print*,totalwetno3,totaldryno3,no3slope,no3intercept
      print*,totalwetnh3,totaldrynh3,nh3slope,nh3intercept
...
******** monthly averages are preserved.
      ...
      day = day1
      do nd = 1,ndays
        dryno3load(nd) = ( no3intercept + real(year)*no3slope )
     .                 * mondryno3(month)/real(ndaysinmonth(year,month))
        drynh3load(nd) = ( nh3intercept + real(year)*nh3slope )
     .                 * mondrynh3(month)/real(ndaysinmonth(year,month))
        call tomorrow(year,month,day)
      end do
