...
******************** get the wetfall data
     ...
     .                year,month,day)
        if (nd.ge.1.and.nd.le.ndays) then
          dailyrain(nd) = Pnldas  ! in inches
          wetnh3load(nd) = DNH3nldas * 10000 ! in kg/m2/day
          wetno3load(nd) = DNO3nldas * 10000 ! in kg/m2/day
        end if
      end do
111   close (dfile)
...
*************** store in wq variable
      ...
      do year = year1,year2
        do day = 1,ndaysinyear(year)
          cellwq(day,year,kprec) = dailyrain(nd)
          cellwq(day,year,kwno3) = wetno3load(nd)
          cellwq(day,year,kdno3) = dryno3load(nd)
          cellwq(day,year,kwnh4) = wetnh3load(nd)
          cellwq(day,year,kdnh4) = drynh3load(nd)
          nd = nd + 1
        end do
      end do
