...
C      parameter (ncells=1)
      ...
      real pctWNOX,pctWNHX,pctDNOX,pctDNHX,pctORGN,pctORGP,pctPO4X

************ END DECLARATIONS ******************************************
      read*,atdepscen,DYEAR,linkdir,
     .      CMAQbase,CbaseDir2,CbaseSuperLong,
     .      pctWNOX,pctWNHX,pctDNOX,pctDNHX,pctORGN,pctORGP,pctPO4X
      ...
      do ny = year1,year2
         fCWNOX(ny) = fCWNOX(ny) * pctWNOX
         fCWNHX(ny) = fCWNHX(ny) * pctWNHX
         fLDNOX(ny) = fLDNOX(ny) * pctDNOX
         fLDNHX(ny) = fLDNHX(ny) * pctDNHX
         fLORGN(ny) = fLORGN(ny) * pctORGN
         fLORGP(ny) = fLORGP(ny) * pctORGP
         fLPO4X(ny) = fLPO4X(ny) * pctPO4X
