...
******** read in all necessary data
      ...
      call getflowcalib(rscen,lenrscen,fnam,err)
      if (err.ne.0) go to 990
...
************* write out rivers
      if (singleseg) then
        fnam='seglists/'//rsegs(rproc(1))//'.riv'
      else
        call lencl(segORbasin,lenbasin)
        fnam='seglists/'//segORbasin(:lenbasin)//'.riv'
      end if
...
*********** write out calibration sites
      if (singleseg) then
        fnam='seglists/'//rsegs(rproc(1))//'.calib'
      else
        call lencl(segORbasin,lenbasin)
        fnam='seglists/'//segORbasin(:lenbasin)//'.calib'
      end if
      open (dfile,file=fnam,status='unknown',iostat=err)
...
********** write out total area
      if (singleseg) then
        fnam='seglists/'//rsegs(rproc(1))//'.land'
      else
        call lencl(segORbasin,lenbasin)
        fnam='seglists/'//segORbasin(:lenbasin)//'.land'
      end if
      open (dfile,file=fnam,status='unknown',iostat=err)
      if (err.ne.0) go to 991
...
************************************************************************
      subroutine getcalibsites(fnam,err)
      include 'basingen.inc'
      integer tempid

C      print*,'getting calibration sites'
      fnam = '/model/temp/calibistes.csv'
