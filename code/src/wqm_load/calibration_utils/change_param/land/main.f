...
************ SPECIFICATION ENTRY SECTION
     ...
     O                            paramscen)
      ...
      if (table(1:3).eq.'MON') then  ! monthly data
      print*,'monthly data '
      print*,' '
      variable = 'JAN'     ! used to find the starting column of monthly data
      else
     ...
     .          action.ne.'m'.and.action.ne.'M')
        print*,'enter action [m,a,e] = [Multiply,Add,Equals]'
        read*,action
      end do

      print*,'enter value for action, enter ',char(39),'-9',char(39),
     .       ' if values are stored in a file'
      if (typeflag.eq.'i'.or.typeflag.eq.'I') then
        read*,ivalue
        if (ivalue.eq.-9) then  ! unique value
          call readvalues(typeflag,paramscen,variable,maxlsegs,
     I                    nlsegs,lsegs,
     O                    rvalues,ivalues)
        else
     ...
     O                    rvalues,ivalues)
      ...
      if (table(1:3).eq.'MON') then
        do while (err.ne.0)
        print 1234,'enter month flag [a,s,m] = ',
     .             '[All,Single,Multiple]'
        read*,monflag
        call procmonflag(lscen,monflag,nmonth,monthidx,err)
        end do
      end if
