...
*************************************************************************
      ...
      subroutine rating_OBcensor(rscen,rseg,year1,year2,pltfil)
...
*********** get observed daily flow  
      obfnam=calibdir//'observed/TSSX/'//rseg(:lenrseg)//
     .                     '.INFLOW'
...
************** partition quick flow and base flow
     ...
     O                   nasdays,ndsdays,asmatch,dsmatch)
       ...
       obldfnam=calibdir//'observed/TSSX'//
     .           '/'//rseg(:lenrseg)//'.OTSSX'
...
************* read observed concentration and instantous flow
        ...
        do
          read(ifl,*,end=222) ny2,nm2,nd2,nh2,nmin2,hconc,hflow
           ...
           if (ny2.ge.year1 .and. ny2.le.year2) then
              ...
              obfl(ny2,nm2,nd2,nh2,nobshour(ny2,nm2,nd2,nh2)) = hflow
           end if              
        end do
           ...
           do nh = 1, 24
             if (nobshour(obyear(n),obmonth(n),obday(n),nh).gt.0) then
              do no = 1,nobshour(obyear(n),obmonth(n),obday(n),nh)
               tflow = obfl(obyear(n),obmonth(n),obday(n),nh,no)
               tconc = obcon(obyear(n),obmonth(n),obday(n),nh,no)
              
               if (tflow.gt.0.0001 .and. tconc .gt. 0.0001) then  ! both inst.flow and concentration exist at the time  
                naspts = naspts + 1
                obasfl(naspts) = tflow
