...
************ Scour Organics
      real scrorn(year1:year2),scrorp(year1:year2)
...
**********            OPEN FILE
      fnam = outdir//'hspf/river/out/'//rscen(:lenrscen)//
     .       '/'//rseg//'_scr.out'

      open(dfile+1,file=fnam,status='old',iostat=err)
      if (err.ne.0) go to 991

      read(dfile+1,'(a140)',err=994,end=111) outline ! header
      do
      !{
        read(dfile+1,*,err=994,end=110) ny,d1,d2
...
************ TN Balance
     ...
     .       average(scrorn,years,years,err)
      tntf = tnout / tnin
...
************ TP Balance
     ...
     .       average(scrorp,years,years,err)
      tptf = tpout / tpin
