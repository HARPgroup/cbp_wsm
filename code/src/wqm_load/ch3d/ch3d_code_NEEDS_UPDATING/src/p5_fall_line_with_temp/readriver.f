...
************************************************************************
      subroutine readriver(rscen,trseg,stupidWDMthing,
...
************ READY TO OPEN WDM FILE
     ...
     .          trseg(:lenrseg)//'.wdm'
      call wdbopn(wdmlnd+stupidWDMthing,wdmfnam,1,err)     ! open land read only
      if (err .ne. 0) go to 998

      do Rvar = 1,nRvarOut  ! loop over river variables

        if (RnameOut(Rvar).ne.'WATR' .and.
     .      RnameOut(Rvar).ne.'HEAT') cycle
        call gethourdsn(wdmlnd+stupidWDMthing,sdate,edate,
     I                  RdsnOut(Rvar),
     O                  nvals,hval)                   
      ...
      call wdflcl (wdmlnd+stupidWDMthing,err)
      if (err.ne.0) call wdflc1(wdmlnd+stupidWDMthing,err)
        ...
        if (RnameOut(Rvar).eq.'HEAT') then
          ...
          do ny = year1,year2  ! populate pairq
            ...
            do jday = j1,j2
              do hour = 1,24
                nv = nv + 1
                pairH(jday,ny) = pairH(jday,ny) + hvaltemp(nv,Rvar)
              end do
            end do
          end do
          do ny = year1+1,year2  ! go back and get days -5,0
