...
*********** output load variables
      ...
      double precision Twq(maxcells,maxBvar)
      real denom
      integer nd,ny,nq
...
********** read in loads
      do ny = year1,year2
        ...
        do nd = 1,ndaysinyear(ny)
          if (day.eq.1) then  ! if 1st day, get new values
            iday = nd
            do nc = 1,ncells
              read(dfile,1234)
     .              icell,iyear,imonth,(Twq(nc,nq),nq=1,nBvar)
              if (icell.ne.cell(nc).or.iyear.ne.year.or.
     .            imonth.ne.month) go to 993
            end do
          end if
          do nc = 1,ncells
            do nq = 1,nBvar
              wq(nc,nq) = wq(nc,nq) + Twq(nc,nq)
            end do
          end do
...
********* write out segment-wise totals
     ...
     .       '/average_out_wqm57_ps_by_cell_'//cy1//'_'//cy2//'.csv'
...
993   report(1) = 'problem reading wqm intput file, wrong order'
      report(2) = fnam
      write(report(3),*) icell,iyear,imonth,cell(nc),year,month
      go to 999
