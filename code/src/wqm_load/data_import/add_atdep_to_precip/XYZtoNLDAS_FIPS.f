...
C      read(*,'(a)') seg
      read*,segSRC,segDST,sdate(1),edate(1),
     .       WriteDaily,WriteMonthly,WriteAnnual
      print*,segSRC,'->',segDST,' ',sdate(1),' ',edate(1)
...
C      print*,sdate,',',edate,',',idays,',',(dlywetno3(id),id=1,idays)
     ...
     .                     'drynh3(lb),wetorn(lb),wetorp(lb),wetpo4(lb)'
       ...
       id=sdate(3)
       do nd=1,idays
         write(dfile,1235,err=951)segDST,iy,im,id,dprc_nldas(nd),
     ...
     .                     'drynh3(lb),wetorn(lb),wetorp(lb),wetpo4(lb)'
       do ny=sdate(1),edate(1)
         do nm=1,12
           write(dfile,1236,err=951)segDST,ny,nm,mprc_nldas(ny,nm),
     ...
     .                     'drynh3(lb),wetorn(lb),wetorp(lb),wetpo4(lb)'
       do ny=sdate(1),edate(1)
           write(dfile,1234,err=951)segDST,ny,aprc_nldas(ny),
...
972   report(1) = 'problem with file: '
      report(2) = fnam
      write(report(3),*)'factors for lseg ',segDST,' do not add up to 1'
      go to 999

977   report(1) = 'programming problem: too many cells for seg '//segDST
