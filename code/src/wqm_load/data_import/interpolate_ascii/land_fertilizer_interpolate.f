...
**  change a test logic to consider all reasonable values
        do nm = 1,12
          if (value1(nm).lt.0 .and.
     .      value2(nm).ge.0) then
            outval(nm) = value2(nm)
          else if (value2(nm).lt.0 .and.
     .      value1(nm).ge.0) then
            outval(nm) = value1(nm)
          else if (value1(nm).ge.0 .and.
     .      value2(nm).ge.0) then
            outval(nm) = interpolate(ro,ry1,value1(nm),ry2,value2(nm))
            outval(nm) = max(outval,0.0)
          else 
            outval(nm) = -9
          end if
        end do
