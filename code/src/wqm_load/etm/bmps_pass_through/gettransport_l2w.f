...
**************END DECLARATION ******************************************
      ...
      do i = 1, maxL2R             ! initialize Trandata
        do k = 1,nBmpCon
          do l = 1,nlu
            trandataL2W(i,k,l) = 1.0
          end do
        end do
      end do

      call trims(tranfile,lastran)              ! open transport file
      fnam = pardir//'transport/'//tranfile(:lastran)//'_l2w.csv'
      ...
      do while (ifound.lt.numsegs)  ! file until all land segs are found
        ...
        if (Trseg.eq.rseg) then
          ...
          do kl = 1,ntranps                            ! now that we have the land segment, read across
            ...
            call fread(dline,x)
            trandataL2W(nl2r,order(kl,1),order(kl,2)) = x
          end do
        end if
      end do
      ...
      if (.not.foundBOD) then
        ...
        if (found) then   ! inherit N or P or average
          ...
          if (foundTN.and.foundTP) then ! take average
            do i = 1, maxL2R           
              do l = 1,nlu
                trandataL2W(i,nBOD,l) = (trandataL2W(i,nTN,l)
     .                             +  trandataL2W(i,nTP,l)) / 2.0
              end do
            end do
            ...
            do i = 1, maxL2R             
              do l = 1,nlu
                trandataL2W(i,nBOD,l) = trandataL2W(i,nTP,l)
              end do
            end do
          else
            go to 989
