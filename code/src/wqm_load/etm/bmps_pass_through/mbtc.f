      ...
      character*50 tranfile                     ! transport factor file
      real trandataL2W(maxL2R,maxBMPcon,nlu)            ! transport factors
      real trandataS2R(maxL2R,maxBMPcon,nlu)
      ...
      integer BBJday(maxTimeBreaks)             ! bmp passthru eff julian day
      character*40 BBfile(maxTimeBreaks)        ! bmp passthru eff files

      integer PBJday(MaxTimeBreaks)             ! pound bmp jday
      character*40 PBfile(MaxTimeBreaks)        ! pound bmp files

      integer PBEOTJday(MaxTimeBreaks)             ! pound bmp EOT jday
      character*40 PBEOTfile(MaxTimeBreaks)        ! pound bmp EOT files
...
*************** Acres for each BMP
      ...
      real BMPPassThru(maxBMPcon,MaxTimeBreaks,MaxL2R,nlu)
...
********** of pounds rather than by a percentage
      ...
      real PoundsReducedEOT(MaxTimeBreaks,maxBMPcon,MaxL2R,nlu)
...
********  hitil = (hom, hwm, nho, nhi )
      ...
      logical writeout
      parameter (writeout=.false.)
c      parameter (writeout=.true.)
      logical debug
