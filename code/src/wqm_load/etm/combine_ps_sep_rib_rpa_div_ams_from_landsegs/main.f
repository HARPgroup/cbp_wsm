...
************************************************************************
      ...
      character*25 PsScen,SepScen,RIBScen,RpaScen,DivScen
      integer lenPsScen,lenSepScen,lenRIBScen,lenRpaScen,lenDivScen
...
************** variables for getdsns
      ...
      character*4 RIBvarname(maxvars)
      ...
      integer RIBdsns(maxvars)
      integer Rpadsns(maxvars)
      integer Divdsns(maxvars)        ! Diversion Dsns
      integer Amsdsns(maxvars)        ! atmos sediment dsns
      integer nPsdsns,nSepdsns,nRIBdsns,nRpadsns,nDivdsns,nAmsdsns  ! number of dsns for each data type
...
********* LOOP OVER SEGMENTS and EXTRACT SEPTIC VARIABLES
     ...
     .            '/'//sep(:lensep)//'_'//l2r(i)(:lenlseg)//'_to_'//
     .            rseg(:lenrseg)//'.wdm'
        call wdbopnlong(wdmlnd,wdmfnam,1,err)     ! open land-to-river 
...
*** START RIB
********* FIRST GET LAND SEGMENTS IN THIS RIVER FOR RIB
      call getl2r(rseg,rscen,lenrscen,                 !read from ./geo/rscen/land_water_area.csv
     O            numsegs,l2r)

********* LOOP OVER SEGMENTS and EXTRACT RIB VARIABLES
      print*,' '
      call ttyput(' RIB: ')
      print*,' '
      call ttyput('  Variables: ')
      do n = 1,nRIBdsns
        call ttyput(RIBvarname(n))
        call ttyput(' ')
      end do
      print*,' '
      call ttyput('  Land segments: ')
      do n = 1,nRIBdsns              ! initialize temp variable
        do nv = 1,ndaymax
          dvaltemp(nv,n) = 0.
        end do
      end do
     ...
     .            '/'//rib(:lenrib)//'_'//l2r(i)(:lenlseg)//'_to_'//
     .            rseg(:lenrseg)//'.wdm'
        call wdbopnlong(wdmlnd,wdmfnam,1,err)     ! open land-to-river
        if (err .ne. 0) go to 998             ! RIB wdm read only

        do n = 1,nRIBdsns
          print*,RIBdsns(n)
          call getdailydsn(wdmlnd,sdate,edate,RIBdsns(n),
     O                            nvals,dval)
          print*,nvals
          do nv = 1,nvals
            dvaltemp(nv,n) = dvaltemp(nv,n) + dval(nv)
          end do
        end do            ! end loop over RIB variables
...
************* STORE IN NEW WDM
      do n = 1,nRIBdsns         ! store in wdm
        do nv = 1,nvals
          dval(nv) = dvaltemp(nv,n)
        end do
        call putdailydsn(wdmrch,sdate,edate,RIBdsns(n),nvals,dval)
      end do
*************** END OF RIB SECTION
...
********* FIRST GET LAND SEGMENTS IN THIS RIVER FOR RPA
C      call getl2r(rseg,rscen,lenrscen,                 !read from ./geo/rscen/land_water_area.csv
C     O            numsegs,l2r)

********* LOOP OVER SEGMENTS and EXTRACT RPA VARIABLES
      print*,' '
     ...
     .            '/'//rpa(:lenrpa)//'_'//l2r(i)(:lenlseg)//'_to_'//
     .            rseg(:lenrseg)//'.wdm'
        call wdbopnlong(wdmlnd,wdmfnam,1,err)     ! open land-to-river 
        if (err .ne. 0) go to 998             ! rpa wdm read only
     ...
     O                            nvals,dval)   
        ...
        end do            ! end loop over rpa variables
