...
************* HANDLING BED/BANK & FLOODPLAIN: START ***********
     ...
     .                            lrfloodplain(ns,icstr)*(eYRbb-sYRbb+1)
        ...
        do Rvar = 1,nRvar
          do icstr = 1,ncstr
            if ( Rname(Rvar) .eq. ncstrname(icstr) ) iPARA(icstr) = Rvar
          end do
        end do
        allgood = .true.
        do icstr = 1,ncstr
           if ( iPARA(icstr) .eq. -9 ) allgood = .false.
        end do
        if ( allgood ) then
           do icstr = 1,ncstr
              do nv = 1,nvals
                 hPARA(nv,icstr) = hvaltemp(nv,iPARA(icstr))
c                 rPARA(icstr)    = rPARA(icstr) + hPARA(nv,icstr)
              end do
              year = reqSRTy
              month = reqSRTm
              day = reqSRTd
              print*,year,month,day,jdaytot
              nv = 0
              do jday = 1,jdaytot
               do hour = 1,24
                nv = nv + 1
                if(year.ge.sYRbb .and. year.le.eYRbb) then
                 rPARAbb(icstr)    = rPARAbb(icstr) + hPARA(nv,icstr)
                end if
                if(year.ge.sYRfl .and. year.le.eYRfl) then
                 rPARAfl(icstr)    = rPARAfl(icstr) + hPARA(nv,icstr)
                end if
               end do
               call tomorrow(year,month,day)
              end do
              facPARAbb(icstr) = rbedbank(icstr)    / rPARAbb(icstr)
              facPARAfl(icstr) = rfloodplain(icstr) / rPARAfl(icstr)
              print*,'BB:',ncstrname(icstr),
     .                rbedbank(icstr),rPARAbb(icstr),facPARAbb(icstr)
              print*,'FL:',ncstrname(icstr),
     .                rfloodplain(icstr),rPARAfl(icstr),facPARAfl(icstr)
     ...
     .                 facPARAbb(icstr) = facPARAbb(icstr) * adjfac
                 if( facPARAfl(icstr) .lt. 0 )
     .                 facPARAfl(icstr) = facPARAfl(icstr) * adjfac
              end if

              print*,'fac = ',(1 + facPARAbb(icstr) + facPARAfl(icstr) )
              do nv = 1,nvals
                 hvaltemp(nv,iPARA(icstr)) = hPARA(nv,icstr) * 
     .                  (1 + facPARAbb(icstr) + facPARAfl(icstr) )
              end do
              do nv = 1,nvals
                 hval(nv) = hPARA(nv,icstr) * facPARAbb(icstr)
                 ! TODO
                 ! call puthourdsn
              end do
              do nv = 1,nvals
                 hval(nv) = hPARA(nv,icstr) * facPARAfl(icstr)
                 ! TODO
                 ! call puthourdsn
              end do
           end do
        end if
************* HANDLING BED/BANK & FLOODPLAIN: FINISH ***********
