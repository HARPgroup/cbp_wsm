...
C
CCBAY  the following 20 lines are modified. These perform a mass-based
CCBAY   isotherm rather than a concentration-based isotherm.
CCBAY   Isotherms are generally concentration-based.  The mass-based 
CCBAY   methodology was creating very high concentrations when the 
CCBAY   of water in the layer was very low - Gary Shenk 5/9/2008
CCBAY
CCBAY  we want the isotherm to be K = Pconc/Sconc where 
CCBAY   Pconc = P / SOILM and Sconc = S / MOISTM
CCBAY   rearranging and avoiding divide by zero:
CCBAY   S = T(MOISTM/SOILM) / (K + MOISTM/SOILM)
CCBAY
C     labile
      TLON= PLON+ SLON
...
C       infinite paritition coefficient for labile
        SLON= 0.0
      ELSE IF (KLON .LT. 1.0E-5) THEN
C       zero coefficient, all particulate
        SLON = TLON
      ELSE
C       partition labile
CCBAY        SLON= TLON/(KLON+ 1.0)
        SLON = (TLON * (MOISTM/SOILM)) / ( KLON + (MOISTM/SOILM))
      END IF
      PLON= TLON- SLON
C
C     refractory
...
C       infinite paritition coefficient for refractory
        SRON= 0.0
      ELSE IF (KRON .LT. 1.0E-5) THEN
C       zero coefficient, all particulate
        SRON = TRON
      ELSE
C       partition refractory
CCBAY        SRON= TRON/(KRON+ 1.0)
        SRON = (TRON * (MOISTM/SOILM)) / ( KRON + (MOISTM/SOILM))
      END IF
      PRON= TRON- SRON
CCBAY   end modifications for concentration isotherm
C
      IF (ALPNFG .EQ. 1) THEN
