...
C               see if current operation fits in the given range
                IPOS= SPBPOS(XPOS)
CGshenk if this catch is not in place, variables will get clobbered
                if (IPOS.gt.MXSPBF) then
                  write(*,*) ' '
                  write(*,*) 'too many special actions'
                  write(*,*) 'raise the parameter MXSPBF in specact.f'
                  write(*,*) 'and recompile'
                  stop
                end if
CGshenk the above should be sent to the message file to stop the program
CGshenk  but I do not know how to do that
