...
C            if (modules(nm).eq.'PHOS') doR = .true.
          ...
          do nm = 1,nmod
            if (modules(nm).eq.'PWATER') doI = .true.
          end do
     ...
     .          doL.or.doT.or.doU.or.doR.or.doQ.or.doI)) return
...
********** OPEN THE SECTION
      ...
      if (doI) call Iuvquan(perlnd,lscen,lenlscen)
     ...
     I                      species,nspecies)
      ...
      if (doI) 
     .  call Iaction(lscen,lenlscen,lseg,lenlseg,clu,year1,year2,fPar)
