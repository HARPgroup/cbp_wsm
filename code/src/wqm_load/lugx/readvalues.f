...
************************************************************************
** GET BIOSOLIDS  APPLICATION -                                       **
** programming strategy   -  for each breakpoint (n in nB), open the  **
**   file and read lines to find the land segment. If the segment and **
**   land use match read-ins, then populate the variable 'biosolids'  **
**   for each species                                                 **
**  if no data found in this file, leave at 'missing' value           **
************************************************************************
      subroutine readbiosolids(lseg,lenlseg,clu,nB,Mfile,biosolids,
     I                      species,nspecies)
      implicit none
      include 'lug.inc'
      include 'acts.inc'
      ...
      integer n,nB,month
      real biosolids(maxTimeBreaks,nspecies,12)
...
********** open land percent cover file
        fnam =ScenDatDir//'land/biosolids/biosolids_'//Mfile(n)(:last)//
     .          '.csv'
        open (dfile,file=fnam,status='old',iostat=err)
        if (err.ne.0) go to 991
        ...
        do  
          ...
          if (Tlseg .eq. lseg) then            ! get land segment
            ...
            if (Tlu.eq.clu) then            ! get land use
              ...
              do month = 1,12     ! we have land seg, read across
                call shift(dline)    
                call fread(dline,biosolids(n,sp,month))
              end do
            end if
          end if
...
** programming strategy   -  for each breakpoint (n in nB), open the  **
**   file and read lines to find the land segment. If the segment and **
**   land use match read-ins, then populate the variable 'wep'        **
**   for each species                                                 **
**  if no data found in this file, leave at 'missing' value           **
************************************************************************
      subroutine readwep(lseg,lenlseg,clu,nB,Mfile,wep,
     I                      species,nspecies)
      implicit none
      include 'lug.inc'
      include 'acts.inc'
      ...
      integer n,nB,month
      real wep(maxTimeBreaks,nspecies,12)
...
********** open land percent cover file
        fnam =ScenDatDir//'land/wep/wep_'//Mfile(n)(:last)//
     .          '.csv'
        open (dfile,file=fnam,status='old',iostat=err)
        if (err.ne.0) go to 991
        ...
        do
          ...
          if (Tlseg .eq. lseg) then            ! get land segment
            ...
            if (Tlu.eq.clu) then            ! get land use
              ...
              do month = 1,12     ! we have land seg, read across
                call shift(dline)
                call fread(dline,wep(n,sp,month))
              end do
            end if
          end if
