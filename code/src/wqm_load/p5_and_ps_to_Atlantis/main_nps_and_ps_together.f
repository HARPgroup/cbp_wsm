...
********** END OF SETUP, OPEN AND READ LRSEG LINK FILE
     ...
     .       '/request/p5_to_BRUSH_lrsegs.csv'
...
******************** got all data, now write to file
      do ny = year1,year2
        ...
        fnam = 'BRUSH_output.YY'           ! open file
        write(fnam(12:15),'(i4)') ny
        fnam(12:13) = 't.' 
        fnam = outdir//'wqm_input/'//rscen(:lenrscen)//'/request'//fnam
        open(dfile,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
        print*,'writing file ',fnam(:50)

        write(dfile,'(a14,12(a1,a4))')'year,month,day',
     .               (',',Bname(nq),nq=1,nBvar)
        year = ny
        month = 1
        day = 1
...
********* write out annual totals
      ...
      write(cy2,'(i4)') year2
      fnam = outdir//'wqm_input/'//rscen(:lenrscen)//
     .       '/request/annual_out_BRUSH_'//cy1//'_'//cy2//'.csv'
      open(dfile,file=fnam,status='unknown',iostat=err)
      if (err.ne.0) go to 991
      print*,'writing file ',fnam(:60)

      write(dfile,'(a4,12(a1,a4))')'year',(',',Bname(nq),nq=1,nBvar)
      do ny = year1,year2
        write(dfile,'(i4,12(a1,e10.3))')
...
********* write out annual totals by cell
      fnam = outdir//'wqm_input/'//rscen(:lenrscen)//
     .       '/request/annual_out_BRUSH_'//cy1//'_'//cy2//'_by_cell.csv'
      open(dfile,file=fnam,status='unknown',iostat=err)
      if (err.ne.0) go to 991
      print*,'writing file ',fnam(:60)

      write(dfile,'(a9,12(a1,a4))')
     .        'year,cell',(',',Bname(nq),nq=1,nBvar)
      do nc = 1,ncells
        do ny = year1,year2
          write(dfile,'(i4,a1,a10,12(a1,e10.3))')
