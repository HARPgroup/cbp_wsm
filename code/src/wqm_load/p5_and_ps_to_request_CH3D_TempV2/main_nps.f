...
************ END DECLARATIONS ******************************************
      read*,rscen,hotstart,Request,year1,year2 ! get river scenario
********* stupid wdm thing
...
************** POPULATE variables relating river vars to bay vars
      rbfnam = 'river_to_wqm57kNPS_TEMP'
      call riverbay(
...
********** END OF SETUP, OPEN AND READ LRSEG LINK FILE
     ...
     .       '/request/p532_'//Request(:lenRequest)//'_lrsegs.csv'
...
********** OPEN AND READ RSEG LINK FILE
      fnam = catdir//'geo/'//geoscen(:lengeoscen)//
     .       '/request/p532_'//Request(:lenRequest)//'_rsegs.csv'
...
*********************** loop over cells in Lrseg
        call readpairY1Y2(rscen,Tlseg,Trseg,
     I                ioscen,lenioscen,
...
C BHATT          write(hotfile)wq,AnnualCell,AnnualTotal,lrseg(nlrseg)
           write(hotfile)((((wq(I1,I2,I3,I4),I1=1,366),
     .                            I2=year1,year2),
     .                            I3=1,maxcells),
     .                            I4=1,maxBvar),
     .      (((AnnualCell(I1,I2,I3),I1=year1,year2),
     .                            I2=1,maxcells),
     .                            I3=1,maxBvar),
     .      ((AnnualTotal(I1,I2),I1=year1,year2),
     .                            I2=1,maxBvar),
     .      lrseg(nlrseg)
          close(hotfile)
        end if
...
*********** read just the river output
     ...
     O                 nRvarOut,RdsnOut,RnameOut)
        ...
        print*,'VARS= ',(RdsnOut(nd),nd=1,nRvarOut),',','<>',
     .          (RnameOut(nd),nd=1,nRvarOut),nRvarOut
        call readriverY1Y2(
     I                 rscen,Trseg,
     I                 year1,month1,day1,year2,month2,day2,
     I                 nBvar,Bname,nRv2Bv,Rname2Bv,Rfactor,
     ...
     O                 pairwq) ! get the data
        print*,"nBvar= ",nBvar
        print*,"Var=1 ", Bname(1)," Year=2005"
        print*,(pairwq(nd,2005,1),nd=1,366)
        print*,"Var=1 ", Bname(1)," Year=2006"
        print*,(pairwq(nd,2006,1),nd=1,366)
        print*,"Var=2 ", Bname(2)," Year=2005"
        print*,(pairwq(nd,2005,2),nd=1,366)
        print*,"Var=2 ", Bname(2)," Year=2006"
        print*,(pairwq(nd,2006,2),nd=1,366)
...
*********** Make Concentrations from Loads for some variables
      ...
      do nq = 1,nBvar  
        ...
        print*,nq,'$',Divnq,'$',Bname(Divnq),'$',DivBvar(nq),'$',nBvar

        do nc = 1,ncells  ! divide
          do ny = year1,year2
            do nd = 1,ndaysinyear(ny)
              if (wq(nd,ny,nc,Divnq).gt.0.000001) then
                print*,wq(nd,ny,nc,nq),',',wq(nd,ny,nc,Divnq)
...
c cell(nc)  imon(ic(j)),jmon(ic(j)),
               ...
               write(78,303) 134,282,wq(nd,ny-1,9,nq)
          end do
          ! ONE YEAR OF DATA FOR THE YEAR2
          do nd = 1,ndaysinyear(ny)
