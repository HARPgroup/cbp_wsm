...
c      parameter (Y1=1984, Y2=2001)
c      parameter (Y1=1995, Y2=2014)
      parameter (Y1=ISYEAR, Y2=IEYEAR)
      integer I1, I2, I3, I4
...
************* utility variables
      ...
      integer lakeflag,timestep  ! variables to send to 
...
************** PS only variables
      ...
      data header2 /'units                     '/
************ END DECLARATIONS ******************************************
      read*,rscen,hotstart,Request,psmethod,year1,year2,doCH3D

      if (.not.(psmethod.eq.'hcell'.or.psmethod.eq.'wcell'.or.
     .          psmethod.eq.'lrseg')) go to 993
      if ( psmethod .eq. 'lrseg' ) KEY = 'L'
      if ( psmethod .eq. 'hcell' ) KEY = 'H'
      if ( psmethod .eq. 'wcell' ) KEY = 'W'
...
********** END OF SETUP, OPEN AND READ LRSEG LINK FILE
     ...
     .       '/request/p600_'//Request(:lenRequest)//'_lrsegs_PS.csv'
...
c          print*,ccell
     ...
     .                              + pairwq(nd,ny,nq)
c                AnnualTotal(ny,nq) = AnnualTotal(ny,nq)+pairwq(nd,ny,nq)
c                AnnualCell(ny,ncells,nq) = AnnualCell(ny,ncells,nq)
c     .                               + pairwq(nd,ny,nq)
              end do
            end do
          end do
...
********** write out DAILY DATA
      ...
      do ny = year1,year2
        ...
        fnam(13:14) = 's.'
        fnam = outdir//'wqm_input/'//Request(:lenRequest)//'_'//KEY//
     .     '/'//rscen(:lenrscen)//'/'//fnam
...
990   report(1) = 'no wqm cell data found for point source'
      report(2) = psmethod
      report(3) = psscen(:lenpsscen)
