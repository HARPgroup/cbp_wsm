************************************************************************
**  program to generate the input file for the 13k Bay model          **
...
********** END OF SETUP, OPEN FILE AND LOOP OVER CELLS
      fnam = catdir//'geo/'//geoscen(:lengeoscen)//'/p5_to_13k_wqm.prn'
...
*********************** loop over Land-river pairs in cell
     ...
     .                '/bay_models/ps_ch3d13k_'//ccell(:lencell)//'.wdm'
          else if (psmethod.eq.'wcell') then
            wdmfnam = ScenDatDir//'river/ps/'//psscen(:lenpsscen)//
     .                '/bay_models/ps_wqm13k_'//ccell(:lencell)//'.wdm'
          end if
          call readdailydat(
     I                      year1,month1,day1,year2,month2,day2,
     I                      nBvar,Bname,nRv2Bv,Rname2Bv,Rfactor,
...
********** get subset of cells to write
      do nc = 1,ncells
        writecell(nc) = .false.
        do ny = year1,year2
          do nd = 1,ndaysinyear(ny)
            do nq = 1,nBvar
              if (wq(nd,ny,nc,nq).gt.eps) then
                writecell(nc) = .true.
                exit
              end if
            end do
            if (writecell(nc)) exit
          end do
          if (writecell(nc)) exit
        end do
      end do
...
********** write out subset
      do ny = year1,year2
        ...
        fnam = 'wsm13k_wsm_ps.YY'           ! open file
...
********* write out annual totals
     ...
     .       '/annual_out_wqm13_ps_'//cy1//'_'//cy2//'.csv'
      open(dfile,file=fnam,status='unknown',iostat=err)
      if (err.ne.0) go to 991
