...
************* utility variables
      ...
      character*4000 bigline  ! file reading variable
...
********* stupid wdm thing
      ...
      if (err .ne. 0) go to 990             ! and never close
      ...
      read*,rscen,hotstart    ! get river scenario
...
********** END OF SETUP, OPEN FILE AND LOOP OVER CELLS
     ...
     .           '/p5_to_boynton.prn'
      ...
      read(dfile-1,'(a4000)') bigline    ! get rid of header lines

      read(dfile-1,'(a4000)',err = 992,end=992) bigline    
...
******************** got all data, now write to file
      do nc = 1,ncells
        ...
        fnam = outdir//'wqm_input/'//rscen(:lenrscen)//'/'//
     .         'boynton_no_ps_'//cell(nc)(:lencell)//'.csv'           ! open file

        open(dfile,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 990
        print*,'writing file ',fnam(:50)

        write(dfile,'(a)')'year,month,flow,nh4,no23,tn,dip,tp,tss'
