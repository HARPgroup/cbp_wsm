...
****************  daily load
      ...
      character*25 pradscen, psscen, sepscen, ribscen, rpascen
...
********** START TO GREP DATA, LOOP OVER LAND SEGMENTS
      ...
      do ns = 1,numsegs
        ...
        do Rvar = 1,nRvar    ! initialize
          do nv = 1,ndaymax
            dWWTPrvar(nv,Rvar)  = 0.0
            dCSOrvar(nv,Rvar)   = 0.0
            dINDrvar(nv,Rvar)   = 0.0
            dATDEPrvar(nv,Rvar) = 0.0
            dSEPrvar(nv,Rvar)   = 0.0
            dRIBrvar(nv,Rvar)   = 0.0
            dRPArvar(nv,Rvar)   = 0.0
          end do
        end do
...
************ INDUSTRIAL DATA
          call ttyput(' Point Source, indus')
          wdmfnam = ScenDatDir//'river/ps/'//psscen(:lenpsscen)//'/'//
...
***************** output section
     ...
     .                        year1,year2)
          ...
          call writeDATaveannEOR(rscen,l2r(ns),rseg,outfil,sdate,
     .                        dops,wateracres,wwtp,
     .                        ndays,nloads,loadname,dWWTPload,
     .                        year1,year2)
          call writeDATaveannEOR(rscen,l2r(ns),rseg,outfil,sdate,
