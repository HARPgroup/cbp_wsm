      ...
      do while (line(:3).ne.'end')
        ...
        if (.not.comment(line)) then
          ...
          if (line(:23).eq.'PRECIP ATMOS DEPOSITION') then
            ...
            read(dfile,'(a100)',err=991)line
            call d2x(line,last)
            if (line(:13).ne.'END RIB LOADS') go to 995
            ...
            read(dfile,'(a100)',err=991)line
            call d2x(line,last)
            if (line(:13).ne.'END RPA LOADS') go to 996
...
995   report(1) = ' ERROR in control file '
      report(2) = fnam
      report(3)=' only allowed one row in table RIB LOAD'
      go to 999

996   report(1) = ' ERROR in control file '
      report(2) = fnam
      report(3)=' only allowed one row in table RPA LOAD'
      go to 999
