...
*********** if pourpoint is a black hole, set to zero and quit
      ...
      print *, 'CalcResDFave.f A : DONE!'
************* get the lakeflags for all segments
      call readcontrol_Rparamscen(
     I                           rscen,lenrscen,
     O                           paramscen)
      print *, 'CalcResDFave.f : readcontrol_Rparamscen DONE!'
      call lencl(paramscen,lenparamscen)
      print *, 'CalcResDFave.f : lencl DONE!'
      print *, rsegs, paramscen
C BHATT ADDED ONE LINE TEMPORARILY nrsegs = nrsegs - 1 DELETE
C      nrsegs = nrsegs - 1
      call getallrflags(
     I                  paramscen,lenparamscen,rsegs,nrsegs,
     O                  Nexitss,lakeflags,resflags,timesteps)

      print *, 'CalcResDFave.f : getallrflags DONE!'
...
******** find list of upstream segments
        print *, 'CalcResDFave.f : FindResBasin A DONE!'
        call FindResBasin(
     I                    respoint,dsid,uniqid,lakeflags,nrsegs,
     O                    allup,nallup,resup,nresup)
        print *, 'CalcResDFave.f : FindResBasin B DONE!'
