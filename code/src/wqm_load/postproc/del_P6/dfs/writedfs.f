...
************************************************************************
     ...
     .             resDFname(ndelmax)
      integer ndl,ny,nm,nm1,nm2,numsegs,ns
     ...
     O                numsegs,l2r)
      ...
      do ns = 1,numsegs  ! write a file for each lrseg
        fnam = outdir//'del/dfs/monthly/'//rscen(:lenrscen)//'/'//
     .         l2r(ns)//'_'//rseg(:lenrseg)//'_'//
     .         rscen(:lenrscen)//'.txt'
        open(11,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
...
************************************************************************
     ...
     O                numsegs,l2r)
      ...
      do ns = 1,numsegs  ! write a file for each lrseg
        ...
        fnam = outdir//'del/dfs/annual/'//rscen(:lenrscen)//'/'//l2r(ns)
     .         //'_'//rseg(:lenrseg)//'_'//rscen(:lenrscen)//'.txt'
        open(11,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
...
************************************************************************
     ...
     O                numsegs,l2r)
      ...
      do ns = 1,numsegs
        fnam = outdir//'del/dfs/aveann/'//rscen(:lenrscen)//'/'//l2r(ns)
     .         //'_'//rseg//'_'//rscen(:lenrscen)//
     .         '_'//cy1//'_'//cy2//'.txt'
        open(11,file=fnam,status='unknown',iostat=err)
        if (err.ne.0) go to 991
