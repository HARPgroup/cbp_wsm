...
************************************************************************
      subroutine hourly(rscen,rseg,wdmfil,Nexits)
...
************* END DECLARATIONS
      ...
      print*,'Hourly River Load ',rscen(:lenrscen),' ',rseg
...
*****************loada
     ...
     .           unit(np)
          ...
          hour = 0
          dload = 0.
cbhatt          oldmonth = asdate(2)
cbhatt          oldyear = asdate(1)
          do i = 1,nvals
            hour = hour + 1
cbhatt            dload = dload + loadvals(i)
cbhatt            if (asdate(2).ne.oldmonth) then
              write(pltfil,1234,err=951) 
     .                loadname(np),asdate(1),asdate(2),asdate(3),hour,
     .                loadvals(i)
cbhatt              dload = 0.
cbhatt              oldyear = asdate(1)
cbhatt              oldmonth = asdate(2)
