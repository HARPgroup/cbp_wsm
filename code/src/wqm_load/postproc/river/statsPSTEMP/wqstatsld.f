...
************************************************************************
      subroutine wqstatsld(
     I                   ob,si,siq,nday2,ndaymax,
...
******************* END DECLARATIONS ***********************************
      ...
      do nd = 1,nday2     ! get rid of missing values
        if (nd.gt.ndays) exit
        if (obs(nd).lt.-8.0.or.sim(nd).lt.-8.0.or.simq(nd).lt.-8.0) then
          do while (obs(ndays).lt.-8.0.or.sim(ndays).lt.-8.0
     .                                .or.simq(ndays).lt.-8.0)
            ndays = ndays - 1
            if (ndays.lt.nd) exit
          end do
          if (ndays.lt.nd) exit
...
*********** Calculate basic statistics on untransformed variables
      ...
      rq= correl(obs,simq,ndays,ndaymax,err)
      if (err.ne.0) r = -9
...
************* write statistics to output file
      ...
      write(pltfil,400,err=951) 'correl q      ,',rq
