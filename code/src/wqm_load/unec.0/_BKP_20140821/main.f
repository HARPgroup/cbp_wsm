...
117         close (dfile)
     ...
     .          luname(i)//'_'//lseg(:lenlseg)//'.'//quals(j)
               ...
               !{
                  ...
                  call d2x(longline,last)
                  read(longline,*,end=995,err=995)
     .               I_tYear,I_tMonth,(R_Meof(k),k=1,nstates)
                  if (I_tYear.ge.I_YearC1.and.I_tYear.le.I_YearC2) then
                     do k = 1,nstates
                        if ( states(k).ne.'A' ) then
                        !{
                           sTarget(i,j) = sTarget(i,j) + R_Meof(k)
                        !}
                        else
                        !{
                           bTarget(i,j) = bTarget(i,j) + R_Meof(k)
                        !}
                        end if
                     end do
                  end if
...
c            bTarget(l,2) = 24.2846
     ...
     .              m,R_MonApp(m),mSCON(m),mDCON(m),mICON(m),mACON(m)
                  write(dfile,1235) 
     .             yy,mm,mSCON(m),mDCON(m),mICON(m),mACON(m)
                  m = m + 1
               end do
            end do
            close(dfile)
...
1235  format(I4,',',I2,',',e15.8,',',e15.8,',',e15.8,',',e15.8)
