...
c      integer            I_aParamNormalize
      ...
      read*,lseg,lscen,I_Year1,I_Year2,I_YearC1,I_YearC2,I_WDM,I_DEBUG0
...
c      D_fracA    = 0.01
      ...
      C25_UNEC        = lscen(:lenlscen) ! TODO Add a parameter table in control file and initialize
      C25_SENSITIVITY = lscen(:lenlscen) ! TODO Add a parameter table in control file and initialize

      call lencl(C25_UNEC,lC25_UNEC)
...
c         if ( luname(i).ne.'for' .and. luname(i).ne.'hom' ) cycle
         ...
         do j=1,I_NQUALS
         !{
            fnam   =pardir//'unec/'//C25_UNEC(:lC25_UNEC)//'/unec_'//
     .              luname(i)//'_'//quals(j)//'.csv'
            open(dfile,file=fnam,status='old',iostat=err)
            if (err.ne.0) then
               fnam=pardir//'unec/'//C25_UNEC(:lC25_UNEC)//'/unec_'//
     .              'default'//'_'//quals(j)//'.csv'
               open(dfile,file=fnam,status='old',iostat=err)
            end if
...
c      call lencl(lscen,lenlscen)
     ...
     O                          geoscen)
      ...
      fnam = catdir//'iovars/'//geoscen(:lengeoscen)//'/perlnd' ! TODO Replace geoscen with iovars parameter
...
112         close (dfile)
     ...
     . print*,'FLOWDSNs ',dsnSURO,dsnSEDM,dsnIFWO,dsnAGWO
      ...
      fnam = pardir//'unec/'//C25_UNEC(:lC25_UNEC)//
...
114   close(dfile)
     ...
     .    print*,(quals_to_appspecies(i,k),k=1,I_NAPPSPECIES)
      ...
      !>> get rSAS switches
      fnam = pardir//'unec/'//C25_UNEC(:lC25_UNEC)//'/rSAS_quals.dat'
      open(dfile,file=fnam,status='old',iostat=err)
      if (err.ne.0) go to 991
...
c            fnam=calibdir//'target/g533/'//'S'//quals(i)//'_target.csv' TODO
            fnam=calibdir//'target/del/'//'S'//quals(i)//'_target.csv' ! TODO Fix me
            if ( I_DEBUG0 .eq. 1 )
     .       print*,'STARGETS ',fnam
            L_found = .false.
            open(dfile,file=fnam,status='old',iostat=err)
...
c            fnam=calibdir//'target/g533/'//'B'//quals(i)//'_target.csv' TODO 
            fnam=calibdir//'target/del/'//'B'//quals(i)//'_target.csv' ! TODO Fix me
            if ( I_DEBUG0 .eq. 1 )
     .       print*,'BTARGETS ',fnam
            L_found = .false.
            open(dfile,file=fnam,status='old',iostat=err)
...
c         if ( luname(l).ne.'hom' ) cycle ! TODO DELETE
