...
************************************************************************
      subroutine readpair_mb(rscen,Tlseg,Trseg,
     I                    ioscen,lenioscen,
...
Ctest      double precision acc(maxRvar)
      ...
      character*13 Trseg
      character*6 Tlseg
      real pairwq(366,reqSRTy:reqENDy,maxBvar)
...
*********** end klugy section
      ...
      do l = 1,nlu   ! loop over land uses
        ...
        if (.not.dothislu(nl2r,l)) then
             print*,'not doing: ', luname(l)
             cycle
        end if
