...
********* for clay - CTAUCS          
     ...
     .             bias(ns,nc,2,itnum)+bias(ns,nc,3,itnum))/3.)
          Ofactor = Ofactor + weight(nr,ns,nc)*((bias(ns,nc,1,itnum-1)+
     .              bias(ns,nc,2,itnum-1)+bias(ns,nc,3,itnum-1))/3.)    
C          factor = factor + (weight(nr,ns,nc)*avebias(ns,nc,itnum))
C          Ofactor = Ofactor + (weight(nr,ns,nc)*avebias(ns,nc,itnum-1))
        end do
     ...
     I                 parmin(np),parmax(np),
     M                 parval(np,nr))
C        write(99,*)'CW ',parval(kCW,nr),' ',rsegs(nr)
C        write(99,*)'SW ',parval(kSW,nr),' ',rsegs(nr)
...
C     .             + bias(ns,nc,3,itnum))/3.0 
     ...
     .             / 3.0
        ...
        Ofactor = 0.0
        do ns = 1,ncsegs
          if (goodobs(ns,ncORGN).gt. 0.5*goodobs(ns,ncTN)) then    ! if observed ORGN data points > half of TN points
...
********* store calculated sensitivity for each parm at each iteration
        ...
        fnam = tree//'output/river/summary/'//paramscen(:lenparamscen)  
