...
!           do iyh = 1, iyhr     ! individual year 
            ...
            enddo
c             N10 = int(N10 / 10)   ! for flash 10 
c             N10 = int(0.1*float(N10))   ! for flash 10 
              N10 = int(0.3*float(N10))   ! for flash 30 
            diff = (sumU - sumB)
            call QSORTR(ord, nvals, raindyB)   ! only for base 
             ...
             do iyh=1, nvals
                ...
                if(iyh .le. N10) then   ! upper 30% from the bottom, first 
                 sumN10 = sumN10 + TraindyB(iyh)
                else                    ! others      
                endif
             enddo
!               now for upper 30%    ! change with a factor
                ratio = 1. + diff / sumN10    !!!!!!
               do iyh = 1, N10
