...
******** variables for lowest 10% and 5% bias - 5% added by ICPRB
      real low10obs,low10sim,low10bias,low05obs,low05sim,low05bias
...
********* calculate biase for low 10% of observed data
      ...
      call qsortr(ord2,ndays,sim) ! added by ICPRB
      low10obs = 0.0
      low10sim = 0.0
      do nd = 1,ndays/10 ! ICPRB modified range to start at 5%
        low10obs = low10obs + obs(ord(nd))
        low10sim = low10sim + sim(ord(nd)) ! commented out by ICPRB
C        low10sim = low10sim + sim(ord2(nd)) ! added by ICPRB
      end do
...
********* calculate biase for low 5% of observed data - added by ICPRB
      low05obs = 0.0
      low05sim = 0.0
      do nd = ndays/20,ndays/10
        low05obs = low05obs + obs(ord(nd))
C        low05sim = low05sim + sim(ord2(nd)) ! unpaired
        low05sim = low05sim + sim(ord(nd))  ! paired
      end do
...
C	 ratio(n) = (allqtplus1(n)-allqt(n))/allqt(n) ! added by ICPRB
      end do
