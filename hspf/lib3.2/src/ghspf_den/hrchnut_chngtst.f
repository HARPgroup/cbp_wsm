...
C         consider denitrification processes, and compute associated fluxes
      write(*,*)'bdenit 0: ,umaxhr,zdpth,Tzden,vol,volsp,vwater,no3,d50'
      write(*,*)'bdenit0:',umaxhr,zdpth,Tzden,vol,volsp,vwater,no3,dnt50
! pw  write(*,*)'bdenit2:,dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css,wtw'
!     write(*,*)'bdenit2:',dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css,
!    .     wtno3wi
! pw      CALL DENIT
!    I                (KNO320,TCDEN,TW,DOX,DENOXT,
!    M                 NO3,VOL, dddnit) 
!    o               vol, dddnit)
          call denit (tw, no3, vol, dddnit) 
      write(*,*)'adenit 4: ,umaxhr,zdpth,Tzden,vol,volsp,vwater,no3,d50'
      write(*,*)'adenit4:',umaxhr,zdpth,Tzden,vol,volsp,vwater,no3,dnt50
!    O                 NO3DE)
C
!         DENNO3= -NO3DE*VOLSP
          DENNO3 = - dddnit
...
!    M                    NO3,voltt,ddnit)      ! adding volt, bedwidd
       subroutine denit (tw,no3, voltt,ddnit) 
!    0                 voltt, ddnit)
!    O                    DENNO3)
C
C     + + + PURPOSE + + +
...
C     + + + END SPECIFICATIONS + + +
C
      write(*,*)'denit 1:umaxhr,zdpth,Tzden,vol,delt,vwater,no3,d50,vt'
      write(*,*)'denit1:',umaxhr,zdpth,Tzden,vol,delt60,vwater,no3,dnt50
     ,  , voltt
!     write(*,*)'denit2: ,dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css,vsw'
!     write(*,*)'denit2:',dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css
...
!!! note fctdv2 = 1/zdpth, 
        ...
        ddnit = 0.0
          dvvt  =0.000001*delt60*  dvvt *(TzDEN**(TW*20.))
          umaxt =0.000001*delt60*  umaxt*(TzDEN**(TW-20.))  ! red fro segmen
!            goto 221
         if(cno3w .gt. cno3s) then
          dw2s = dvvt * (cno3w-cno3s)/fctdv2  ! diffus rate:water 2 soil
          dw2s = dw2s*bedarea*porop       ! * area * time = distance or mass into
         else            ! kg / time step ->      
          dw2s = 0.
         endif
          dw2s = min(wtno3w*0.9, dw2s)
         dw2s = 0.000000000
          wtno3w = wtno3w - dw2s
          ddnit = umaxt* cno3s / (cno3s + dnt50)  !kg/m2/depth
          ddnit = ddnit * bedarea * porop * zdpth
          ddnit = min(wtno3s*0.9, ddnit)
         ddnit = 0.000000000
          wtno3s = wtno3s + dw2s - ddnit   ! update kg? 
!         cno3s  = wtno3s * 1000. / (bedarea*porop*zdpth) ! update, g/m3 = mg/l
          cno3s  = wtno3s *1.0000 / vsoilw                ! update, g/m3 = mg/l
!         NO3 = wtno3w / (VOL/delt60*0.0283168)  ! not times 43560. )
!????     NO3 = wtno3w / ((VOL/delt60)*43560.*0.0283168)  ! not times 43560. )
          ddnit = ddnit         * 2.2045 !      kg -> pound  
221      continue 
         goto 222
...
C
!     END IF
      write(*,*)'denit 2:umaxhr,zdpth,Tzden,vol,delt,vwater,no3,d50,vt'
      write(*,*)'denit2:',umaxhr,zdpth,Tzden,vol,delt60,vwater,no3,dnt50
     ,  , voltt
!     write(*,*)'denit2: ,dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css,vsw'
!     write(*,*)'denit2:',dvvhr,ength,bedwidd,bedarea,cwwi,cssi,css
