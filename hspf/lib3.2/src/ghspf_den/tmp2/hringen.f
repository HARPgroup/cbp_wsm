...
C
     ...
     O                     RESMFG,RUNFG,EMFG,RNINFO)
C
C     + + + PURPOSE + + +
C     process the global block
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER      MSGFL,MESSU,ECOUNT,SEDAT(10),SDATIM(5),EDATIM(5),
     $             OUTLEV,SPOUT,RESMFG,RUNFG,RUNMIN,EMFG
      CHARACTER*80 RNINFO
C
C     + + + ARGUMENT DEFINITIONS + + +
C     MESSU  - ftn unit no. to be used for printout of messages
...
C     + + + EXTERNALS + + +
      ...
      EXTERNAL  PMXTFT,PMXTFI,PMXTFC,ZIPI,GETSE,HDMES2
C
...
C     + + + INPUT FORMATS + + +
 ...
 1040 FORMAT (25X,2I5)
 1050 FORMAT (9X,I5,5X,I5,31X,I5)
C
C     + + + END SPECIFICATIONS + + +
C
      SCLU= 211
...
C       ascertain functions to be performed in this run
     ...
     M               KEY,
     O               UCIBF)
        READ (UCIBF,1050,ERR=60)  RESMFG,RUNFG,UNIT
          GO TO 70
 60     CONTINUE
...
C         no message here for compat
        END IF
C
C       echo run flag and units flag
        IF (MESSU .NE. 0) THEN
          SGRP= RUNFG+ 117
          CALL PMXTFT (MSGFL,MESSU,SCLU,SGRP)
          SGRP= EMFG+ 133
...
C     + + + LOCAL VARIABLES + + +
     ...
     $             TABNO,ERRINT,
     $             SCLU,SGRP,INITFG,CLEN,CONT,KTYP,KEYST,KEYND
      CHARACTER*4  CEND
...
C     + + + EQUIVALENCES + + +
      EQUIVALENCE (CHSTR,CHSTR1),(UCIBF,UCIBF1)
      CHARACTER*1  CHSTR1(16),UCIBF1(80)
C
C     + + + FUNCTIONS + + +
...
C           error - cannot read time step of run
     ...
     M                 ECOUNT)
            ...
            HRMIN(2)= 0
 70       CONTINUE
C         have a keyword? - test each of 3 possible fields
...
C
C     + + + LOCAL VARIABLES + + +
      INTEGER      KEY,SCLU,SGRP,I,KEYST,KEYND,I1,I0,ITYP,ERRFLG,ICH,I2
      CHARACTER*2  CATID
      CHARACTER*16 LCATNM
      CHARACTER*22 TBUFF
      CHARACTER*80 UCIBF
...
C     + + + LOCAL VARIABLES + + +
     ...
     #             CLEN,KEY,INITFG,CONT,I4,LEN,ITYP
      CHARACTER*1  BLANK
      CHARACTER*64 CHSTR
      CHARACTER*80 UCIBUF
C
