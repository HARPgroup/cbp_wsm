...
C
C     + + + LOCAL VARIABLES + + +
      INTEGER   I,I1,I1440,LAST,TBNO,TBSB,NVAL,
...
C
C     + + + EXTERNALS + + +
      EXTERNAL  DAYMNH,ITABLE,HSCKFL,OMSTI,OMSG
C
C     + + + INTRINSICS + + +
      INTRINSIC   MIN0,MOD
C
...
C
      IF (SNOWFG .EQ. 1 .AND. PFLAG(2) .LT. 6) THEN
C       section snow is active & producing printout
        CALL SNOACC
     I             (FRMROW,TOROW)
      END IF
C
      IF (PWATFG .EQ. 1 .AND. PFLAG(3) .LT. 6) THEN
C       section pwater is active and producing printout
        CALL PWAACC
     I             (FRMROW,TOROW)
      END IF
C
      IF (SEDFG .EQ. 1 .AND. PFLAG(4) .LT. 6) THEN
C       section sedmnt is active and producing printout
        CALL SDACC
     I            (FRMROW,TOROW)
      END IF
C
C     section PSTEMP has no printout fluxes
C
      IF (PWGFG .EQ. 1 .AND. PFLAG(6) .LT. 6) THEN
C       section pwtgas is active and producing printout
        CALL PWGACC
     I             (FRMROW,TOROW)
      END IF
C
      IF (PQALFG .EQ. 1 .AND. PFLAG(7) .LT. 6) THEN
C       section pqual is active & producing printout
        CALL PQACC
     I            (FRMROW,TOROW)
      END IF
C
      IF (AGFG .EQ. 1) THEN
C       section mstlay has no printout fluxes
C
        IF (PESTFG .EQ. 1 .AND. PFLAG(9) .LT. 6) THEN
C         section pest is active and producing printout
          CALL PSTACC
     I               (FRMROW,TOROW)
        END IF
C
        IF (NITRFG .EQ. 1 .AND. PFLAG(10) .LT. 6) THEN
C         section nitr is active and producing printout
          CALL NITACC
     I               (FRMROW,TOROW)
        END IF
C
        IF (PHOSFG .EQ. 1 .AND. PFLAG(11) .LT. 6) THEN
C         section phos is active and producing printout
          CALL PHOACC
     I               (FRMROW,TOROW)
        END IF
C
        IF (TRACFG .EQ. 1 .AND. PFLAG(12) .LT. 6) THEN
C         section tracer is active and producing printout
          CALL TRACC
     I              (FRMROW,TOROW)
        END IF
      END IF
...
C     + + + END SPECIFICATIONS + + +
C
      IF (AIRTFG .EQ. 1 .AND. PFLAG(1) .LE. LEV) THEN
C       section atemp is active and produces printout at this level
        CALL AIRPRT
     I             (UNITFG,PRINTU)
      END IF
C
      IF (SNOWFG .EQ. 1 .AND. PFLAG(2) .LE. LEV) THEN
C       section snow is active and produces printout at this level
        CALL SNOPRT
     I             (UNITFG,LEV,PRINTU)
      END IF
C
      IF (PWATFG .EQ. 1 .AND. PFLAG(3) .LE. LEV) THEN
C       section pwater is active and produces printout at this level
        CALL PWAPRT
     I             (UNITFG,LEV,PRINTU)
      END IF
C
      IF (SEDFG .EQ. 1 .AND. PFLAG(4) .LE. LEV) THEN
C       section sedmnt is active and produces printout at this level
        CALL SDPRT
     I             (UNITFG,LEV,PRINTU)
      END IF
C
      IF (PSTFG .EQ. 1 .AND. PFLAG(5) .LE. LEV) THEN
C       section pstemp is active and produces printout at this level
        CALL PTPRT
     I             (UNITFG,PRINTU)
      END IF
C
      IF (PWGFG .EQ. 1 .AND. PFLAG(6) .LE. LEV) THEN
C       section pwtgas is active and produces printout at this level
        CALL PWGPRT
     I             (UNITFG,LEV,PRINTU)
      END IF
C
      IF (PQALFG .EQ. 1 .AND. PFLAG(7) .LE. LEV) THEN
C       section pqual is active and produces printout at this level
        CALL PQPRT
     I             (UNITFG,LEV,PRINTU)
      END IF
C
...
C       get correct values for identifying printout units
        ...
        END IF
C
        IF (MSTLFG .EQ. 1 .AND. PFLAG(8) .LE. LEV) THEN
C         section mstlay is active and produces printout at this level
          CALL MSTPRT
     I               (PRINTU,AGMAID,MFACTA,MFACTB)
        END IF
C
        IF (PESTFG .EQ. 1 .AND. PFLAG(9) .LE. LEV) THEN
C         section pest is active and produces printout at this level
          CALL PESPRT
     I               (LEV,PRINTU,AGMAID,MFACTA,MFACTB,UNITFG)
        END IF
C
        IF (NITRFG .EQ. 1 .AND. PFLAG(10) .LE. LEV) THEN
C         section nitr is active and produces printout at this level
          CALL NITPRT
     I               (LEV,PRINTU,AGMAID,MFACTA,MFACTB,UNITFG)
        END IF
C
        IF (PHOSFG .EQ. 1 .AND. PFLAG(11) .LE. LEV) THEN
C         section phos is active and produces printout at this level
          CALL PHOPRT
     I               (LEV,PRINTU,AGMAID,MFACTA,MFACTB,UNITFG)
        END IF
C
        IF (TRACFG .EQ. 1 .AND. PFLAG(12) .LE. LEV) THEN
C         section tracer is active and produces printout at this level
          CALL TRAPRT
     I               (LEV,PRINTU,AGMAID,MFACTA,MFACTB,UNITFG)
        END IF
      END IF
...
C
          IF (PIVLNO .EQ. PIVL .AND. PERPFG .EQ. 2) THEN
