...
C
     ...
     I                   (UUNITS,
     O                    SOILM,SOILD)
       common/pwsoil/pwsoild,fieldcp,pwwiltpt  ! ,pwsoilfc  ! pw add 
C
C     + + + PURPOSE + + +
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     .         , pwsoild(4), fieldcp(4), pwsoilfc(4),pwwiltpt(4) ! pw 11/27/06
c    x         , pwsoild(4), fieldcp(4), pwsoilfc(4),pwwilpt(4) ! pw err 
C
C     + + + ARGUMENT DEFINITIONS + + +
C     UUNITS - system of units   1-english, 2-metric
...
c ! pw adding
c       pwsoilm(i) = soild(i)*RVAL(I+4)*FACT 
        pwsoild(i) = RVAL(i)
c        pwsoilfc(i) = 0.1*160./RVAL(i+4)
c ! pw end added
 30   CONTINUE
C
...
C
      SUBROUTINE   STORGE
     I                   (MESSU,OUTLEV,UUNITS,NBLKS,NBLKSI,NUM,
     I                    NVALS,HEADG,NUMI,NVALSI,HEADGI,
     M                    SUB,SUBI,
     O                    SMAT,UMAT,IMAT,SMATB,UMATB,
     O                    IMATB,LMAT,AMAT,TMAT,TOTMAT)
C
C     + + + PURPOSE + + +
C     Process input of the initial storages of a material in the soil
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER      MESSU,NUM,NUMI,
     $             NBLKS,NVALS,NVALSI,OUTLEV,SUB,SUBI,UUNITS
      REAL         AMAT(NVALS),IMAT(NVALSI),IMATB(NVALSI,NBLKS),
     $             LMAT(NVALS),NBLKSI,SMAT(NVALS),SMATB(NVALS,NBLKS),
     $             TMAT(NVALS),TOTMAT,UMAT(NVALS),UMATB(NVALS,NBLKS)
...
C     + + + OUTPUT FORMATS + + +
 2000 FORMAT (/,' SEGMENT-WIDE STORAGES IN SURFACE, UPPER, AND ',
     $            'INTERFLOW LAYERS')
 2010 FORMAT (/,' STORAGES IN SURFACE, UPPER, AND INTERFLOW ',
     $            'LAYERS FOR BLOCK',I3)
...
C
     ...
     M                 IMATB(1,N))
C
C         add to segment-wide values
          DO 30 J= 1, NVALS
            SMAT(J)= SMAT(J)+ (SMATB(J,N)*NBLKSI)
            UMAT(J)= UMAT(J)+ (UMATB(J,N)*NBLKSI)
 30       CONTINUE
C
          DO 35 J= 1, NVALSI
            IMAT(J)= IMAT(J)+ (IMATB(J,N)*NBLKSI)
 35       CONTINUE
 40     CONTINUE
C
        IF (OUTLEV.GT.2) THEN
C         summary info
          WRITE (MESSU,2020)  AGUNIT(UUNITS)
          WRITE (MESSU,2030)  HEADG
          WRITE (MESSU,2040)  SMAT
C
          WRITE (MESSU,2030)  HEADG
          WRITE (MESSU,2040)  UMAT
C
          WRITE (MESSU,2030)  HEADGI
          WRITE (MESSU,2040)  IMAT
        END IF
      END IF
C
      IF (OUTLEV.GT.2) THEN
...
C
      SUBROUTINE   AGRGET
     I                   (SEDFG,NBLKS,MSTLFG,PSTFG,TMPFG,IVL1,SOSDFP,
...
C     + + + DUMMY ARGUMENTS + + +
      INTEGER    NBLKS,
     $           FRACBX(5,NBLKS),FRACFP(8),IVL1,TMPFG,LGTFP,
     $           MSTBFP(3,NBLKS),MSTFP(5),MSTLFG,PSTFG,SEDFG,SLTFP,
     $           SOSDBX(NBLKS),SOSDFP,ULTFP
      REAL       FRAC(8),FRACB(5,NBLKS),LGTMP,MST(5),MSTB(3,NBLKS),
     $           SLTMP,SOSDB(NBLKS),SOSED,ULTMP
C
C     + + + ARGUMENT DEFINITIONS + + +
C     SEDFG  - ???
C     NBLKS  - number of blocks into which pls zones will be subdivided
...
CCBAY  added by gary shenk 8/97 for newton's method
      REAL         FofC,FPRIME
C
C
C     + + + EQUIVALENCES + + +
...
C     + + + INTRINSICS + + +
      INTRINSIC  ABS
C
CCBAY iteration routine changed by gary shenk 8/97  found non-convergence
CCBAY  even after 999 iterations in some cases.  Using equations found in
CCBAY  following lines
CCBAY    X= KF*(C**NI)+ XFIX
CCBAY    FRAC= (TCM-FIXCAP)/DENOM
CCBAY        where
CCBAY     DENOM= (X*SOILM+ C*MOISTM)*1.0E-06- FIXCAP
CCBAY     and FRAC should converge to 1
CCBAY    therefore TCM-FIXCAP = (X*SOILM+ C*MOISTM)*1.0E-06- FIXCAP
CCBAY  solving:  X = (TCM/1.0E-06 - C*MOISTM)/SOILM
CCBAY  remove X: f(C) = KF*(C**NI) + C*MOISTM/SOILM + XFIX - TCM*1.0E6/SOILM =0
CCBAY  solving by newton's method requires first derivative
CCBAY  f'(C) = KF*NI*(C**(NI-1)) + MOISTM/SOILM
CCBAY  trials determined that newton's method failed sometimes as well but that
CCBAY  using a lower convergence criterion could be useful but could result in 
CCBAY  reaching ITMAX more often.  Solution used is to invoke newton's method only
CCBAY  when original method failed.
CCBAY 
C     + + + END SPECIFICATIONS + + +
C
C     error/warn message cluster
...
C       determine if these estimates are acceptable
        RCLOSE= FRAC- 1.0
C
        IF ( (ABS(RCLOSE) .GT. 0.0001) .AND. (COUNT .LE. ITMAX) ) THEN
...
C         estimate solution phase value
          C= C*FRAC
        END IF
      IF ( (ABS(RCLOSE) .GT. 0.0001) .AND. (COUNT .LE. ITMAX) ) GO TO 10
C
C       adjust c and x to account for tolerance in the iterative
C       process
        C= C*FRAC
        X= XFIX+ (X- XFIX)*FRAC
CCBAY  if count is greater than itmax, solution routine failed, try newton
      IF (COUNT .GE. ITMAX) THEN
        COUNT=0
11      CONTINUE
C        determine value of function and derivative
          IF (C.LE.0.) C=0.00001
        FofC=KF*(C**NI) + C*MOISTM/SOILM + XFIX - TCM*1.0E6/SOILM
 
        FPRIME= KF*NI*(C**(NI-1)) + MOISTM/SOILM
 
 
 
        IF ( (ABS(FofC) .GT. 0.00001) .AND. (COUNT .LE. ITMAX) ) THEN
C         get ready for new iteration
C
          COUNT= COUNT+ 1
C
C         estimate solution phase value
        ...
        END IF
C
        IF ( (ABS(FofC).GT.0.00001) .AND. (COUNT.LE.ITMAX)) GO TO 11
CCBAY  estimate X
        X= KF*(C**NI)+ XFIX
...
C
      SUBROUTINE   CYLDPM
     I                    (MESSU,MSGFL,NUM1,UUNITS,OUTLEV,NUM2,NDAY,
     I                     SOILD,
     M                     ECOUNT,
...
C
C     + + + DUMMY ARGUMENTS + + +
      INTEGER MESSU,MSGFL,NUM1,UUNITS,OUTLEV,NUM2,NDAY(12),ECOUNT,NCRP,
...
C     NUM1   - table number of soil-data2
C     UUNITS - system of units   1-english, 2-metric
C     OUTLEV - run interpreter output level
C     NUM1   - table number of crop-dates
...
C
C     + + + LOCAL VARIABLES + + +
      INTEGER   I,J,I0,LNDAY(12),NC,MTH,TCDAY(13),IVAL(13),I1,WRAPFG,JJ,
     #          PFG,HFG,SCLU,SGRP,LAPFG
      REAL      R0
C
C     + + + INTRINSICS + + +
      INTRINSIC FLOAT
C
C     + + + EXTERNALS + + +
      EXTERNAL ZIPI,ZIPR,COPYI,CKCRDT,OMSTI,OMSG
...
C     convert from in/in or in/cm to inches
      DO 10 J= 1, 4
c!pwout WILTPT(I)= WILTPT(I)* SOILD(I)
        WILTPT(J)= WILTPT(J)* SOILD(J)   ! pw change
 10   CONTINUE
C
C     get cropping data - table-type crop-dates
      I= 13
      CALL ITABLE (NUM2,I1,I,UUNITS,
     M             IVAL)
      NCRP= IVAL(1)
      DO 30 J= 1, 3
        DO 20 I= 1, 4
