...
c         factk = pwsoildd*0.015 !.1,.2! for input M-M max to be pound /acre/in
          factk = 1.0             !!! for input M-M max to be mg/l 
c  ! pw end added
...
c ! pw 8/9/06 adding, because input max is mass/area/day for per depth soil. 
              ...
              KSINIK = KSINIK * factk           ! otherwise, input *0.01  
              KSIAMK = KSIAMK * factk           ! otherwise, input *0.01  
c ! pw end added 
555            continue 
            ELSE
C             zero rates
...
ctmp           if( SMST    .le. fldcp) then ! less than field capacity
...
ctmp       NICONC = ( SMST /(fldcp))*NO3/MOISTM*1.0E6   ! cif 
ctmp            KSINIKe=KSINIK !*SMST   /(fldcp) !effective mass max immobil
ctmp            immni = KSINIKe *NICONC/(NICONC+CSINI)
ctmp           else
cifout     NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
           NICONC =        NO3/MOISTM*1.0E6    ! cif 
                 immni= KSINIK*NICONC / (NICONC + CSINI)   ! pounds 
                IMMNI= IMMNI*MOISTM/1.0E6    ! ctmp adding 
ctmp           endif
              goto 332
331           continue   
c !!!   M-M mg/l/day 
...
ctmp       AMCONC = ( SMST /(fldcp))*AMSU/MOISTM*1.0E6   ! cif 
ctmp            KSIAMKe=KSIAMK !*SMST   /(fldcp) !effective mass max immobil
ctmp            immam = KSIAMKe *AMCONC/(AMCONC+CSIAM)
ctmp           else
cifout     AMCONC = factmg*AMSU/pwmoistV ! MOISTM*1.0E6
           AMCONC =        AMSU/MOISTM*1.0E6    ! cif 
                 immam= KSIAMK*AMCONC / (AMCONC + CSIAM)
                IMMAM= IMMAM*MOISTM/1.0E6 ! ctmp adding  
ctmp           endif
              goto 334
333           continue   
c !!!   M-M mg/l/day 
...
C
            IF (SMST .GE. WILTPT) THEN
               unptgg = min(nuptg+ndefc, nuptg*nmxrat)
              goto 311     ! goto as old code
...
c !! using next  utntot = (nuptg + ndefc)*(smst-wiltpt)/(fldcp-wiltpt)
                 utntot = unptgg*(smst-wiltpt)/(fldcp-wiltpt)
...
C             nitrate present
c ! pw adding !!!!
ctmp         if(smst .lt. wiltpt ) then   ! compare depth of water
ctmp             utni = 0.0 
ctmp         else          ! greater than wilting point 
...
cifout      NICONC = factmg*(  SMST  /(fldcp))*NO3/pwmoistV ! MOISTM*1.0E6 
ctmp        NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6  ! cif 
ctmp            KSUNIKe=KSUNIK  *smst/(fldcp) ! effective mass max uptake
ctmp            utni = KSUNIKe *NICONC/(NICONC+CSUNI)
ctmp           else
cifout      NICONC = factmg*NO3/pwmoistV ! MOISTM*1.0E6
            NICONC =                    NO3/MOISTM*1.0E6  ! cif
                 utni = KSUNIK*NICONC / (NICONC + CSUNI)  ! pound M-M 
                utni = utni * MOISTM/1.0E6        ! ctmp adding
ctmp           endif 
...
cifout      AMCONC =factmg*(  SMST  /(fldcp))*AMSU/pwmoistV ! MOISTM*1.0E6 
ctmp        AMCONC =(  SMST  /(fldcp))*AMSU/MOISTM*1.0E6   ! cif
ctmp            KSUAMKe=KSUAMK  *smst /(fldcp) ! effective mass max uptake
ctmp            utam = KSUAMKe *AMCONC/(AMCONC+CSUAM)
ctmp           else 
cifout      AMCONC =factmg*AMSU/pwmoistV ! MOISTM*1.0E6
            AMCONC =       AMSU/MOISTM*1.0E6    ! cif
                 utam = KSUAMK*AMCONC / (AMCONC + CSUAM)
                utam = utam * MOISTM/1.0E6        ! ctmp adding
ctmp           endif 
                goto 444
...
cifout           AMCONC =factmg*AMSU/pwmoistV ! MOISTM*1.0E6
                 AMCONC =       AMSU/MOISTM*1.0E6    ! cif 
                 utam = KSUAMK*AMCONC/(AMCONC + CSUNI)/factk  ! mg/l 
