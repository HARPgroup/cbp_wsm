c     ===================================================================
      SUBROUTINE PART(sim,ndaymax,ndays,watershedfile,ny,nm,nd,catcode,
     O                bsim,qsim)
C
...
c
********* the following variables were added to convert to a 
*********  subroutine with no reads other than the watershedfile
      integer ndaymax  ! vector dimension
      integer ndays    ! number of values in the vector
      real sim(ndaymax),qsim(ndaymax),bsim(ndaymax) ! input and output
      integer ny,nm,nd,storeyear  ! date
*********************************
...
c
      ...
      CHARACTER*100 ofile,fname1,fname2,watershedfile
      CHARACTER*90 LINE
...
c
c      OPEN (UNIT=9,FILE=ofile)
c
c      do i = 1,100
c        if (ofile(i:i).ne.' ') last = i
c      end do
c
c      catcode = ofile(last-17:last-5)
c      fname1 = ofile(:last-4)//'bflw'
c      fname2 = ofile(:last-4)//'qflw'
c
c      OPEN (UNIT = 21,FILE = fname1)
c      OPEN (UNIT = 22,FILE = fname2)
c
...
C      IFRSTYR=0
       ...
       storeYEAR = IYEAR-IBEFORE
       Q(storeYEAR,IMONTH,IDAY) = sim(nd)
       do nd = 2,ndays
...
c
c   35 READ (9,'(A90)',END = 37) LINE
c      do i=1,90
c	if (LINE(i:i).eq.CHAR(13)) LINE(i:i)=' '
c      end do
c
c      READ (LINE,*) IYEAR,IMONTH,IDAY,IHR,IMIN,QRD
c
C      IF(IFRSTYR.EQ.0) IFRSTYR=IYEAR
C      IYEAR = IYEAR-IBEFORE
c      DO 36 J=1,8
c        IF(FLOCHAR(J:J).EQ.CHAR(9)) FLOCHAR(J:8)= ' '
c   36   CONTINUE
c      READ(FLOCHAR,14) FLONUM
c
C      Q(IYEAR,IMONTH,IDAY) = QRD
C      GO TO 35
C   37 CONTINUE
c
      ILSTYR= IYEAR
