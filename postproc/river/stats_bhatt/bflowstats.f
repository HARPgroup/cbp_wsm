...
************************************************************************
      ...
      subroutine bflowstats(ob,si,nday2,rscen,rseg,year1,year2,suffix,
...
******** variables for low10
      ...
      integer ord(ndaymax), ord2(ndaymax)
...
********* calculate biase for low 10% of observed data
      ...
      call qsortr(ord2,ndays,sim) ! added by ICPRB, for option of unpaired low-flows
      low10obs = 0.0
      low10sim = 0.0
      do nd = 1,ndays/10  ! original p52
C      do nd = 1, ndays/2  ! ICPRB try - redefine lo10 as a lo50 stat
        low10obs = low10obs + obs(ord(nd))
        low10sim = low10sim + sim(ord(nd)) ! p52 original, with paired low flows
C        low10sim = low10sim + sim(ord2(nd)) ! ICPRB try - use unpaired low flows
      end do
...
C        low05sim = low05sim + sim(ord2(nd)) ! unpaired lowflows; maybe problem here if n's different
...
C              if (qplus1.lt.obar) then ! ICPRB try - focus on lower flows
                nqtobs = nqtobs + 1 
                allqt(nqtobs) = q
...
C        call calcRC(allqt,allqtplus1,nqtobs,ndaymax,
        call bcalcRC(allqt,allqtplus1,nqtobs,ndaymax,
     O              mobs2,mobs3,err)
        if (err.ne.0) go to 993
      else
        mobs = -9.0
...
C               if (qplus1.lt.sbar) then ! ICPRB try
                nqtsim = nqtsim + 1 
                allqt(nqtsim) = q
...
C        call calcRC(allqt,allqtplus1,nqtsim,ndaymax,
        call bcalcRC(allqt,allqtplus1,nqtsim,ndaymax,
     O              msim2,msim3,err)
        if (err.ne.0) go to 994
      else
        msim  = -9.0
      ...
      if (mobs2.le.0.) then
        ...
        indind2 = msim2/mobs2
c        indind2 = (1-msim2)/(1-mobs2)    !ICPRB consistent with calcRC from slope ln q
      end if
      if (mobs3.le.0.) then
        indind3 = -9.0
      else
        indind3 = msim3/mobs3
c        indind3 = (1-msim3)/(1-mobs3)    !ICPRB consistent with calcRC from slope ln q
      end if
      ...
      do nd = 2,ndays-1
        if (obmonth(nd).eq.12.or.obmonth(nd).le.2) then
          ...
          if (q.lt.qminus1) then  ! not peak and falling
            if (qplus1.lt.q) then  ! still decending
              if (qplus1.gt.0.01*sbar) then  ! not at rock bottom
C              	if (qplus1.lt.sbar) then ! ICPRB try - focus on lower flows
                nqtsim = nqtsim + 1
                allqt(nqtsim) = q
                allqtplus1(nqtsim) = qplus1
C               end if ! ICPRB try
...
C        call calcRC(allqt,allqtplus1,nqtobs,ndaymax,
     ...
     O              mobs6,mobs7,err)
      ...
      if (msim4.le.0..or.mobs4.le.0.) then
        ...
        indind4 = msim4/mobs4
c        indind4 = (1-msim4)/(1-mobs4)    !ICPRB consistent with calcRC from slope ln q
      end if
      if (msim4.le.0..or.mobs5.le.0.) then
        indind5 = -9.0
      else
        indind5 = msim5/mobs5
c        indind5 = (1-msim5)/(1-mobs5)    !ICPRB consistent with calcRC from slope ln q
      end if
      if (msim6.le.0..or.mobs6.le.0.) then
        indind6 = -9.0
      else
        indind6 = msim6/mobs6
c        indind6 = (1-msim6)/(1-mobs6)    !ICPRB consistent with calcRC from slope ln q
      end if
      if (msim7.le.0..or.mobs7.le.0.) then
        indind7 = -9.0
      else
        indind7 = msim7/mobs7
c        indind7 = (1-msim7)/(1-mobs7)    !ICPRB consistent with calcRC from slope ln q
      end if
...
**    yesterday's flow                                                **
************************************************************************
C ICPRB change, Jan 2010: change to a def of recession coeff that's more usable with nonlinear model.
C      subroutine calcRC(allqt,allqtplus1,nqt,ndaymax,     !doesn't work to use same subroutine name
      subroutine bcalcRC(allqt,allqtplus1,nqt,ndaymax,
     O                  ave,mdn,err)
      ...
      real ave,mdn
      real kvary, beta   !ICPRB - for temporary estimates for kvary, beta
      real mean,median
      ...
      beta = 1.0 + kvary
      do n = 1,nqt
        ratio(n) = allqtplus1(n)/allqt(n) ! original eq. for e^-k est.
c        ratio(n) = 2.0*(allqt(n)-allqtplus1(n))
c     O /(allqt(n)+allqtplus1(n)) ! ICPRB alternative - based on slope of log(q)
C        ratio(n) = (1/beta)*(allqt(n)-allqtplus1(n))
C     O /(allqtplus1(n)*(allqt(n))**(kvary/beta) )  ! ICPRB alternative - based on nonlin k
c       ratio(n) = (allqt(n)-allqtplus1(n))/100.0   ! ICPRB alternative - straight slope of q
      end do
C end ICPRB change
