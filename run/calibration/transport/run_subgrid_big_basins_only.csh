#!/bin/csh
########################################################################
##  makes regional adjustment factors to convert EOF to EOS           ##
...
#   GET SCENARIO 
  if (${#argv} != 6) then
    if (${#argv} != 5) then
      echo ' '
      echo 'usage:  run_subgrid_big_basins_only.csh river_scenario calibration_scenario basin year1 year2 '
      echo 'usage:  run_subgrid_big_basins_only.csh river_scenario calibration_scenario basin year1 year2 tree'
      echo ' '
      exit
    endif
  endif
  ...
  echo $rscen $calscen $basin $year1 $year2 | $tree/code/bin/EOF_to_EOS_mass_balance_and_instantaneous_bias_method_regional_basins.exe
