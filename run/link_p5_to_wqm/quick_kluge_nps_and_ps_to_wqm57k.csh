#!/bin/csh
  ...
  if (${#argv} != 5 ) then
    echo 'usage: run_nps_and_ps_to_wqm57k.csh BASE_DIRECTORY RIVER_SCENARIO POINT_SOURCE_METHOD YEAR1 YEAR2'
  ...
  set fnam = '../src/quick_kluge_p5_and_ps_to_wqm57k/date.inc'
  echo "       parameter ( year1 = $year1 )" > $fnam
  echo "       parameter ( year2 = $year2 )" >> $fnam
  echo 'compiling'

  f77 -c -o ../src/lib/dsn/dsn_utils.o ../src/lib/dsn/dsn_utils.f
  cd ../src/quick_kluge_p5_and_ps_to_wqm57k/
  compile
  cd ../../run/

  mkdir -p ../out/kluge_${rscen}/

  echo 'running'

  echo $rscen $psmethod $hotstart | ../bin/quick_kluge_p5_ps_to_wqm57k.exe
  ...
  echo $rscen $psmethod $hotstart | ../bin/quick_kluge_p5_nps_to_wqm57k.exe
