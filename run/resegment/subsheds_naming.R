...
if (length(argst) < 4) {
    message("Use: Rscript subsheds_naming.R current_segid(hydrocode minus prefix) full_path_rivernames.csv model_version [downstream_riverseg(will infer from current riverseg if blank)")
    q()
}
...
message(paste("searching for segment hydrocode:", paste("vahydrosw_wshed",subshed,sep = "_")))
sub <- RomFeature$new(
  ds,
  list(
...
if (!is.na(sub$hydroid)) {
   message(paste("Found subshed with hydroid", sub$hydroid))
} else {
   message(paste("Could not find", paste("vahydrosw_wshed",subshed,sep = "_"),"with ftype=vaydro and bundle=watershed"))
   q("no")
}
...
message(paste("Model attached is pid:", model6$pid))
