...
#   GET SCENARIO, BASIN, and TREE
  ...
  if (${#argv} != 3) then
    if (${#argv} != 2) then
      echo ' '
      echo 'usage:  run_scenario_postproc.csh scenario basin'
      echo ' or     run_scenario_postproc.csh scenario basin tree'
      echo ' '
      exit
    endif
  endif
...
# LOGICAL VARIABLES0TO GENERATE DIFFERENT TYPES OF OUTPUT.  0 = NO OUTPUT, 1 = OUTPUT
  set Rallmonth =  0
  set Rannual =    0
  set Raveann =    1
  set Rdaily =     0

####### do not turn on, these may need some checking
  set TSSRating =  0
  set Rstats =     0
  set RdailyPart = 0
...
####### DO RIVER CALCULATIONS NEXT
  foreach seg ($segments)
    ...
    if ($Rstats) then
      echo $scenario $seg, $AVEYEAR1, $AVEYEAR2, $npeaks $window | $tree/code/bin/Rstats.exe
      if (-e problem) then
        echo ' '
        cat problem
        exit
