...
echo "landseg,landuse,year,Snh4,Sno3,Sslorn,Sdlorn,Ssrorn,Sdrorn,Spltn,Fno3Sout,Fno3Uout,Fno3Gout,Fnh4Sout,Fnh4Uout,Fnh4Gout,FlornSout,FlornUout,FlornGout,FrornSout,FrornUout,FrornGout" > $stout
...
for j in `ls $files`; do
  ...
  echo "Parsing $j "
   ...
   for i in `fgrep -n "*** NITR ***" $fname | awk -F: '{print int($1)}'`; do 
      ...
      LNreNo=`expr $sline + 45`
      BGreNo=`expr $sline + 53`
      ...
      # PLANT UPTAKE
      AGnh4upNo=`expr $sline + 61`
      BGnh4upNo=`expr $sline + 65`
      AGno3upNo=`expr $sline + 69`
      BGno3upNo=`expr $sline + 73`
      ...
      dntrNo=`expr $sline + 77`
      # nitrification line
      ntrNo=`expr $sline + 81`
      # NH3 immobilization line
      nh4imbNo=`expr $sline + 85`
      # ORNG minilization line
      ognminNo=`expr $sline + 89`
      # NO3 immobilization line
      no3imbNo=`expr $sline + 93`
      # Labile/Refer ORGN conversion
      lbtorefNo=`expr $sline + 97`
      ...
      echo $landseg,$landuse,$syear,$Snh4,$Sno3,$Sslorn,$Sdlorn,$Ssrorn,$Sdrorn,$Spltn,$Fno3Sout,$Fno3Uout,$Fno3Gout,$Fnh4Sout,$Fnh4Uout,$Fnh4Gout,$FlornSout,$FlornUout,$FlornGout,$FrornSout,$FrornUout,$FrornGout >> $stout
