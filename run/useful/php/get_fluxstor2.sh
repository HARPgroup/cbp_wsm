...
for j in `ls $files`; do
  ...
  echo "Parsing $j "
   ...
   for i in `fgrep -n "*** NITR ***" $fname | awk -F: '{print int($1)}'`; do 
      ...
      Splant=`echo $storline | awk '{print $9}'`
      
      # FLUXES
      Fnh4No=`expr $sline + 34`
      Fno3No=`expr $sline + 35`
      ...
      Fnh4Qout=`echo "$Fnh4Sout $Fnh4Uout" | awk '{print($1 + $2)}'`
      Fno3Qout=`echo "$Fno3Sout $Fno3Uout" | awk '{print($1 + $2)}'`
      FdinQout=`echo "$Fno3Qout $Fnh4Qout" | awk '{print($1 + $2)}'`
      FdinGout=`echo "$Fno3Gout $Fnh4Gout" | awk '{print($1 + $2)}'`
      
      # TRANSFORMATIONS
      TbglretNo=`expr $sline + 47`
      TbgrretNo=`expr $sline + 51`
      Tnh4upNo=`expr $sline + 55`
      Tno3upNo=`expr $sline + 59`
      TdenitNo=`expr $sline + 63`
      TnitNo=`expr $sline + 67`
      Tnh4immNo=`expr $sline + 71`
      TminNo=`expr $sline + 75`
      Tno3immNo=`expr $sline + 79`
      Tl2rNo=`expr $sline + 83`
      TvolNo=`expr $sline + 87`

      Tbglretline=`head -n $TbglretNo $fname | tail -n 1`
      Tbgrretline=`head -n $TbgrretNo $fname | tail -n 1`
      Tnh4upline=`head -n $Tnh4upNo $fname | tail -n 1`
      Tno3upline=`head -n $Tno3upNo $fname | tail -n 1`
      Tdenitline=`head -n $TdenitNo $fname | tail -n 1`
      Tnitline=`head -n $TnitNo $fname | tail -n 1`
      Tnh4immline=`head -n $Tnh4immNo $fname | tail -n 1`
      Tminline=`head -n $TminNo $fname | tail -n 1`
      Tno3immline=`head -n $Tno3immNo $fname | tail -n 1`
      Tl2rline=`head -n $Tl2rNo $fname | tail -n 1`
      Tvolline=`head -n $TvolNo $fname | tail -n 1`

      Tbglret=`echo $Tbglretline | awk '{print $5}'`
      Tbgrret=`echo $Tbgrretline | awk '{print $5}'`
      Tnh4up=`echo $Tnh4upline | awk '{print $5}'`
      Tno3up=`echo $Tno3upline | awk '{print $5}'`
      Tdenit=`echo $Tdenitline | awk '{print $5}'`
      Tnit=`echo $Tnitline | awk '{print $5}'`
      Tnh4imm=`echo $Tnh4immline | awk '{print $5}'`
      Tmin=`echo $Tminline | awk '{print $5}'`
      Tno3imm=`echo $Tno3immline | awk '{print $5}'`
      Tl2r=`echo $Tl2rline | awk '{print $5}'`
      Tvol=`echo $Tvolline | awk '{print $5}'`
      
      # calculate some totals
      Tbgret=`echo "$Tbglret $Tbgrret" | awk '{print($1 + $2)}'`
      Tup=`echo "$Tnh4up $Tno3up" | awk '{print($1 + $2)}'`
      Timm=`echo "$Tnh4imm $Tno3imm" | awk '{print($1 + $2)}'`
      ...
      echo $prj,$scn,$4,$landseg,$landuse,$syear,$Snh4,$Sno3,$Sslorn,$Sdlorn,$Ssrorn,$Sdrorn,$Splant,$FdinQout,$FdinGout,$FlornQout,$FlornGout,$FrornQout,$FrornGout,$Tbgret,$Tup,$Timm,$Tdenit,$Tvol,$Tl2r >> $anout
