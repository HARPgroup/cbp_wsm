...
#   GET SCENARIO, BASIN, and TREE
  ...
  if (${#argv} != 5) then
    if (${#argv} != 4) then
      echo ' '
      echo 'usage:  run_postproc_pssep_aveann.csh scenario segment aveyear1 aveyear2'
      echo ' or     run_postproc_pssep_aveann.csh scenario segment aveyear1 aveyear2 tree'
      echo ' '
      exit
    endif
  endif
...
####### FIND DATA INPUTS (POINT SOURCE, AT DEP, SEPTIC)
  foreach seg ($segments)
    ...
    if ($DAT) then
      echo $scenario $seg $DATdaily $DATmonthly $DATannual $DATaveann $AVEYEAR1 $AVEYEAR2 | $tree/code/bin/PSSEP.exe
      if (-e problem) then
        echo ' '
        cat problem
        exit
