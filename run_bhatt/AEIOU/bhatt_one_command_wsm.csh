...
if ($RUN_POSTPROC == 1) then
     ...
     if( $CDF == 0 ) then
          ...
          foreach segment ( $segments )
               ...
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$iSLURM_JOB_NAME bhatt_run_postproc_river_annual_oneseg.csh $scenario $segment $year1 $year2 $user-$iSLURM_JOBID-`printf "%04d" $i` &
...
AEIOU:
          ...
          ## 0. CALCULATE FACTOR
          ## 1. APPLY FACTOR
          ## 2. RUN SEGMENT ALONG WITH DOWNSTREAM SEGMENTS
          ## 3. ARCHIVE ANNUAL OUTPUTS
          ...
          echo "/modeling/gb604/output/river/annual/20190212_AEIOU/$data[3]_$data[2]_$data[4].csv"
          ls -al /modeling/gb604/output/river/annual/20190212_AEIOU/
          if ( -e /modeling/gb604/output/river/annual/20190212_AEIOU/$data[3]_$data[2]_$data[4].csv ) rm /modeling/gb604/output/river/annual/20190212_AEIOU/$data[3]_$data[2]_$data[4].csv
          foreach PARAM ( $PARAMS )
             foreach segment ( $segments )
                set RID = `echo $segment | awk '{print substr($0,9,4)}'`
                if ( $RID != "0000") then
                   foreach line ("`cat ../../output/river/annual/$scenario/${segment}_${PARAM}_year.prn`")
                      echo "$data[3],$data[2],$line" >> /modeling/gb604/output/river/annual/20190212_AEIOU/$data[3]_$data[2]_$data[4].csv
                   end
                endif
             end
          end
