...
####### DO DELIVERED CALCULATIONS LAST
  ...
  foreach seg ($segments)
    ...
    if ($TFS) then
      echo $scenario, $seg, $TFSmonthly, $TFSannual, $TFSaveann, $AVEYEAR1, $AVEYEAR2 | $tree/code/bin/TFS_P6.exe
      if (-e problem) then
        echo ' '
        cat problem
        exit
  ...
  foreach seg ($segments)
    ...
    if ($DEL) then
      echo $scenario, $seg, $DELmonthly, $DELannual, $DELaveann, $AVEYEAR1, $AVEYEAR2 | $tree/code/bin/DEL_P6.exe
      if (-e problem) then
        echo ' '
        cat problem
        exit
