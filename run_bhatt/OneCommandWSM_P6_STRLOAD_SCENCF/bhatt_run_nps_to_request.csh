...
#  cd $tree/tmp/scratch/temp$$/
  ...
  if ( $year1 <= 2000 && $year2 <= 2000 ) then
     echo $rscen $hotstart $Request $year1 $year2 $CH3D | $tree/code/bin/p5_nps_to_request_y1y2_P6_RPA_RIB_PSMETHOD_8401.exe
  else if ( $year1 > 2000 && $year2 > 2000 ) then
     echo $rscen $hotstart $Request $year1 $year2 $CH3D | $tree/code/bin/p5_nps_to_request_y1y2_P6_RPA_RIB_PSMETHOD_9514.exe
  else
...
######### self-documentation
  set notefile =  ${tree}/output/wqm_input/$Request/$rscen/AutoNotesNPS_${year1}_${year2}
  if (-e $notefile) then
    rm $notefile
  endif

  echo 'This NPS WQM Input data for ' ${Request} ' was created by' $user ' on ' `date` >> $notefile
  echo ' ' >> $notefile
