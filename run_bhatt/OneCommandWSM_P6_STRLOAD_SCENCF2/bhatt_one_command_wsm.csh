...
if ($RUN_ETM1 == 1) then
     ...
     foreach segment ($segments)
       ...
       if ( $SKIP_ETMPP == 0 ) then
          ...
          srun --nodes=1 --ntasks=1 --exclusive --unbuffered --job-name=`printf "%03d" $i`_$iSLURM_JOB_NAME bhatt_run_etm_and_land_and_dat_simultaneously_oneseg.csh $scenario $segment $year1 $year2 $user-$iSLURM_JOBID-`printf "%04d" $i` &
       else
