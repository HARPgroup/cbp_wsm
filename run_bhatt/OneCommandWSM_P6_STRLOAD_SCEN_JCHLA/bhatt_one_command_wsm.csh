...
if ($DISTRIBUTE_SB_DATA == 1) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=SB_DATA_$iSLURM_JOB_NAME  bhatt_distribute_sb_data.csh $SB_FOLDER $SB_VERSION $SB_SCENARIO OVERWRITE &
     wait
...
if ($RUN_WQM_INPUT == 1) then
     ...
     if ( $PSMETHOD == 'wcell' ) then
        cp -vp ../../output/wqm_input/all_W/$SCENARIO/wsm57k_wsm_ps.*  ../../output/wqm_input/all/$SCENARIO/
     else if ( $PSMETHOD == 'lrseg' ) then
        cp -vp ../../output/wqm_input/all_L/$SCENARIO/wsm57k_wsm_ps.*  ../../output/wqm_input/all/$SCENARIO/
     else
