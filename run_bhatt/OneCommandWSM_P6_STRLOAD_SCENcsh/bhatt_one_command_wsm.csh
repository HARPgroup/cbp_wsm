...
set iSLURM_JOBID    = `uuidgen`
set iSLURM_JOB_NAME = $SCENARIO
...
srun aws_wsm_script_init.sh $PROJECT_HOME
...
if ($RUN_ETM1 == 1) then
     ...
     foreach segment ($segments)
       ...
       if ( $SKIP_ETMPP == 0 ) then
          ...
          #   sleep 5
          #endif
          srun --threads-per-core=2 --ntasks-per-node=32 --cpus-per-task=1 --unbuffered --job-name=$iSLURM_JOB_NAME bhatt_run_etm_and_land_and_dat_simultaneously_oneseg.csh $scenario $segment $year1 $year2 $user-$iSLURM_JOBID-`printf "%04d" $i` &
       else
