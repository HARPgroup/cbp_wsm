...
if ($DISTRIBUTE_SB_DATA == 1) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=SB_DATA_$SLURM_JOB_NAME  bhatt_distribute_sb_data.csh $SB_SCENARIO &
     wait
     ...
     if( $SEPTIC_WDMS == 1) then
          ...
          srun --nodes=1 --ntasks=1 --exclusive --job-name=SB_DATA_$SLURM_JOB_NAME  bhatt_create_scenario_septic_wdms_from_scenario_csv.csh $SCENARIO $user-$SLURM_JOBID-SEPTIC_WDMS &
          wait
...
if ($POINT_SOURCE_WDMS == 1) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=PS_DATA_$SLURM_JOB_NAME  Clean_PS_TextFiles.csh $PS_DATAVERSION $WWTP tempWWTP &
     srun --nodes=1 --ntasks=1 --exclusive --job-name=PS_DATA_$SLURM_JOB_NAME  Clean_PS_TextFiles.csh $PS_DATAVERSION $IND  tempIND  &
     srun --nodes=1 --ntasks=1 --exclusive --job-name=PS_DATA_$SLURM_JOB_NAME  Clean_PS_TextFiles.csh $PS_DATAVERSION $CSO  tempCSO  &
     wait

     srun --nodes=1 --ntasks=1 --exclusive --job-name=PS_DATA_$SLURM_JOB_NAME  bhatt_create_scenario_ps_wdms_from_csv.csh $SCENARIO $user-$SLURM_JOBID-PS_WDMS &
     wait
...
if ($RUN_MAKE_LAND_DIR == 1) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=LAND_DIR_$SLURM_JOB_NAME  make_land_directories.csh $SCENARIO $user-$SLURM_JOBID-LAND &
     wait
...
if ($RUN_MAKE_RIVER_DIR == 1) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=RIV_DIR_$SLURM_JOB_NAME  make_river_directories.csh $SCENARIO $user-$SLURM_JOBID-RIVER &
     wait
...
if ($RUN_LUG == 1) then
     ...
     foreach segment ($segments)
          ...
          srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_lug_oneseg.csh $scenario $segment $user-$SLURM_JOBID-`printf "%04d" $i` &
...
if ($RUN_LAND == 1) then
     ...
     foreach segment ($segments)
          ...
          srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_land_oneseg.csh $scenario $segment $user-$SLURM_JOBID-`printf "%04d" $i` &
...
if ($RUN_ETM1 == 1) then
     ...
     foreach segment ($segments)
          ...
          srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_etm_and_land_and_dat_simultaneously_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` &
...
if ($RUN_ETM2 == 1) then
     ...
     foreach segment ($segments)
          ...
          srun -l --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_etm_and_land_and_dat_simultaneously_oneseg_ETM2.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` &
	       ...
	       source ../../config/seglists/${odr}.riv
               ...
               foreach segment ( $segments )
                    ...
                    if($RUN_RIVER_MODE == 'SCENARIO') then
                         srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_river_oneseg.csh $scenario $segment $user-$SLURM_JOBID-`printf "%04d" $i` &
                    endif
                    if($RUN_RIVER_MODE == 'CALIB') then
                         srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_river_oneseg_calib.csh $scenario $segment $user-$SLURM_JOBID-`printf "%04d" $i` &
                    endif
...
if ($RUN_POSTPROC == 1) then
     ...
     if( $CDF == 1 ) then 
          ...
          foreach odr ($order)
               ...
               foreach segment ( $segments )
                    ...
                    srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_PostProcOtherDF_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` $CDF_Sce DEL &
          ...
          while ( $i <= ${#basins} )
               ...
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_summarize_OtherDF_aveann.csh $scenario $CDF_Sce $basins[$i] $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` &
     ...
     if( $CDF == 0 ) then
          ...
          foreach segment ( $segments )
               ...
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_postproc_river_aveann_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` &
          ...
          foreach odr ($order)
               ...
               foreach segment ( $segments )
                    ...
                    srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_tf_df_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` TFS &
          ...
          foreach odr ($order)
               ...
               foreach segment ( $segments )
                    ...
                    srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_tf_df_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` DFS &
          ...
          foreach odr ($order)
               ...
               foreach segment ( $segments )
                    ...
                    srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_tf_df_oneseg.csh $scenario $segment $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` DEL &
          ...
          while ( $i <= ${#basins} )
               ...
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_summarize_output_aveann.csh $scenario $basins[$i] $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` &
...
if ($RUN_WQM_INPUT == 1) then
     ...
     if ( $RUN_CH3D_INPUT == 0 ) then
          ...
          while ( $yyyy <= $year2 )
               ...
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_PS_$SLURM_JOB_NAME bhatt_run_ps_to_request.csh $scenario $Request $yyyy $yyyy $user-$SLURM_JOBID-PS-`printf "%04d" $i` $CH3D &
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_NPS_$SLURM_JOB_NAME bhatt_run_nps_to_request.csh $scenario $Request $yyyy $yyyy $user-$SLURM_JOBID-NPS-`printf "%04d" $i` $CH3D &
     ...
     if ( $RUN_CH3D_INPUT == 1 ) then
          ...
          while ( $y2 <= $year2 )
               ...
               @ y1 = $y2 - 1
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_PS_$SLURM_JOB_NAME bhatt_run_ps_to_request.csh $scenario $Request $y1 $y2 $user-$SLURM_JOBID-PS-`printf "%04d" $i` $CH3D &
               srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_NPS_$SLURM_JOB_NAME bhatt_run_nps_to_request.csh $scenario $Request $y1 $y2 $user-$SLURM_JOBID-NPS-`printf "%04d" $i` $CH3D &
...
if ($RUN_REMOVE_DIRS > 0) then
     ...
     srun --nodes=1 --ntasks=1 --exclusive --job-name=RM_LAND_DIR_$SLURM_JOB_NAME remove_land_directories.csh $SCENARIO $user-$SLURM_JOBID-RM_LAND &
     srun --nodes=1 --ntasks=1 --exclusive --job-name=RM_RIV_DIR_$SLURM_JOB_NAME remove_river_directories.csh $SCENARIO $user-$SLURM_JOBID-RM_RIV &
     if ($RUN_REMOVE_DIRS == 2) then
          srun --nodes=1 --ntasks=1 --exclusive --job-name=RM_ALL_DIR_$SLURM_JOB_NAME remove_everything_else.csh $SCENARIO $CDF_SCENARIO $user-$SLURM_JOBID-RM_ALL &
     endif
     wait 
...
if ($ABC == 1) then
     ...
     ???? srun --nodes=1 --ntasks=1 --exclusive --job-name=LAND_DIR_$SLURM_JOB_NAME  ????.csh $SCENARIO $user-$SLURM_JOBID-LAND &
     wait 
