...
if ($RUN_UNEC == 1) then
     ...
     @ i = 1
     foreach segment ($segments)
          srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$iSLURM_JOB_NAME bhatt_run_unec_oneseg.csh $scenario $segment $UNEC_SPIN_MONTHS $user-$iSLURM_JOBID-`printf "%04d" $i` &
          @ i += 1
     end
