...
#     sbatch --mem-per-cpu=1500 --nice=$PRIORITY --output=$SCENARIO_SLURM --error=$SCENARIO_SLURM --job-name=$SCENARIO --dependency=singleton --nodes=$NUM_NODES --ntasks-per-node=$NUM_CORES $ProcPool --mail-type=FAIL --mail-user=$EMAIL_TO bhatt_one_command_wsm.csh $SCENARIO $SCENARIO_SLURM $SCENARIO_LOG
     ...
     #salloc --exclude=dummy-compute --mem=54000 --mem-per-cpu=1500 --cpus-per-task=1 --nice=$PRIORITY --output=$SCENARIO_SLURM --error=$SCENARIO_SLURM --job-name=$SCENARIO --dependency=singleton --nodes=$NUM_NODES --mincpus=$NUM_CORES --ntasks-per-node=$NUM_CORES --partition=$ProcPool --mail-type=FAIL --mail-user=$EMAIL_TO bhatt_one_command_wsm.csh $SCENARIO $SCENARIO_SLURM $SCENARIO_LOG
     csh bhatt_one_command_wsm.csh $SCENARIO $SCENARIO_SLURM $SCENARIO_LOG
     set iRETURN = $?
