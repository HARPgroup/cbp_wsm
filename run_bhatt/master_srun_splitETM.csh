...
set RUN_ETM2           = 0
set RUN_RIVER          = 1
set RUN_RIVER_MODE	= CALIB
#set RUN_RIVER_MODE     = SCENARIO
...
if($RUN_MAKE_LAND_DIR == 1) then
        ...
        mkdir -p $SlurmDir
        sbatch --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton make_land_directories.csh $SceName
...
if($RUN_MAKE_RIVER_DIR == 1) then
        ...
        mkdir -p $SlurmDir
        sbatch --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton make_river_directories.csh $SceName
...
if($RUN_ETM == 1) then
        ...
        mkdir -p $SlurmDir
        sbatch --mem-per-cpu=1500 --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton --nodes=$NumNodes --ntasks-per-node=$NumCores $ProcPool bhatt_run_etm_and_land_and_dat_simultaneously_parallel_srun.csh $SceName ${Basin}_x1 $AveYear1 $AveYear2 $NumNodes $NumCores
        sbatch --mem-per-cpu=1500 --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton --nodes=$NumNodes --ntasks-per-node=$NumCores $ProcPool bhatt_run_etm_and_land_and_dat_simultaneously_parallel_srun.csh $SceName ${Basin}_x2 $AveYear1 $AveYear2 $NumNodes $NumCores
#BHATT        sbatch --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton --nodes=$NumNodes --ntasks-per-node=$NumCores $ProcPool bhatt_run_etm_and_land_and_dat_simultaneously_parallel_srun.csh $SceName $Basin $AveYear1 $AveYear2 $NumNodes $NumCores
...
if($RUN_ETM2 == 1) then
        cd ../run_bhatt/useful/
        pwd
        set SlurmDir = ../../tmp/${user}-SlurmOutputs/${SceName}_ETM2_$$
        mkdir -p $SlurmDir
        sbatch --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton --nodes=$NumNodes --ntasks-per-node=$NumCores $ProcPool bhatt_run_etm_and_land_and_dat_simultaneously_parallel_srun_splitETM2.csh $SceName $Basin $AveYear1 $AveYear2 $NumNodes $NumCores
        #-vvvv -d4 -x bluefish4       #wait

        #CHECK SUCCESS
        cd ../../run_bhatt/useful/
        #sbatch --output=$CheckDir/$SceName-%j.out --job-name=$SceName --dependency=singleton --nodes=1 --ntasks-per-node=1 $ProcPool bhatt_check_slurm.csh $SlurmDir STEP_ETM2
...
if($CONSTANT_DELIVERY_FACTORS == 'NAY') then
    if($RUN_RIVER == 1) then
	echo 'CONSTANT_DELIVERY_FACTORS == NAY'
        cd ../run_bhatt/useful/
        set SlurmDir = ../../tmp/${user}-SlurmOutputs/${SceName}_RIVER_$$
        mkdir -p $SlurmDir
	echo 'DONE'
	echo 'RIVER MODE = ',$RUN_RIVER_MODE,
	if($RUN_RIVER_MODE == 'SCENARIO') then
		echo 'RIVER MODE = SCENARIO'
		...
		echo 'RIVER MODE = CALIB'
                sbatch --output=$SlurmDir/slurm-%j.out --job-name=$SceName --dependency=singleton --nodes=$NumNodes --ntasks-per-node=$NumCores $ProcPool bhatt_run_river_parallel_srun_calib.csh $SceName $Basin $NumNodes $NumCores
        endif

	#CHECK SUCCESS
        echo 'SBATCH : check status river'
	cd ../../run_bhatt/useful/
