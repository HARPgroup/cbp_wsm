...
while ( $i <= $numforks )
	...
	srun -l --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_etm_and_land_and_dat_simultaneously_multiseg_splitETM2.csh $scenario $year1 $year2 $user-$SLURM_JOBID-`printf "%04d" $i` $xsegments &
	sleep 2
    #############################
#    @ j = $i % 24
#    echo Hello-j= $j
