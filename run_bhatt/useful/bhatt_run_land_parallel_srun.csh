...
# Created By Gopal Bhatt (gopal.bhatt@psu.edu)
    ...
    if (${#argv} < 2) then
      echo ' '
      echo 'usage: bhatt_run_land_parallel_srun.csh  scenario basin year1 year2'
      echo ' '
      exit
    endif
...
while ( $i <= $numforks )
	...
	srun --nodes=1 --ntasks=1 --exclusive --job-name=`printf "%03d" $i`_$SLURM_JOB_NAME bhatt_run_land_multiseg.csh $scenario $user-$SLURM_JOBID-`printf "%04d" $i` $xsegments &
