...
************ CHECK FOR HOT START FILE
      if (hotstart.eq.2) then
        ...
        fnam= '../out/'//rscen(:lenrscen)//'/'//'hotstart_wqm57_nps.bin'
...
*********************** loop over Land-river pairs in cell
     ...
     .                                 + pairwq(nd,ny,nq) * weight(np)
            ...
            if (doatdep) then
              call ttyput(' atdep')
              wdmfnam = tree//'wdm/prad/'//pradscen(:lenpradscen)//
     .                  '/prad_'//tlseg//'.wdm'
...
********* write out annual totals
      write(cy1,'(i4)') year1
      write(cy2,'(i4)') year2
      fnam = '../out/'//rscen(:lenrscen)//'/annual_out_wqm57_nps_'//
     .        cy1//'_'//cy2//'.csv'
      open(dfile,file=fnam,status='unknown',iostat=err)
      if (err.ne.0) go to 991
