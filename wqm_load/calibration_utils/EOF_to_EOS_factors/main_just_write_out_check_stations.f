...
C******************** get simulated load
C            fnam = outdir//'river/aveann/'//rscen(:lenrscen)//'/'//
C     .             rsegsnr//'_'//cy1//'_'//cy2//'.ave'
C            open(11,file=fnam,status='old',iostat=err)
C            if (err.ne.0) go to 991
C
C            read(11,*,err=993) dummy ! header line
C            do 
C              read(11,*,err=993,end=990) tempname,units,staLoad(ns,nl)
C              if (tempname.eq.loadname(nl)) exit
C            end do
C            close(11)
...
**************** atdep, and septic
C      print*,'getting eos loads from eos wdms'
C      call getEOSload(
C     I                rscen,rsegs,nrsegs,year1,year2,
C     I                nloads,loadname,
C     O                EOSland,EOSsep,EOSatdep,EOSps)

************* DETERMINE SGTF *******************************************
************* loop over estimated bias points
C      print*,'determining SGTF'
C
C      do nl = 1,nloads
C        do nr = 1,nrsegs 
C          sgtf(nr,nl) = -999.0
