...
************************************************************************
      ...
      logical pourpoint, bayward, iscalib
      external iscalib
     ...
     O                  pourpoint,downsegs(nd))
      ...
      bayward = .true.
      if (nd.ne.1) then
        do n = 2,nd  ! look for downstream stations
          if (iscalib(downsegs(n))) then
            bayward = .false.
            exit
          end if
        end do
...
************************************************************************
      function iscalib(seg)
      implicit none
      logical iscalib
      character*13 seg,Tseg
      integer tempid
      character*100 fnam

      open(11,file='/model/temp/calibsites.csv',status='old')
      iscalib = .false.
      do
        read(11,*,end=111) Tseg
        if (Tseg.eq.seg) then
          iscalib = .true.
          exit
        end if
