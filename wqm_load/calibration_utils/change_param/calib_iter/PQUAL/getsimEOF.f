...
************ open copy file for records
     ...
     .        '/aveann_summary_'//clu//'_'//version//'.csv'
      ...
      read(dfile,'(a400)',err=996)dline            ! read header line
      call d2x(dline,i)
...
*************** loop over all segments in file, look for active segs
      ...
      do 
        ...
        read(dfile,'(a400)',err=996,end=111)dline    ! read next line
        call d2x(dline,i)
        call rytdos(dline,dfile+1)
      end do
...
995   report(1) = 'data line too long, modify '
      report(2) = fnam
      report(3) = ' or adjust dline in ./pp/src/.../PQUAL/getsimEOF.f'
      go to 999
