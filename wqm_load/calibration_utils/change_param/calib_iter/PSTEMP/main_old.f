...
************ SPECIFICATION ENTRY SECTION
      ...
      call readcontrol_lscens(rscen,lscens)
      do nl = 1,numlu
        call readcontrol_paramscen(lscens(lus(nl)),paramscens(lus(nl)))
        call lencl(paramscens(lus(nl)),lenparamscen(lus(nl)))
      end do
...
************* HOUSEKEEPING
C      call getriver(module,lenmod,calscen,lencalscen,
C     O           rsegs,uniqindex,uniqid,dsid,nrsegs,fnam,err)
C      if (err.eq.991) go to 991
C      if (err.ne.0) go to 998

C      call getland(module,lenmod,calscen,lencalscen,
C     O             lsegs,nlsegs,fnam,err)
C      if (err.eq.991) go to 991
C      if (err.ne.0) go to 998

C      call getorphans(module,lenmod,calscen,lencalscen,
C     O                orphans,surrogates,norphans,fnam,err)
C      if (err.eq.991) go to 991
C      if (err.ne.0) go to 998
...
*********** READ IN LAND-RIVER WEIGHTS
     ...
     I            lsegs,nlsegs,uniqindex,orphans,norphans,
