...
*********************** END DECLARATIONS ******************************
      ...
      read*,calscen,caltype,oneseg,rscen
...
*********** read in calibration stations
      fnam = calscen(:lencalscen)//'_'//caltype(:lencaltype)
      call readCalibSeglist(
     I                      fnam,
     O                      csegs,ncsegs)
      foundoneseg = .false.
      do ns = 1,ncsegs
        if (csegs(ns).eq.oneseg) then
          foundoneseg = .true.
          onesegindex = ns 
          exit
        end if
...
******** NOW FIND ALL OF THE RSEGS THAT ARE ABOVE ONESEG, BUT NOT ABOVE UPSTREAM CSEGS
