#!/bin/csh
  ...
  if (${#argv} != 2 ) then
    echo 'usage: run_p5_to_ICPRB.com BASE_DIRECTORY RIVER_SCENARIO'
  ...
  if (!(-e $tree/wdm/river/$rscen)) then
    echo 'could not find ' $tree/wdm/river/$rscen
...
###  cp  $tree/pp/src/lib/dsn/dsn_utils.o ../src/lib/dsn/
  ...
  mkdir -p ../out/ICPRB/$rscen/
  echo 'running'

  echo $rscen | ../bin/p5_to_ICPRB.exe
