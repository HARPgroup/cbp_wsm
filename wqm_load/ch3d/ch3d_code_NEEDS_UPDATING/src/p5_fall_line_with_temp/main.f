...
************************************************************************
      ...
      real H(-5:366,year1:year2,ncellmax)
     ...
     O                   pairq,pairH)  ! get the data
        ...
        do ny = year1,year2  ! add to big storage variable
          do nd = -5,ndaysinyear(ny)
            ...
            H(nd,ny,nc) = H(nd,ny,nc) + pairH(nd,ny) * fac1
          end do
        end do
...
101   close(dfile-1)
     ...
     .       iostat=err)
      ...
      fnam = 'check_gage_flows_XXXX_YYYY.csv'
      write(fnam(18:21),'(i4)')year1
      write(fnam(23:26),'(i4)')year2
      fnam = '../out/'//rscen(:lenrscen)//'/'//fnam
      open(dfile,file=fnam,status='unknown')
      write(dfile,*)'year, day, 0001s (cfs)'
      do ny = year1,year2
        do nd = 1,ndaysinyear(ny)
      ...
      do ny = year1,year2
        fnam = 'modeled_gages_temp_XXXX.bin'
        write(fnam(20:23),'(i4)')ny
        fnam = '../out/'//rscen(:lenrscen)//'/'//fnam
        open(dfile,file=fnam,status='unknown',form='unformatted',
     .       iostat=err)
        if (err.ne.0) go to 991
        write(dfile) ncells,i,j
        do nd = -5,ndaysinyear(ny)
          do nc = 1,ncells  ! convert to temperature
            if (abs(q(nd,ny,nc)).gt.0.0001) then
              H(nd,ny,nc) = H(nd,ny,nc)/abs(q(nd,ny,nc))*heat2c
            else
              H(nd,ny,nc) = 20
            end if
          end do
          write(dfile) nd
          write(dfile) (H(nd,ny,nc),nc=1,ncells)
        end do
        close(dfile)
      end do
      stop
