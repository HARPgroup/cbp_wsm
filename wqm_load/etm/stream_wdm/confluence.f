************************************************************************
**  The purpose of this program is to prepare write wdm for a stream  **
**    which lies below a confluence.  The major steps are:            **
**      1.  Only run if there is an upstream confluence               **
...
************************************************************************
      ...
      integer n,RvarDown,RvarUp         ! indices

      integer upNexits,Nexits,uplakeflag ! to see if upstream is a reservoir
...
**************END OF DECLARATION **************************************
     ...
     O                         ioscen)
      ...
      Nexits = 3    !   new wdm file should have one set of dsns

      if (nup.ne.0) then                      ! upstream segments exist

        wdmfnam = rseg(:lenrseg-4)//'0003.wdm'
     ...
     O                   upNexits,uplakeflag,upresflag,timestep)
          ...
          call Rmasslink_confluence(
     I                   ioscen,lenioscen,
     I                   upNexits,Nexits,modules,nmod,
     O                   nRvarDown,RdsnDown, RnameDown,
     O                   nRvarUp,RdsnUp,RnameUp)    ! POPULATE nRvar, Rdsn, Rname
     ...
     .              '/stream/'//upstream(n)(:lenup(n))//'.wdm'
          ...
          call wtdate(wdmfil+n,1,RdsnUp(1),2,asdate,aedate,err)    ! tests the date
         
          if (err .ne. 0) go to 994

          do RvarDown = 1,nRvarDown

            call ttyput(' ')
            call ttyput(RnameDown(RvarDown))

            do RvarUp = 1,nRvarUp

         
              if (RnameDown(RvarDown).eq.RnameUp(RvarUp)) then

                if (RvarDown.eq.1) call checkflow(wdmfnam,wdmfil,n,
     .                                RdsnUp(RvarUp),sdate,edate)

                
                call addRwdm(wdmfnam,wdmfil,n,RdsnDown(RvarDown),
     .                       RdsnUp(RvarUp),sdate,edate)
...
991   report(1) = 'Problem with opening wdm for main segment '//wdmfnam
      report(2) = '  Error =    '
