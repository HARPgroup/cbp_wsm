...
C
C
CCBAY subroutine added 1/22/2001 by gary shenk to close last wdm file
      SUBROUTINE   WDFLC1
     I                   (WDMSFL,
     O                    RETCOD)
C
...
C               unusable
 ...
 30         CONTINUE
          END IF
        IF (I.LT.WDMCNT) GO TO 10
      ELSE IF (WDMCNT.LT.1) THEN
C       no file open
        PRINT*,'No Open WDM File to Close'
        RETCOD= -87
      ELSE
C       using wrong subroutine
        PRINT*,' '
        PRINT*,' PROGRAMMING ERROR'
        PRINT*,' USE WDFLCL TO CLOSE THIS FILE'
        RETCOD= -87
      END IF
C
      RETURN
