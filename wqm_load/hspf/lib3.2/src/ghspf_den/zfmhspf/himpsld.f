...
CGaryShenk      IF (PREC.GT.0.0) THEN
CC Our rainfall data set had a high percentage of wet days
CC  which meant that affix rarely worked
        IF (PREC.GT.0.05) THEN
...
C     + + + END SPECIFICATIONS + + +
C
      IF (SOSDFP.GE.1)  PAD(SOSDFP+IVL1)= SOSLD
C
      RETURN
      END
C
...
C     + + + END SPECIFICATIONS + + +
C
      IF (SLDSFP.GE.1)  PAD(SLDSFP+IVL1)= SLDS
C
      RETURN
      END
C
...
C
C     + + + LOCAL VARIABLES + + +
      REAL       MFACTA,PSLDS,PCFLX1,PSLDIF
C
C     + + + OUTPUT FORMATS + + +
 2000 FORMAT (/,' *** SOLIDS ***')
