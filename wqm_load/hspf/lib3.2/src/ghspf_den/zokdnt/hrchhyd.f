...
C     but are read as real values with the other real values
     ...
     M             RVAL(1))
      ...
      SLOPE=  DELTH/LEN
       ength = len * 0.3048 
!   already facter from mi to ft , not to meter. 
!        cssss=DB50
! pw     write(*,*)'hyd:', ength 
...
C     process values in table-type HYDR-INIT
     ...
     M             RVAL(1))
      VOL= RVAL(1)
       vwater = VOL
! initial will not change, not actual use: ft3 
!     write(*,*) 'hyd"','vwater,VOL ,ength '  ! pw 
!     write(*,*) 'hyd"', vwater,VOL ,ength 
! pw adding 
      TBNO= 14
      TBSB= 1
      NVAL= 2
      CALL RTABLE (TBNO,TBSB,NVAL,UUNITS,
     M            RRVAL)
       dvvhr = RRVAL(2) *0.000001 / 43560. 
       dvvhr = dvvhr * DELT60  ! pw 
! because read in it already * vfact.
! pw added
C
      IF (NCATS .LE. 0) THEN
...
C           subroutine DISCH did not find current row
     ...
     I                RCHNO,MESSU,MSGFL,DATIM,SCLU,
     M                HYECNT,
...
C             end of loop iterating on a level
            IF ( (LOVOL .GT. 0.0) .AND. (AVAIL .GT. 0.0) ) GO TO 30
C         end of loop on levels
          IF ( (LOVOL .GT. 0.0) .AND. (LEVND .LT. N) ) GO TO 10
        END IF
      END IF
C
      IF (NFRAC .NE. 0) THEN
...
C     + + + LOCAL VARIABLES + + +
     ...
     $             MESSU,NC,NO,NR,SCLU,SGRP,MSGFL,I0
...
C
C     + + + EXTERNALS + + +
      EXTERNAL   VALNO,OMSG,OMSTI,OMSTC,DUMPER,GETUCI
C
C     + + + INPUT FORMATS + + +
 1010 FORMAT (2I5)
 1020 FORMAT (8F10.0)
...
C         and eight columns and not more than 100 values
          CALL OMSTI (FTABNO)
          CALL OMSTI (NROWS)
          CALL OMSTI (NCOLS)
          SGRP= 15
...
C     + + + LOCAL VARIABLES + + +
      ...
      REAL             SAS,VOLPEV,CORELS(MXEXIT,MXCAT),RTMP,
...
C       thus, hydraulic routing is required
     ...
     I              NXTMON,DAY,NDAYS,ODFVFG,ODGTFG,COLIND,OUTDGT,
     I              COTDGT,RCHNO,MESSU,MSGFL,SCLU,DATIM,MXCAT,NCATS,
     I              MXCUSR,NCOGT,COGTC,COGTE,CFVOL,CFVOLC,CFVOLP,CFVOLF,
...
C
      ROD= 0.
C
C     get function of volume demand
      DO 10 IX= 1,NEXITS
        FG1= ODFVFG(IX)
        IF (FG1 .NE. 0) THEN
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     $                 NCOGT,COGTC(NCOGT),COGTE(NCOGT),ROWPT,HYWCNT(2)
      REAL             KS,COKS,OS(NEXITS),ROS,DELTS,OUTDGT(NEXITS),
     $                 COTDGT(MAXEX,NCATS),RCHTAB(100),
     $                 CORELS(MAXEX,NCATS),O(NEXITS),RO,OVOL(NEXITS),
     $                 ROVOL,COREL(MAXEX,NCATS),CO(MAXEX,NCATS),
     $                 CRO(NCATS),COVOL(MAXEX,NCATS),CROVOL(NCATS)
...
C     AUX1FG - auxiliary calculation flag - 1: calc DEP, STAGE, AVDEP,
C                                                   TWIN, HRAD
C     NROWS  - number of rows in ftable
C     NCOLS  - number of columns in ftable
C     RCHTAB - ftable stored as a scaler
...
C
C     + + + LOCAL VARIABLES + + +
      INTEGER          IX,I0,IFIVE(5),CDUMI(MXCUSR)
      REAL             OD(MXEXIT),ROD,TRO,R0,R1,CDUMR(MXCUSR),RFIVE(5),
     $                 OINTSP,OSEFF(MXEXIT),ROSEFF
...
C       allocate outflows among categories
     ...
     I               CDUMR,ODGTFG,NCOGT,COGTC,COGTE,IFIVE,
     M               CFRAC,
     O               CVOL,CO,CRO,COVOL,CROVOL)
      END IF
C
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     $                 ROWPT,HYECNT(5),HYWCNT(2)
      REAL             CONVFM(12),DELTS,KS,COKS,OS(NEXITS),ROS,
     $                 RCHTAB(100),FACTA1,COLIND(NEXITS),OUTDGT(NEXITS),
     $                 COTDGT(MAXEX,MXCAT),CFVOLF(MXCUSR,NEXITS),
     $                 CORELS(MAXEX,MXCAT),CONVF,O(NEXITS),RO,
     $                 OVOL(NEXITS),ROVOL,COREL(MAXEX,MXCAT),
     $                 CO(MAXEX,MXCAT),CRO(MXCAT),COVOL(MAXEX,MXCAT),
     $                 CROVOL(MXCAT)
...
C       allocate outflows among categories
     ...
     I               CFVOLF,ODGTFG,NCOGT,COGTC,COGTE,FUNCT,
     M               CFRAC,
     O               CVOL,CO,CRO,COVOL,CROVOL)
      END IF
C
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     $                 DATIM(5),ROWPT,HYECNT(5),HYWCNT(2),MXCAT
      REAL             RCHTAB(100),FACTA1,CONVF,COLIND(NEXITS),
     $                 OUTDGT(NEXITS),COREL(MAXEX,MXCAT),DELTS,
     $                 O(NEXITS),RO
...
C     + + + DUMMY ARGUMENTS + + +
     ...
     $                 DATIM(5),SCLU
      REAL             AVDEP,DEP,HRAD,LEN,RCHTAB(100),SAREA,STAGE,
     $                 STCOR,TWID
      DOUBLE PRECISION VOL
C
C     + + + ARGUMENT DEFINITIONS + + +
...
C
C     + + + LOCAL VARIABLES + + +
      INTEGER       I,I1,IC,IX,RLEN,TLEN,TPOS
     ...
     $              VOLIN,PCST(MXCAT,2),PCFLX(MXCAT,MXEXIT+2)
      CHARACTER*8   UNITID
      CHARACTER*132 TBUFF
C
C     + + + EQUIVALENCES + + +
...
C         determine offset for outflow
          IF (AUX1FG.EQ.1 .AND. AUX2FG.EQ.1 .AND. AUX3FG.EQ.1) THEN
            TPOS= TPOS+ 20
          ELSE IF (AUX1FG.EQ.1 .AND. AUX2FG.EQ.1) THEN
            TPOS= TPOS+ 90
          ELSE IF (AUX1FG.EQ.1) THEN
            TPOS= TPOS+ 70
          ELSE IF (AUX1FG.NE.1 .AND. AUX2FG.NE.1 .AND. AUX3FG.NE.1)THEN
            TPOS= TPOS+ 10
          END IF
 ...
 60     CONTINUE
      END IF
C
C     flux output (**** why calc so much earlier PIFLX, etc.)
C     initialize output array
      TBUFF= ' '
      TPOS= 32
      IF (NEXITS .GT. 1) THEN
