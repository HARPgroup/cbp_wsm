...
c ! pw adding !!!!
             if(smst .lt. wiltpt*0.1 ) then   ! compare depth of water
                 utni = 0.0 
             else          ! greater than wilting point 
               if(smst .le. fldcp*0.1) then ! less than field capacity
c !                             or  0.1??? 
            NICONC =                    NO3/MOISTM*1.0E6 ! tests 
            NICONC = (  SMST*10.  /(fldcp))*NO3/MOISTM*1.0E6 
                KSUNIKe=KSUNIK*smst*10./(fldcp) ! effective mass max uptake
