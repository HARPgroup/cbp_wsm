...
c        pwwiltpt = pwwiltpt * pwsoild ! wiltpt from in/in as (in) spec soil
           wiltpt(jw) = 0.1 *1.0 ! 0.5    ! 5   ! hardwireding   (1n/1n) 
...
C     + + + DATA INITIALIZATIONS + + +
      DATA       NH4ID/'AMMO','NIUM','    ','    ','    '/
          BRXNFG = 1   ! pw 
C
c      testout='/model/w510/output/testout.out'
c      write(98,*) YEAR, MD, DAY, SMST
c      write(98,*) SMST, pwsoildd, wiltpt, fldcp 
C     + + + END SPECIFICATIONS + + +
c  ! pw adding  
c !         goto 550          
         if(uunits .eq. 1) then
...
c !                             or  0.1??? 
c                NICONC = NO3/MOISTM*1.0E6
            NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6 
                KSINIKe=KSINIK!*SMST   /(fldcp) !effective mass max immobil
               immni = KSINIKe *NICONC/(NICONC+CSUNI)
caa            else   ! greater than field capacity 
caa              NICONC = NO3/MOISTM*1.0E6
cbb         NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6 
caa              immni= KSINIK*NICONC / (NICONC + CSUNI)
caa           IMMNI= IMMNI*MOISTM/1.0E6
               else
                 NICONC = NO3/MOISTM*1.0E6
...
chh          else          ! greater than wilting point 
               if( SMST    .le. fldcp) then ! less than field capacity
c !                             or  0.1??? 
c                AMCONC =AMSU/MOISTM*1.0E6
            AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6 
                KSIAMKe=KSIAMK!*SMST   /(fldcp) !effective mass max immobil
               immam = KSIAMKe *AMCONC/(AMCONC+CSUAM)
c              else   ! greater than field capacity 
caa              AMCONC =AMSU/MOISTM*1.0E6
caa         AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6 
caa              immam= KSIAMK*AMCONC / (AMCONC + CSUAM)
c             IMMAM= IMMAM*MOISTM/1.0E6
               else 
                 AMCONC =AMSU/MOISTM*1.0E6
...
c ! pw adding !!!!
             if(smst .lt. wiltpt ) then   ! compare depth of water
               ...
               if(smst .le. fldcp) then ! less than field capacity
c !                             or  0.1??? 
c           NICONC =                    NO3/MOISTM*1.0E6 ! tests 
            NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6 
...
c                AMCONC =AMSU/MOISTM*1.0E6
            AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6 
c               KSUAMKe=KSUAMK*smst /(fldcp) ! effective mass max uptake
...
c         END IF
C
c     write(98,*) SMST, pwsoildd, wiltpt, fldcp, utni, utam
c     write(98,*) SMST, pwsoildd, wiltpt, fldcp, utni, utam
C
      write(98,431)
     .     SMST,pwsoildd,wiltpt,fldcp,utni,utam,niconc,amconc,
     .     MOISTM, TMP, pwsoildd, ksunik, NO3, AMSU
          END IF
