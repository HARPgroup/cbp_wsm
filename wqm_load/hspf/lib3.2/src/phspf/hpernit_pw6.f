...
c        pwwiltpt = pwwiltpt * pwsoild ! wiltpt from in/in as (in) spec soil
           wiltpt(jw) = 0.1 ! 5   ! hardwireding   (1n/1n) 
...
c ! pw 8/9/06 adding, because input max is mass/area/day for per depth soil. 
              KSUNIK = KSUNIK * pwsoildd *10. !* 0.1  ! otherwise, input *0.01  
              KSUAMK = KSUAMK * pwsoildd *10. !* 0.1  ! otherwise, input *0.01  
...
c ! pw adding !!!!
cdd          if(smst .lt. wiltpt ) then   ! compare depth of water
cdd              utni = 0.0 
cdd          else          ! greater than wilting point 
cdd            if(smst .le. fldcp) then ! less than field capacity
c !                             or  0.1??? 
caa         NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6 
c           NICONC =                    NO3/MOISTM*1.0E6 ! tests 
caa             KSUNIKe=KSUNIK*smst/(fldcp) ! effective mass max uptake
caa             utni = KSUNIKe *NICONC/(NICONC+CSUNI)
caa            else   ! greater than field capacity 
                 NICONC = NO3/MOISTM*1.0E6
cbb         NICONC = (  SMST  /(fldcp))*NO3/MOISTM*1.0E6 
              ...
              UTNI= UTNI*MOISTM/1.0E6
cdd            else
cdd              NICONC = NO3/MOISTM*1.0E6
cdd              utni = KSUNIK*NICONC / (NICONC + CSUNI)
cdd            endif 
cdd          endif
c  ! pw end added 
442           continue   ! pw 
...
c !                             or  0.1??? 
caa         AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6 
caa             KSUAMKe=KSUAMK*smst /(fldcp) ! effective mass max uptake
caa             utam = KSUAMKe *AMCONC/(AMCONC+CSUAM)
caa            else   ! greater than field capacity 
                 AMCONC =AMSU/MOISTM*1.0E6
cbb         AMCONC = (  SMST  /(fldcp))*AMSU/MOISTM*1.0E6 
              ...
              UTAM= UTAM*MOISTM/1.0E6
cdd            else 
cdd              AMCONC =AMSU/MOISTM*1.0E6
cdd              utam = KSUAMK*AMCONC / (AMCONC + CSUAM)
cdd            endif 
cdd          endif
c  ! pw end added   
444           continue 
