...
************************************************************************
      subroutine monthly(rscen,rseg,wdmfil,Nexits)
...
************* END DECLARATIONS
      ...
      print*,'Monthly River Load ',rscen(:lenrscen),' ',rseg
...
*****************loada
     ...
     .             loadname(np)//'_month.prn'
          open (pltfil,file = pfname, status = 'unknown',iostat = err)
          if (err.ne.0) goto 992
 
          write(pltfil,'(a16,a4)',err=951)'CON   YEAR MO   ',unit(np)
     ...
     .                loadname(np),oldyear,oldmonth,dload
              dload = 0.
              oldyear = asdate(1)
              oldmonth = asdate(2)
            end if
