...
c
c
      write(*,*) 'Enter a 13-digit reach identifier: '
      read(*,17) catcode
      ofile = catcode//'.OFLOW'
c
c     if ofile exists,
      inquire(file=ofile,exist=its)
c
      if (its) then
        call part(ofile)
        ofile = catcode//'.flow'
...
c
      ...
      CHARACTER*19 ofile,fname1,fname2
      CHARACTER*90 LINE
...
c     simulated input flow file is 'catcode.flow'
      catcode = ofile(1:13)
      if (ofile(15:15).eq.'O') then
	fname1 = catcode//'.OBFLW'
	fname2 = catcode//'.OQFLW'
      else if (ofile(15:15).eq.'f') then
	fname1 = catcode//'.bflw'
	fname2 = catcode//'.qflw'
      end if
c
...
C
      ...
      DO 100 IDAY= 1, 31
          ...
          IF(ITEST.EQ.1.AND.ITESTX.EQ.0) THEN
                 ...
                 IF(IBREAK.GT.1) THEN
                   WRITE (*,*) '***************************************'
                   WRITE (*,*) '*** THERE IS A BREAK IN THE STREAM- ***'
                   WRITE (*,*) '*** FLOW RECORD WITHIN THE PERIOD OF **'
                   WRITE (*,*) '*** INTEREST.  PROGRAM TERMINATION. ***'
                   WRITE (*,*) '***************************************'
                   GO TO 1500
                 ENDIF
          ENDIF
...
c      WRITE (*,*) 'READING FILE NAMED watershed_area.csv'
      OPEN (UNIT=11,FILE='watershed_area.csv')
